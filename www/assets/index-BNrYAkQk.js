...
- No markdown, no code fences, just raw JSON`;try{let t=await fetch(CLAUDE_API,{method:`POST`,headers:{"x-api-key":e,"anthropic-version":`2023-06-01`,"content-type":`application/json`},body:JSON.stringify({model:`claude-haiku-4-5-20251001`,max_tokens:4096,messages:[{role:`user`,content:o}]})});if(!t.ok)throw Error(`API ${t.status}`);let n=(await t.json()).content[0].text.replace(/```[\w]*\n?/g,``).trim().match(/\[[\s\S]*\]/);if(!n)throw Error(`No JSON`);JSON.parse(n[0]).forEach(e=>{let t=(g.planner?.events||[]).find(t=>t.id===e.id);t&&e.items&&(t.estimates=e.items.map((e,t)=>({id:`est-${Date.now()}-${t}`,name:e.name,amount:Number(e.amount)||0,category:e.category||`Other`,accepted:!0})))}),window.saveData(g),tn()}catch(e){n&&(n.textContent=`Estimate all (${t.length} events)`,n.disabled=!1),console.error(`Batch estimate error:`,e)}}function L(){let e=document.getElementById(`planner-content`);if(!e)return;g.planner||(g.planner={events:[]});let t=new Date().toISOString().slice(0,10),n=Ut(),r={id:`everyone`,name:`Everyone`,emoji:`👨‍👩‍👧`,dot:`#94a3b8`,bg:`#f1f5f9`,text:`#475569`},i=[...n,r],a=i.map(e=>e.name[0].toUpperCase()),o=i.map((e,t)=>{if(e.id===`everyone`)return`👨‍👩‍👧`;let n=e.name[0].toUpperCase();return a.filter(e=>e===n).length>1?(e.name[0]+(e.name[1]||``)).toUpperCase():n}),s=i.map((e,t)=>{let n=e.id===`everyone`,r=n?window._plannerFilterMembers.size===0:window._plannerFilterMembers.has(e.id),i=!r&&window._plannerFilterMembers.size>0&&!n,a=n?`<span style="font-size:14px">👨‍👩‍👧</span>`:`<span>${o[t]}</span>`;return`<div class="pl-legend-chip ${r?`active`:``} ${i?`dimmed`:``}"
    ...
    </div>`}).join(``),m=Nn(),h=m.map(e=>{let t=e.days<0?`Now!`:e.days===0?`Today!`:e.days===1?`Tomorrow`:`In ${e.days} days`,n=e.days<=0?`#ef4444`:e.days===1?`var(--good)`:e.days<=3?`#f59e0b`:`var(--iris-1)`;return`<div class="pl-nudge-tile">
  ...
  `,()=>{let t=document.getElementById(`f-cat-selected`).value,n=document.getElementById(`f-cat-new`).value.trim()||t;n&&(as(e,n),window.closeModal())})}function ss(e,t){let n=g.categoryGroups.find(t=>t.id===e);n&&(n.categories=n.categories.filter(e=>e!==t),window.saveData(g),renderSettings())}var cs=!1,ls=null;function us(){cs||(ls=JSON.parse(JSON.stringify(g))),cs=!0;let e=document.getElementById(`settings-save-bar`);e&&(e.style.display=`flex`)}function ds(){cs=!1,ls=null,window.saveData(g);let e=document.getElementById(`settings-save-bar`);e&&(e.style.display=`none`);let t=document.getElementById(`settings-save-btn`);if(t){let e=t.textContent;t.textContent=`Saved`,t.style.background=`#10b981`,setTimeout(()=>{t.textContent=e,t.style.background=``},1500)}window.renderAll()}function fs(){ls&&Object.assign(g,ls),cs=!1,ls=null;let e=document.getElementById(`settings-save-bar`);e&&(e.style.display=`none`),renderSettings()}function ps(e){cs&&(confirm(`You have unsaved changes in Settings. Save before leaving?`)?ds():fs())}function ms(e,t){g.settings||(g.settings={}),g.settings[e]=t,us()}function hs(){let e=document.getElementById(`settings-api-key`)?.value.trim();if(!e)return;rt(`toto_ai_key`,e),rt(`toto_ai_key_meta`,JSON.stringify({addedAt:new Date().toISOString(),prefix:e.slice(0,10),suffix:e.slice(-4)}));let t=document.getElementById(`api-key-status`);t&&(t.textContent=`✓ Key saved!`,t.style.color=`var(--success)`,setTimeout(()=>{t.textContent=``,t.style.color=``},2e3));let n=document.getElementById(`api-key-summary`);n&&(n.outerHTML=_s())}function gs(){confirm(`Remove saved API key? Toto and cost estimation will stop working.`)&&(it(`toto_ai_key`),it(`toto_ai_key_meta`),renderSettings())}function _s(){let e=F(`toto_ai_key`);if(!e)return`<div id="api-key-summary"></div>`;let t=JSON.parse(F(`toto_ai_key_meta`)||`{}`),n=t.addedAt?new Date(t.addedAt).toLocaleDateString(`en-AU`,{day:`numeric`,month:`short`,year:`numeric`}):`Unknown date`;return`
    ...
    <div class="summary-mini-grid">
      <div class="summary-mini">
        <div class="summary-mini-num" style="color:#10b981">${D(n)}</div>
      ...
      <div class="summary-mini">
        <div class="summary-mini-num" style="color:${a>r?`#ef4444`:`#18181b`}">${D(a)}</div>
    ...
    <div class="detail-panel ${Os?`expanded`:`collapsed`}" id="budget-detail" style="margin:0 -4px">`,window.isMonthCustomized(window.selectedBudgetMonth)||(d+=`<div style="display:flex;align-items:center;justify-content:space-between;background:var(--primary-light);border:1px solid #bfdbfe;border-radius:8px;padding:10px 16px;margin-bottom:16px;gap:12px;flex-wrap:wrap">
      <div>
...
//# sourceMappingURL=index-BNrYAkQk.js.map