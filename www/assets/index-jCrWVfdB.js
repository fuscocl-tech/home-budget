...
If nothing new found, return [].`;try{let t=await fetch(CLAUDE_API,{method:`POST`,headers:{"x-api-key":n,"anthropic-version":`2023-06-01`,"content-type":`application/json`},body:JSON.stringify({model:`claude-haiku-4-5-20251001`,max_tokens:1024,messages:[{role:`user`,content:o}]})});if(!t.ok)throw Error(`API error ${t.status}`);let i=(await t.json()).content[0].text.trim().match(/\[[\s\S]*\]/);if(!i)throw Error(`No JSON found in response`);tt=JSON.parse(i[0]).map((e,t)=>({...e,_key:`import_${Date.now()}_${t}`})),nt=new Set,r&&(r.style.display=`none`),e.target.value=``,renderSubscriptions()}catch(e){r&&(r.textContent=`⚠ ${e.message}`,r.style.display=``)}}function lt(e,t){let n=tt.find(t=>t._key===e);if(n){if(t===`subscription`)s.subscriptions||(s.subscriptions=[]),s.subscriptions.push({id:uid(),name:n.name,amount:n.amount,category:n.category,frequency:n.frequency}),window.saveData(s);else{let e=window.ensureMonthOverride(window.selectedBudgetMonth);e.expenses.push({id:k(e.expenses),name:n.name,amount:n.amount,frequency:`monthly`,category:`Subscriptions`,recurring:!0}),window.saveData(s)}nt.add(e),renderSubscriptions()}}function ut(e){nt.add(e),renderSubscriptions()}var dt=[{label:`Mortgage / Rent`,icon:`🏠`},{label:`Electricity`,icon:`⚡`},{label:`Gas`,icon:`🔥`},{label:`Water`,icon:`💧`},{label:`Internet`,icon:`📡`},{label:`Phone`,icon:`📱`},{label:`Insurance`,icon:`🛡️`},{label:`Car Registration`,icon:`🚗`},{label:`Rates & Taxes`,icon:`🏛️`},{label:`Loan Repayment`,icon:`💳`},{label:`Education`,icon:`📚`},{label:`Subscriptions`,icon:`📺`},{label:`Health`,icon:`🏥`},{label:`Other`,icon:`📦`}],ft=[`Monthly`,`Fortnightly`,`Weekly`,`Quarterly`,`Annually`];function pt(e){let t=dt.find(t=>t.label===e);return t?t.icon:`📦`}function mt(e){if(e<0)return`<span class="bill-due-badge overdue">Overdue ${Math.abs(e)}d</span>`;if(e===0)return`<span class="bill-due-badge today">Due today</span>`;if(e<=7)return`<span class="bill-due-badge soon">Due in ${e}d</span>`;let t=new Date;return t.setDate(t.getDate()+e),`<span class="bill-due-badge ok">${t.toLocaleDateString(`en-AU`,{day:`numeric`,month:`short`})}</span>`}function ht(e){return(parseFloat(e.amount)||0)*({Weekly:52/12,Fortnightly:26/12,Monthly:1,Quarterly:1/3,Annually:1/12}[e.frequency||`Monthly`]||1)}function gt(){let e=document.getElementById(`bills-content`);if(!e)return;let r=s.bills||[],i=s.subscriptions||[],a=window._billsSubsFilter,o=`<svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M11 4H4a2 2 0 0 0-2 2v14a2 2 0 0 0 2 2h14a2 2 0 0 0 2-2v-7"/><path d="M18.5 2.5a2.121 2.121 0 0 1 3 3L12 15l-4 1 1-4 9.5-9.5z"/></svg>`,c=`<svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><polyline points="3 6 5 6 21 6"/><path d="M19 6l-1 14a2 2 0 0 1-2 2H8a2 2 0 0 1-2-2L5 6"/><path d="M10 11v6"/><path d="M14 11v6"/><path d="M9 6V4h6v2"/></svg>`,l=r.reduce((e,t)=>e+ht(t),0)+i.reduce((e,t)=>e+subMonthlyAmount(t),0),u=r.filter(e=>n(e)<0).length,d=r.filter(e=>{let t=n(e);return t>=0&&t<=7}).length,f=new Date;f.setHours(0,0,0,0);let p=[`Sun`,`Mon`,`Tue`,`Wed`,`Thu`,`Fri`,`Sat`],m=Array.from({length:14},(e,n)=>{let i=new Date(f);i.setDate(i.getDate()+n);let a=r.filter(e=>t(e).toDateString()===i.toDateString()),o=[i.toDateString()===f.toDateString()&&`today`,a.length&&`has-bill`].filter(Boolean).join(` `);return`<div class="bills-day" title="${a.map(e=>e.name).join(`, `)}">
    ...
    </div>`}function bt(e){let t=e?(s.bills||[]).find(t=>t.id===e):null;gt();let n=document.getElementById(`bill-modal`);n&&(document.getElementById(`bill-modal-title`).textContent=t?`Edit Bill`:`Add Bill`,document.getElementById(`bill-edit-id`).value=e||``,t&&(document.getElementById(`bill-name`).value=t.name||``,document.getElementById(`bill-cat`).value=t.category||dt[0].label,document.getElementById(`bill-freq`).value=t.frequency||`Monthly`,document.getElementById(`bill-amount`).value=t.amount||``,document.getElementById(`bill-day`).value=t.dueDay||``,document.getElementById(`bill-start`).value=t.startDate||``,document.getElementById(`bill-autopay`).checked=!!t.autopay),St(),n.style.display=`flex`)}function xt(){let e=document.getElementById(`bill-modal`);e&&(e.style.display=`none`)}function St(){let e=document.getElementById(`bill-freq`)?.value,t=document.getElementById(`bill-day-wrap`),n=document.getElementById(`bill-start-wrap`);if(!t||!n)return;let r=e===`Monthly`;t.style.display=r?`block`:`none`,n.style.display=r?`none`:`block`}function Ct(){let e=document.getElementById(`bill-name`).value.trim(),t=parseFloat(document.getElementById(`bill-amount`).value),n=document.getElementById(`bill-cat`).value,r=document.getElementById(`bill-freq`).value,i=parseInt(document.getElementById(`bill-day`).value)||null,a=document.getElementById(`bill-start`).value||null,o=document.getElementById(`bill-autopay`).checked,c=document.getElementById(`bill-edit-id`).value;if(!e||isNaN(t))return;let l={name:e,amount:t,category:n,frequency:r,autopay:o};if(r===`Monthly`?l.dueDay=i:l.startDate=a,s.bills||(s.bills=[]),c){let e=s.bills.findIndex(e=>e.id===c);e!==-1&&(s.bills[e]={...s.bills[e],...l})}else s.bills.push({id:uid(),...l});window.saveData(s),xt(),gt()}function wt(e){s.bills=(s.bills||[]).filter(t=>t.id!==e),window.saveData(s),gt()}function Tt(e){let t=(s.bills||[]).find(t=>t.id===e);t&&(t.lastPaid=new Date().toISOString().slice(0,10),window.saveData(s),gt())}function Et(e,t){if(!e)return!0;let{type:n,days:r,intervalDays:i,startDate:a,endDate:o}=e;if(a&&t<a||o&&t>o)return!1;let s=new Date(t+`T12:00:00`),c=s.getDay();switch(n){case`daily`:return!0;case`weekdays`:return c>=1&&c<=5;case`weekends`:return c===0||c===6;case`specific_days`:return Array.isArray(r)&&r.includes(String(c));case`interval`:{if(!a||!i)return!1;let e=new Date(a+`T12:00:00`),t=Math.round((s-e)/864e5);return t>=0&&t%i===0}case`one_time`:return t===a;default:return!0}}var P={work:{label:`Work`,emoji:`💼`,color:`#dbeafe`,text:`#1e40af`,financial:!1},study:{label:`Study`,emoji:`📚`,color:`#fef3c7`,text:`#92400e`,financial:!1},social:{label:`Social`,emoji:`🎉`,color:`#ede9fe`,text:`#5b21b6`,financial:!0},family:{label:`Family`,emoji:`👨‍👩‍👧`,color:`#fce7f3`,text:`#9d174d`,financial:!1},travel:{label:`Travel`,emoji:`✈️`,color:`#e0f2fe`,text:`#075985`,financial:!0},health:{label:`Health`,emoji:`🏥`,color:`#fef2f2`,text:`#991b1b`,financial:!0},finance:{label:`Finance`,emoji:`💰`,color:`#ecfeff`,text:`#155e75`,financial:!0},home:{label:`Home`,emoji:`🏠`,color:`#ecfeff`,text:`#166534`,financial:!0},school:{label:`School`,emoji:`🏫`,color:`#fff7ed`,text:`#9a3412`,financial:!0},other:{label:`Other`,emoji:`📦`,color:`#f1f5f9`,text:`#475569`,financial:!1}},Dt=new Date().toISOString().slice(0,7),Ot=new Date().toISOString().slice(0,10),kt=new Set,At=`week`,jt=new Set,Mt={"life-areas":!1,nudge:!1},Nt=!1,Pt=null;Object.defineProperty(window,`_plannerMonth`,{get(){return Dt},set(e){Dt=e},configurable:!0}),Object.defineProperty(window,`_plannerSelectedDay`,{get(){return Ot},set(e){Ot=e},configurable:!0}),Object.defineProperty(window,`_plannerView`,{get(){return At},set(e){At=e},configurable:!0}),Object.defineProperty(window,`_plannerDetailEvId`,{get(){return Pt},set(e){Pt=e},configurable:!0}),Object.defineProperty(window,`_typeADimsExpanded`,{get(){return Nt},set(e){Nt=e},configurable:!0}),window._plannerExpanded=kt,window._plannerFilterMembers=jt,window._plannerCollapseState=Mt;var Ft=[{dot:`#2563eb`,bg:`#dbeafe`,text:`#1e40af`},{dot:`#db2777`,bg:`#fce7f3`,text:`#9d174d`},{dot:`#d97706`,bg:`#fef3c7`,text:`#92400e`},{dot:`#7c3aed`,bg:`#ede9fe`,text:`#5b21b6`},{dot:`#16a34a`,bg:`#dcfce7`,text:`#166534`},{dot:`#0891b2`,bg:`#ecfeff`,text:`#155e75`},{dot:`#ea580c`,bg:`#ffedd5`,text:`#9a3412`},{dot:`#be185d`,bg:`#fdf2f8`,text:`#831843`}];function It(){let e=s.householdProfile?.members||[],t=s.kids?.profiles||[],n=[],r=0;return e.forEach((e,t)=>{let i=Ft[r%Ft.length];e.role===`adult`&&e.name&&(n.push({id:`adult-`+t,name:e.name,emoji:e.emoji||`🧑`,...i}),r++)}),t.forEach((e,t)=>{let i=Ft[r%Ft.length];n.push({id:e.id||`kid-`+t,name:e.name,emoji:e.emoji||`🧒`,...i}),r++}),n.length===0&&n.push({id:`adult-0`,name:`Everyone`,emoji:`👨‍👩‍👧`,...Ft[0]}),n}function Lt(e){return!e||e===`everyone`?{id:`everyone`,name:`Everyone`,emoji:`👨‍👩‍👧`,dot:`#94a3b8`,bg:`#f1f5f9`,text:`#475569`}:It().find(t=>t.id===e)||{id:`everyone`,name:`Everyone`,emoji:`👨‍👩‍👧`,dot:`#94a3b8`,bg:`#f1f5f9`,text:`#475569`}}function Rt(e){return Array.isArray(e.memberIds)?e.memberIds:e.memberId?[e.memberId]:[`everyone`]}function zt(e){return Lt(Rt(e).find(e=>e!==`everyone`)||`everyone`)}function Bt(e){let t=Rt(e);return t.includes(`everyone`)||t.length===0?`Everyone`:t.map(e=>Lt(e).name).join(`, `)}function Vt(e){if(e.recurrence&&e.recurrence.type&&e.recurrence.type!==`one_time`){let t={daily:`Every day`,weekdays:`Mon–Fri`,weekends:`Sat & Sun`};if(t[e.recurrence.type])return t[e.recurrence.type];if(e.recurrence.type===`specific_days`)return(e.recurrence.days||[]).map(e=>[`Sun`,`Mon`,`Tue`,`Wed`,`Thu`,`Fri`,`Sat`][e]).join(`, `)||`Specific days`;if(e.recurrence.type===`interval`)return`Every ${e.recurrence.intervalDays} days`}return e.recurring&&e.recurring!==`none`?{weekly:`Every week`,fortnightly:`Every 2 weeks`,monthly:`Every month`,quarterly:`Every 3 months`,yearly:`Annually`}[e.recurring]||e.recurring:``}function Ht(){let e=s.planner?.events||[];return jt.size===0?e:e.filter(e=>{let t=Rt(e);return t.includes(`everyone`)?!0:[...jt].some(e=>t.includes(e))})}function Ut(e){return Ht().filter(t=>t._recurringSourceId?t.date===e:t.recurrence&&t.recurrence.type!==`one_time`?Et(t.recurrence,e):t.endDate&&t.endDate>t.date?e>=t.date&&e<=t.endDate:t.date===e)}function Wt(e){if(!e)return``;let[t,n]=e.split(`:`).map(Number);return`${t%12||12}:${String(n).padStart(2,`0`)}${t>=12?`pm`:`am`}`}var Gt=new Date().toISOString().slice(0,7);function Kt(){let[e,t]=Gt.split(`-`).map(Number),n=new Date(e,t-2,1);Gt=`${n.getFullYear()}-${String(n.getMonth()+1).padStart(2,`0`)}`,renderForecast()}function qt(){let[e,t]=Gt.split(`-`).map(Number),n=new Date(e,t,1);Gt=`${n.getFullYear()}-${String(n.getMonth()+1).padStart(2,`0`)}`,renderForecast()}function Jt(){let e=document.getElementById(`forecast-content`);if(!e)return;let t=(s.planner?.events||[]).filter(e=>e.date&&e.date.startsWith(Gt)),n=window.getMonthData(Gt),r=O(n.income)-O(n.expenses),[i,a]=Gt.split(`-`).map(Number),o=new Date(i,a,0).getDate(),c=[],l=1;for(;l<=o;){new Date(i,a-1,l);let e=l;for(;e<o&&new Date(i,a-1,e+1).getDay()!==1;)e++;c.push({start:l,end:e,events:[]}),l=e+1}t.sort((e,t)=>e.date.localeCompare(t.date)).forEach(e=>{let t=parseInt(e.date.split(`-`)[2]),n=c.find(e=>t>=e.start&&t<=e.end);n&&n.events.push(e)}),t.filter(e=>e.estimates&&e.estimates.length>0);let u=t.filter(e=>!e.estimates||e.estimates.length===0),d=t.reduce((e,t)=>e+(t.estimates||[]).filter(e=>e.accepted).reduce((e,t)=>e+(t.amount||0),0),0),f=r-d,p=new Date(i,a-1).toLocaleDateString(`en-AU`,{month:`long`,year:`numeric`}),m=!!N(`toto_ai_key`),h=`
    <div style="display:flex;align-items:center;justify-content:space-between;margin-bottom:20px;flex-wrap:wrap;gap:12px">
...
- No markdown, no code fences, just raw JSON`;try{let t=await fetch(CLAUDE_API,{method:`POST`,headers:{"x-api-key":e,"anthropic-version":`2023-06-01`,"content-type":`application/json`},body:JSON.stringify({model:`claude-haiku-4-5-20251001`,max_tokens:4096,messages:[{role:`user`,content:o}]})});if(!t.ok)throw Error(`API ${t.status}`);let n=(await t.json()).content[0].text.replace(/```[\w]*\n?/g,``).trim().match(/\[[\s\S]*\]/);if(!n)throw Error(`No JSON`);JSON.parse(n[0]).forEach(e=>{let t=(s.planner?.events||[]).find(t=>t.id===e.id);t&&e.items&&(t.estimates=e.items.map((e,t)=>({id:`est-${Date.now()}-${t}`,name:e.name,amount:Number(e.amount)||0,category:e.category||`Other`,accepted:!0})))}),window.saveData(s),Jt()}catch(e){n&&(n.textContent=`Estimate all (${t.length} events)`,n.disabled=!1),console.error(`Batch estimate error:`,e)}}function F(){let e=document.getElementById(`planner-content`);if(!e)return;s.planner||(s.planner={events:[]});let t=new Date().toISOString().slice(0,10),n=It(),r={id:`everyone`,name:`Everyone`,emoji:`👨‍👩‍👧`,dot:`#94a3b8`,bg:`#f1f5f9`,text:`#475569`},i=[...n,r],a=i.map(e=>e.name[0].toUpperCase()),o=i.map((e,t)=>{if(e.id===`everyone`)return`👨‍👩‍👧`;let n=e.name[0].toUpperCase();return a.filter(e=>e===n).length>1?(e.name[0]+(e.name[1]||``)).toUpperCase():n}),c=i.map((e,t)=>{let n=e.id===`everyone`,r=n?window._plannerFilterMembers.size===0:window._plannerFilterMembers.has(e.id),i=!r&&window._plannerFilterMembers.size>0&&!n,a=n?`<span style="font-size:14px">👨‍👩‍👧</span>`:`<span>${o[t]}</span>`;return`<div class="pl-legend-chip ${r?`active`:``} ${i?`dimmed`:``}"
    ...
    </div>`}).join(``),h=En(),_=h.map(e=>{let t=e.days<0?`Now!`:e.days===0?`Today!`:e.days===1?`Tomorrow`:`In ${e.days} days`,n=e.days<=0?`#ef4444`:e.days===1?`var(--good)`:e.days<=3?`#f59e0b`:`var(--iris-1)`;return`<div class="pl-nudge-tile">
    ...
    </div>`}function Xt(){let[e,t]=_plannerMonth.split(`-`).map(Number),n=new Date().toISOString().slice(0,10),r=new Date(e,t-1,1).getDay(),i=r===0?6:r-1,a=new Date(e,t,0).getDate(),o=new Date(e,t-1,0).getDate(),s=[];for(let n=i-1;n>=0;n--){let r=o-n,i=t-1||12,a=i===12?e-1:e;s.push({dateStr:`${a}-${String(i).padStart(2,`0`)}-${String(r).padStart(2,`0`)}`,day:r,muted:!0})}for(let n=1;n<=a;n++)s.push({dateStr:`${e}-${String(t).padStart(2,`0`)}-${String(n).padStart(2,`0`)}`,day:n,muted:!1});let c=s.length%7==0?0:7-s.length%7;for(let n=1;n<=c;n++){let r=t+1>12?1:t+1,i=r===1?e+1:e;s.push({dateStr:`${i}-${String(r).padStart(2,`0`)}-${String(n).padStart(2,`0`)}`,day:n,muted:!0})}return`<div class="pl-month-grid">${s.map(e=>{let t=e.dateStr===n,r=e.dateStr===_plannerSelectedDay,i=e.dateStr?Ut(e.dateStr):[],a=[...new Set(i.flatMap(e=>Rt(e)))].filter(e=>e!==`everyone`).slice(0,3).map(e=>`<div class="pl-cell-dot" style="background:${Lt(e).dot}"></div>`).join(``),o=i.length>3?`<div style="font-size:8px;color:var(--text-muted);font-weight:600">+</div>`:``;return`<div class="pl-cal-cell ${e.muted?`muted`:``} ${t?`today`:``} ${r?`selected`:``}"
                 onclick="_plannerSelectDay('${e.dateStr}')">
      <div class="pl-cell-num">${e.day}</div>
      <div class="pl-cell-dots">${a}${o}</div>
    </div>`}).join(``)}</div>`}function Zt(){let e=new Date(_plannerSelectedDay+`T12:00:00`),t=e.getDay(),n=new Date(e);n.setDate(e.getDate()-(t===0?6:t-1));let r=new Date().toISOString().slice(0,10),i=[`S`,`M`,`T`,`W`,`T`,`F`,`S`];return`<div class="week-strip">${Array.from({length:7},(e,t)=>{let r=new Date(n);return r.setDate(n.getDate()+t),{date:r,dateStr:r.toISOString().slice(0,10)}}).map(({date:e,dateStr:t})=>{let n=t===r,a=t===_plannerSelectedDay,o=Ut(t),s=o.length>0,c=[...new Set(o.flatMap(e=>Rt(e)))].filter(e=>e!==`everyone`).slice(0,3),l=a?`rgba(255,255,255,0.6)`:c.length?Lt(c[0]).dot:`#C4C2D4`;return`<div class="${a?`ws-day selected`+(n?` today-outline`:``):n?`ws-day today-outline`:`ws-day`}${s?` has`:``}" onclick="_plannerSelectDay('${t}')">
      <div class="ws-init">${i[e.getDay()]}</div>
      <div class="ws-num">${e.getDate()}</div>
      <div class="ws-dot" style="${s?`background:${l}`:``}"></div>
    </div>`}).join(``)}</div>`}function Qt(e){let t=P[e.category]||P.other,n=_plannerExpanded.has(e.id),r=e.estimates||[],i=r.filter(e=>e.accepted),a=i.reduce((e,t)=>e+(t.amount||0),0),o=r.reduce((e,t)=>e+(t.amount||0),0),s=e.pushed?`<span class="planner-pushed-badge">✓ In budget</span>`:r.length>0?`<span style="font-size:12px;color:var(--text-muted)">$${a.toLocaleString(`en-AU`)}</span>`:``,c=Vt(e);c&&(s=`<span class="recurring-badge">🔄 ${c}</span> `+s);let l=``;return n&&(l=`<div class="planner-event-body">`,e.notes&&(l+=`<p class="planner-notes">${g(e.notes)}</p>`),r.length>0?(l+=r.map(t=>`
    ...
    ${l}
  </div>`}function $t(){let[e,t]=_plannerMonth.split(`-`).map(Number),n=t===1?e-1:e,r=t===1?12:t-1;_plannerMonth=`${n}-${String(r).padStart(2,`0`)}`,_plannerSelectedDay=`${n}-${String(r).padStart(2,`0`)}-01`,F()}function en(){let[e,t]=_plannerMonth.split(`-`).map(Number),n=t===12?e+1:e,r=t===12?1:t+1;_plannerMonth=`${n}-${String(r).padStart(2,`0`)}`,_plannerSelectedDay=`${n}-${String(r).padStart(2,`0`)}-01`,F()}function tn(e){_plannerSelectedDay=e,_plannerMonth=e.slice(0,7),F()}function nn(e){let t=new Date(e+`T12:00:00`),n=e===new Date().toISOString().slice(0,10)?`Today`:t.toLocaleDateString(`en-AU`,{weekday:`long`}),r=t.toLocaleDateString(`en-AU`,{day:`numeric`,month:`long`,year:`numeric`}),i=Ut(e).sort((e,t)=>e.allDay&&!t.allDay?-1:!e.allDay&&t.allDay?1:(e.time||`99:99`).localeCompare(t.time||`99:99`)),a=``;return a=i.length===0?`<div class="pl-agenda-empty">Nothing planned — enjoy the quiet ☀️<br><span style="color:var(--iris-1);cursor:pointer;font-weight:600;font-size:13px" onclick="openPlannerModal(null,'${e}')">+ Add an event</span></div>`:`<div class="pl-agenda-list">${i.map(t=>{let n=zt(t),r=P[t.category]||P.other,i=t.allDay||!t.time?`All day`:Wt(t.time),a=Bt(t),o=new Date().getHours()*60+new Date().getMinutes(),s=t.time?parseInt(t.time.split(`:`)[0])*60+parseInt(t.time.split(`:`)[1]):-1,c=e===new Date().toISOString().slice(0,10)&&s>=0&&o>=s&&o<s+90,l=r.color||`#f1f5f9`,u=r.text||`#475569`;return`<div class="pl-agenda-ev">
    ...
    <div style="padding:${i.length?`12px 16px`:`0`}">${a}</div>
  </div>`}function rn(e){_plannerMonth=e.slice(0,7),_plannerSelectedDay=e,activateTab(`planner`)}function an(){let e=new Date().toISOString().slice(0,10);_plannerMonth=e.slice(0,7),_plannerSelectedDay=e,F()}function on(e){_plannerView=e,_plannerCollapseState[`life-areas`]=e===`week`,_plannerCollapseState.nudge=e===`week`,F()}function sn(e){_plannerCollapseState[e]=!_plannerCollapseState[e],F()}function cn(e){e===`everyone`?window._plannerFilterMembers.clear():window._plannerFilterMembers.has(e)?window._plannerFilterMembers.delete(e):window._plannerFilterMembers.add(e),F()}function ln(e){let t=document.getElementById(`pl-day-sheet-overlay`);if(!t)return;let n=new Date(e+`T12:00:00`),r=new Date().toISOString().slice(0,10);document.getElementById(`pl-sheet-title`).textContent=e===r?`Today`:n.toLocaleDateString(`en-AU`,{weekday:`long`}),document.getElementById(`pl-sheet-date`).textContent=n.toLocaleDateString(`en-AU`,{day:`numeric`,month:`long`,year:`numeric`}),document.getElementById(`pl-sheet-add-btn`).dataset.date=e,un(e),t.classList.add(`open`)}function un(e){let t=Ut(e).sort((e,t)=>(e.time||`99:99`).localeCompare(t.time||`99:99`)),n=document.getElementById(`pl-sheet-list`);if(n){if(t.length===0){n.innerHTML=`<div class="pl-sheet-empty">Nothing planned. <span style="color:var(--primary);cursor:pointer;font-weight:600" onclick="_plannerOpenModalFromSheet()">Add an event →</span></div>`;return}n.innerHTML=t.map((e,t)=>{let n=zt(e),r=P[e.category]||P.other,i=Bt(e),a=e.allDay||!e.time?`All day`:Wt(e.time);return`<div class="pl-sheet-ev" onclick="_plannerOpenDetail('${e.id}')">
      ...
      <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="#D4D4D8" stroke-width="2" stroke-linecap="round"><polyline points="9 18 15 12 9 6"/></svg>
    </div>`}).join(``)}}function dn(){let e=document.getElementById(`pl-day-sheet-overlay`);e&&e.classList.remove(`open`)}function fn(e){e.target===document.getElementById(`pl-day-sheet-overlay`)&&dn()}function pn(){let e=document.getElementById(`pl-sheet-add-btn`)?.dataset?.date||_plannerSelectedDay;dn(),Rn(null,e)}function mn(e){let t=P[e];if(!t)return;let n=new Date().toISOString().slice(0,10),r=(s.planner?.events||[]).filter(t=>t.category===e&&t.date>=n).sort((e,t)=>e.date.localeCompare(t.date));document.getElementById(`pl-life-sheet-icon`).textContent=t.emoji,document.getElementById(`pl-life-sheet-title`).textContent=t.label,document.getElementById(`pl-life-sheet-count`).textContent=r.length+` upcoming`;let i=``;if(r.length===0)i=`<div class="pl-sheet-empty">No upcoming ${t.label.toLowerCase()} events.</div>`;else{let e=``;r.forEach(t=>{let n=t.date.slice(0,7);if(n!==e){let r=new Date(t.date+`T12:00:00`);i+=`<div class="pl-life-day-hdr">${r.toLocaleDateString(`en-AU`,{month:`long`,year:`numeric`})}</div>`,e=n}let r=zt(t),a=Bt(t),o=new Date(t.date+`T12:00:00`);i+=`<div class="pl-life-ev-row" onclick="_plannerCloseLifeSheet();_plannerOpenDetail('${t.id}')">
        ...
        <div class="pl-life-ev-content">
          ...
          <div class="pl-life-ev-meta">${a}${t.time?` · `+Wt(t.time):``}</div>
        </div>
      </div>`})}document.getElementById(`pl-life-sheet-list`).innerHTML=i,document.getElementById(`pl-life-overlay`).classList.add(`open`)}function hn(){document.getElementById(`pl-life-overlay`)?.classList.remove(`open`)}function gn(e){e.target===document.getElementById(`pl-life-overlay`)&&hn()}function _n(e){let t=(s.planner?.events||[]).find(t=>t.id===e);if(!t)return;_plannerDetailEvId=e;let n=zt(t),r=P[t.category]||P.other,i=Rt(t);document.getElementById(`pl-detail-title`).textContent=t.title,document.getElementById(`pl-detail-color-bar`).style.background=n.dot;let a=new Date(t.date+`T12:00:00`).toLocaleDateString(`en-AU`,{weekday:`long`,day:`numeric`,month:`long`,year:`numeric`}),o=t.allDay?`All day`:t.time?Wt(t.time):``;!t.allDay&&t.endTime&&(o+=` – `+Wt(t.endTime));let c=(i.includes(`everyone`)?[{id:`everyone`,name:`Everyone`,emoji:`👨‍👩‍👧`,dot:`#94a3b8`,bg:`#f1f5f9`,text:`#475569`}]:i.map(e=>Lt(e))).map(e=>`<span style="display:inline-flex;align-items:center;gap:5px;background:${e.bg};color:${e.text};padding:4px 10px;border-radius:99px;font-size:12px;font-weight:600">${e.emoji} ${e.name}</span>`).join(` `),l=(t.estimates||[]).filter(e=>e.accepted).reduce((e,t)=>e+(t.amount||0),0),u=[{icon:`📅`,label:`Date`,value:a},o?{icon:`🕐`,label:`Time`,value:o}:null,t.location?{icon:`📍`,label:`Address`,value:g(t.location)}:null,{icon:`👥`,label:`Who`,value:`<div style="display:flex;flex-wrap:wrap;gap:6px;margin-top:4px">${c}</div>`},{icon:r.emoji,label:`Category`,value:r.label},Vt(t)?{icon:`🔄`,label:`Repeats`,value:Vt(t)}:null,l>0?{icon:`💰`,label:`Est. cost`,value:`$${l.toLocaleString(`en-AU`)}`}:null,t.notes?{icon:`📝`,label:`Notes`,value:g(t.notes)}:null].filter(Boolean);document.getElementById(`pl-detail-body`).innerHTML=u.map(e=>`<div class="pl-detail-row">
      <div class="pl-detail-icon">${e.icon}</div>
      <div style="flex:1;min-width:0">
        <div class="pl-detail-row-label">${e.label}</div>
        <div class="pl-detail-row-value">${e.value}</div>
      </div>
    </div>`).join(``)+(r.financial?`<button class="planner-ai-btn" style="width:100%;justify-content:center;margin-top:16px" onclick="_plannerCloseDetail();estimatePlannerEvent('${e}')">✦ Estimate costs with AI</button>`:``)+`<button class="pl-detail-share-btn" onclick="_plannerOpenShare('${e}')">🔗 Share this event</button>`,document.getElementById(`pl-detail-overlay`).classList.add(`open`)}function vn(){document.getElementById(`pl-detail-overlay`)?.classList.remove(`open`),_plannerDetailEvId=null}function yn(e){e.target===document.getElementById(`pl-detail-overlay`)&&vn()}function bn(){let e=_plannerDetailEvId;vn(),e&&Rn(e)}function xn(e){let t=(s.planner?.events||[]).find(t=>t.id===e);if(!t)return;let n=`https://toto.app/event/${t.title.toLowerCase().replace(/[^a-z0-9]+/g,`-`).replace(/(^-|-$)/g,``)}-${Math.random().toString(36).slice(2,10)}`,r=document.getElementById(`pl-share-sub`),i=document.getElementById(`pl-share-url`),a=document.getElementById(`pl-share-copy-btn`);r&&(r.textContent=t.title),i&&(i.textContent=n),a&&(a.textContent=`Copy`,a.classList.remove(`copied`)),document.getElementById(`pl-share-overlay`)?.classList.add(`open`)}function Sn(){document.getElementById(`pl-share-overlay`)?.classList.remove(`open`)}function Cn(e){e.target===document.getElementById(`pl-share-overlay`)&&Sn()}function wn(){let e=document.getElementById(`pl-share-url`)?.textContent;e&&navigator.clipboard?.writeText(e).catch(()=>{});let t=document.getElementById(`pl-share-copy-btn`);t&&(t.textContent=`Copied!`,t.classList.add(`copied`),setTimeout(()=>{t.textContent=`Copy`,t.classList.remove(`copied`)},2e3))}function Tn(e){let t=document.getElementById(`pl-share-url`)?.textContent||``,n=(s.planner?.events||[]).find(e=>e.id===_plannerDetailEvId),r=n?`${n.title} — ${t}`:t;e===`sms`&&window.open(`sms:?body=${encodeURIComponent(r)}`),e===`whatsapp`&&window.open(`https://wa.me/?text=${encodeURIComponent(r)}`),e===`email`&&window.open(`mailto:?subject=${encodeURIComponent(n?.title||`Event`)}&body=${encodeURIComponent(r)}`)}function En(){function e(e){return Math.ceil((e-new Date().setHours(0,0,0,0))/864e5)}function t(e,t,n,r){let i=new Date(e,t,1);for(;i.getDay()!==n;)i.setDate(i.getDate()+1);return i.setDate(i.getDate()+(r-1)*7),i}let n=new Date().getFullYear();return[{emoji:`🧾`,title:`EOFY`,days:e(new Date(n,5,30)),body:`Tax time — accountant fees, donations, prepayments`},{emoji:`🎄`,title:`Christmas`,days:e(new Date(n,11,25)),body:`Gifts, travel, food — start budgeting early`},{emoji:`💐`,title:`Mother's Day`,days:e(t(n,4,0,2)),body:`Gift, brunch or dinner for Mum`},{emoji:`👔`,title:`Father's Day`,days:e(t(n,8,0,1)),body:`Gift or outing for Dad`}].filter(e=>e.days>=-3&&e.days<=60).sort((e,t)=>e.days-t.days)}function Dn(e){_plannerExpanded.has(e)?_plannerExpanded.delete(e):_plannerExpanded.add(e),F()}function On(e,t){let n=(s.planner?.events||[]).find(t=>t.id===e);if(!n)return;let r=(n.estimates||[]).find(e=>e.id===t);r&&(r.accepted=!r.accepted,window.saveData(s),F())}function kn(e){let t=(s.planner?.events||[]).find(t=>t.id===e);if(!t)return;let n=(t.estimates||[]).filter(e=>e.accepted);if(!n.length)return;let r=t.date.slice(0,7);s.budget.suggestions||(s.budget.suggestions=[]),s.budget.suggestions=s.budget.suggestions.filter(e=>e.eventId!==t.id),n.forEach(e=>{s.budget.suggestions.push({id:`sug-`+Date.now()+`-`+Math.random().toString(36).slice(2,5),month:r,eventId:t.id,eventTitle:t.title,estId:e.id,name:e.name,amount:e.amount,category:e.category,status:`pending`})}),t.pushed=`suggested`,window.saveData(s),_plannerExpanded.add(e),F();let i=new Date(r+`-15`).toLocaleDateString(`en-AU`,{month:`long`,year:`numeric`});r===window.selectedBudgetMonth?window.safeRender(renderBudget):confirm(`${n.length} suggestion${n.length>1?`s`:``} sent to ${i} budget.\n\nGo to Monthly Budget to approve them?`)&&(window.selectedBudgetMonth=r,activateTab(`budget`))}function An(e){let t=(s.budget.suggestions||[]).find(t=>t.id===e);if(!t)return;let n=window.ensureMonthOverride(t.month);n.expenses.push({id:k(n.expenses),name:`${t.name} (${t.eventTitle})`,amount:t.amount,frequency:`monthly`,category:t.category,recurring:!1,_plannerEventId:t.eventId}),t.status=`approved`;let r=(s.planner?.events||[]).find(e=>e.id===t.eventId);r&&(s.budget.suggestions||[]).filter(e=>e.eventId===r.id&&e.status===`pending`).length===0&&(r.pushed=!0),window.saveData(s),window.safeRender(renderBudget),window.safeRender(F)}function jn(e){let t=(s.budget.suggestions||[]).find(t=>t.id===e);if(!t)return;t.status=`dismissed`;let n=(s.planner?.events||[]).find(e=>e.id===t.eventId);n&&(s.budget.suggestions||[]).filter(e=>e.eventId===n.id&&e.status===`pending`).length===0&&(n.pushed=n.pushed===`suggested`?!1:n.pushed),window.saveData(s),window.safeRender(renderBudget)}function Mn(e){let t=(s.budget?.suggestions||[]).filter(t=>t.month===e&&t.status===`pending`);if(!t.length)return``;let n={};t.forEach(e=>{n[e.eventTitle]||(n[e.eventTitle]=[]),n[e.eventTitle].push(e)});let r=t.map(e=>`
    ...
    ${r}
  </div>`}function Nn(e){let t=(s.planner?.events||[]).find(t=>t.id===e);t&&(Object.values(s.budget.months||{}).forEach(t=>{t.expenses=(t.expenses||[]).filter(t=>t._plannerEventId!==e)}),s.budget.expenses=(s.budget.expenses||[]).filter(t=>t._plannerEventId!==e),s.budget.suggestions=(s.budget.suggestions||[]).filter(t=>t.eventId!==e),t.pushed=!1,window.saveData(s),F(),window.safeRender(renderBudget))}function Pn(e){confirm(`Delete this event and remove it from the budget?`)&&(Nn(e),s.planner.events=s.planner.events.filter(t=>t.id!==e),_plannerExpanded.delete(e),window.saveData(s),window.closeModal(),F())}async function Fn(e){let t=N(`toto_ai_key`);if(!t){alert(`Add your AI API key in Settings to use cost estimation.`);return}let n=(s.planner?.events||[]).find(t=>t.id===e);if(!n)return;let r=document.getElementById(`ai-btn-${e}`);r&&(r.disabled=!0,r.textContent=`✦ Estimating…`);let i=(s.householdProfile?.members||[]).filter(e=>e.role===`adult`).length||2,a=(s.householdProfile?.members||[]).filter(e=>e.role===`child`).length||0,o=new Date(n.date+`T12:00:00`).toLocaleDateString(`en-AU`,{day:`numeric`,month:`long`,year:`numeric`}),c=`You are a family finance assistant for an Australian family. Suggest realistic cost estimates for the following life event.
...
- Use ONLY these categories: Transport, Accommodation, Food & Dining, Entertainment, Gifts, Clothing, Health, Education, Shopping, Other`;try{let r=((await(await fetch(CLAUDE_API,{method:`POST`,headers:{"x-api-key":t,"anthropic-version":`2023-06-01`,"content-type":`application/json`},body:JSON.stringify({model:`claude-haiku-4-5-20251001`,max_tokens:512,messages:[{role:`user`,content:c}]})})).json()).content?.[0]?.text||``).match(/\[[\s\S]*?\]/);if(!r)throw Error(`No JSON array in response`);n.estimates=JSON.parse(r[0]).map((e,t)=>({id:`est-${Date.now()}-${t}`,name:e.name,amount:Number(e.amount)||0,category:e.category||`Other`,accepted:!0})),_plannerExpanded.add(e),window.saveData(s),F()}catch(e){console.error(`Planner estimate error:`,e),r&&(r.disabled=!1,r.innerHTML=`✦ Try again`),alert(`Could not estimate costs. Check your AI API key in Settings.`)}}var I=new Set,In=`start`,Ln=new Date().toISOString().slice(0,7);function Rn(e,t){let n=e?(s.planner?.events||[]).find(t=>t.id===e):null,r=t||_plannerSelectedDay||new Date().toISOString().slice(0,10);I=new Set,In=`start`,Ln=(n?.date||r).slice(0,7),n?Rt(n).forEach(e=>I.add(e)):window._plannerFilterMembers.size>0&&window._plannerFilterMembers.forEach(e=>I.add(e)),document.getElementById(`modal-title`).textContent=n?`Edit Event`:`Add Event`,document.getElementById(`modal-body`).innerHTML=`
    ...
    <div style="display:grid;grid-template-columns:minmax(0,1.15fr) minmax(0,0.85fr);gap:8px" class="form-group">
      <div>
        ...
        <button type="button" class="form-input" id="pm-start-trigger" onclick="_pmDpOpen('start')" style="text-align:left;cursor:pointer;white-space:nowrap;overflow:hidden;text-overflow:ellipsis">
          <span id="pm-start-display">${Bn(n?n.date:r)}</span>
        </button>
      </div>
    ...
    <div style="display:grid;grid-template-columns:minmax(0,1.15fr) minmax(0,0.85fr);gap:8px" class="form-group" id="pm-end-group">
      <div>
        ...
        <button type="button" class="form-input" id="pm-end-trigger" onclick="_pmDpOpen('end')" style="text-align:left;cursor:pointer;white-space:nowrap;overflow:hidden;text-overflow:ellipsis;color:${n?.endDate?`var(--text)`:`var(--text-muted)`}">
          <span id="pm-end-display">${n?.endDate?Bn(n.endDate):`Same day`}</span>
        </button>
      </div>
  ...
  </div>`}function tr(e){(s.planner?.events||[]).filter(t=>t.date?.slice(0,7)===e&&!t.pushed&&(t.estimates||[]).some(e=>e.accepted)).forEach(e=>kn(e.id)),renderBudget()}function nr(e,t,n,r){let i=0;for(let a=1;a<=31;a++){let o=new Date(e,t,a);if(o.getMonth()!==t)break;if(o.getDay()===n&&(i++,i===r))return o}}function rr(){let e=new Date;e.setHours(0,0,0,0);let t=e.getFullYear(),n=t=>Math.ceil((t-e)/864e5);return[{d:new Date(t,3,25),emoji:`🌿`,title:`Anzac Day`,body:`Public holiday — any plans or travel?`},{d:new Date(t,5,30),emoji:`🧾`,title:`EOFY`,body:`Tax time — accountant fees, donations, prepayments`},{d:new Date(t,11,25),emoji:`🎄`,title:`Christmas`,body:`Gifts, travel, food — start budgeting early`},{d:new Date(t,11,26),emoji:`🛍️`,title:`Boxing Day`,body:`Sales, travel, family catch-ups`},{d:new Date(t+1,0,1),emoji:`🎆`,title:`New Year's`,body:`Celebrations, travel plans`},{d:nr(t,4,0,2),emoji:`💐`,title:`Mother's Day`,body:`Gift, brunch or dinner for Mum`},{d:nr(t,8,0,1),emoji:`👔`,title:`Father's Day`,body:`Gift or outing for Dad`},{d:nr(t,10,2,1),emoji:`🏆`,title:`Melbourne Cup`,body:`Event day — sweepstakes, lunch, outfits`},{d:new Date(t,3,6),emoji:`🎒`,title:`Term 1 Holidays`,body:`2 weeks — activities, childcare, day trips`},{d:new Date(t,6,5),emoji:`🎒`,title:`Term 2 Holidays`,body:`2 weeks — winter school holidays`},{d:new Date(t,8,19),emoji:`🎒`,title:`Term 3 Holidays`,body:`2 weeks — spring school holidays`},{d:new Date(t,11,18),emoji:`🎒`,title:`Summer Holidays`,body:`6 weeks — the big one, plan early`}].filter(e=>e.d).map(e=>({...e,days:n(e.d)})).filter(e=>e.days>=-3&&e.days<=45).sort((e,t)=>e.days-t.days).slice(0,4)}function ir(){let e=rr();return e.length===0?``:`<div class="nudge-section">
  ...
  </div>`}function ar(e,t){let n=new Date(e);switch(t){case`weekly`:n.setDate(n.getDate()+7);break;case`fortnightly`:n.setDate(n.getDate()+14);break;case`monthly`:n.setMonth(n.getMonth()+1);break;case`quarterly`:n.setMonth(n.getMonth()+3);break;case`yearly`:n.setFullYear(n.getFullYear()+1);break}return n}function or(){if(!s.planner?.events)return;let e=s.planner.events,t=new Date;t.setHours(0,0,0,0);let n=!1,r={weekly:3,fortnightly:3,monthly:6,quarterly:12,yearly:24};e.filter(e=>e.recurring&&e.recurring!==`none`&&!e._recurringSourceId).forEach(i=>{let a=i.recurring,o=new Date(t);o.setMonth(o.getMonth()+(r[a]||12));let s=new Date(i.date+`T12:00:00`);for(;s<t;)s=ar(s,a);let c=0;for(;s<=o&&c++<200;){let t=s.toISOString().slice(0,10);e.some(e=>e.date===t&&(e.id===i.id||e._recurringSourceId===i.id))||(e.push({id:`ev-`+Date.now()+`-r`+Math.random().toString(36).slice(2,6),title:i.title,category:i.category,date:t,notes:i.notes||``,recurring:a,_recurringSourceId:i.id,estimates:(i.estimates||[]).map(e=>({...e,id:`est-`+Date.now()+Math.random(),accepted:!0})),pushed:!1}),n=!0),s=ar(s,a)}}),n&&window.saveData(s)}var sr=!1,cr=[],lr=!1,ur=[{icon:`📅`,text:`What should I plan for this month?`},{icon:`💸`,text:`Any upcoming events I should budget for?`},{icon:`🎒`,text:`Help me plan for school holidays`},{icon:`✈️`,text:`I have a trip coming up — what do I need to organise?`}];function dr(){sr?pr():fr()}function fr(){sr=!0,document.getElementById(`toto-panel`).classList.add(`open`),cr.length===0&&mr()}function pr(){sr=!1,document.getElementById(`toto-panel`).classList.remove(`open`)}function mr(){let e=new Date().toISOString().slice(0,10),t=new Date;t.setDate(t.getDate()+7);let n=t.toISOString().slice(0,10),r=(s.planner?.events||[]).filter(t=>t.date>=e&&t.date<=n),i=[];r.slice(0,2).forEach(t=>{let n=P[t.category]||P.other,r=new Date(t.date+`T12:00:00`).toLocaleDateString(`en-AU`,{weekday:`short`,day:`numeric`,month:`short`});n.financial&&(!t.estimates||t.estimates.length===0)?i.push({icon:n.emoji,text:`${g(t.title)} is ${r} — want me to estimate costs?`}):t.date===e&&i.push({icon:n.emoji,text:`You have "${g(t.title)}" today — anything to prepare?`})});let a=getSeasonalNudges().slice(0,2).map(e=>({icon:e.emoji,text:`Help me plan for ${g(e.title)} (${e.days<=0?`now!`:`in ${e.days} days`})`})),o=[...i,...a,...ur].slice(0,4),c=document.getElementById(`toto-suggestions`);c.innerHTML=o.map(e=>`<button class="toto-suggestion" onclick="_totoSendSuggestion(this)">${e.icon} ${e.text}</button>`).join(``);let l=new Date().getHours(),u=l<12?`Good morning`:l<17?`Good afternoon`:`Good evening`,d=(s.householdProfile?.members||[]).filter(e=>e.role===`adult`).length||2,f=(s.householdProfile?.members||[]).filter(e=>e.role===`child`).length||0,p=f>0?` and ${f} kid${f>1?`s`:``}`:``;yr(`toto`,`${u}! 👋 I'm Toto, your family planning assistant. You have ${d} adult${d>1?`s`:``}${p} in your household.\n\nI can help you plan events, estimate costs, and make sure nothing slips through the cracks. What would you like to work on?`)}function hr(e){let t=e.textContent.trim();e.closest(`.toto-suggestions`).style.display=`none`,_r(t)}async function gr(){let e=document.getElementById(`toto-input`),t=e.value.trim();!t||lr||(e.value=``,document.getElementById(`toto-suggestions`).style.display=`none`,_r(t))}async function _r(e){let t=N(`toto_ai_key`);if(!t){yr(`toto`,`To chat with me, you'll need to add your AI API key in Settings. It only takes a second! ⚙️`);return}yr(`user`,e),cr.push({role:`user`,content:e}),lr=!0,document.getElementById(`toto-send`).disabled=!0;let n=br();try{let e=vr(),r=(await(await fetch(CLAUDE_API,{method:`POST`,headers:{"x-api-key":t,"anthropic-version":`2023-06-01`,"content-type":`application/json`},body:JSON.stringify({model:`claude-haiku-4-5-20251001`,max_tokens:600,system:e,messages:cr})})).json()).content?.[0]?.text||`Sorry, I couldn't get a response. Try again?`;cr.push({role:`assistant`,content:r}),xr(n),yr(`toto`,r)}catch{xr(n),yr(`toto`,`Oops, something went wrong. Check your internet connection and try again.`)}finally{lr=!1,document.getElementById(`toto-send`).disabled=!1,document.getElementById(`toto-input`).focus()}}function vr(){let e=new Date().toISOString().slice(0,10),t=new Date().toLocaleDateString(`en-AU`,{weekday:`long`,day:`numeric`,month:`long`,year:`numeric`}),n=(s.householdProfile?.members||[]).filter(e=>e.role===`adult`).length||2,r=(s.householdProfile?.members||[]).filter(e=>e.role===`child`).length||0,i=new Date;i.setDate(i.getDate()+60);let a=(s.planner?.events||[]).filter(t=>t.date>=e&&t.date<=i.toISOString().slice(0,10)).sort((e,t)=>e.date.localeCompare(t.date)).slice(0,15).map(e=>{let t=P[e.category]||P.other,n=new Date(e.date+`T12:00:00`).toLocaleDateString(`en-AU`,{weekday:`short`,day:`numeric`,month:`short`}),r=(e.estimates||[]).filter(e=>e.accepted).reduce((e,t)=>e+(t.amount||0),0);return`- ${n}: ${e.title} (${t.label})${r>0?` — $`+r.toLocaleString(`en-AU`)+` budgeted`:``}${e.notes?` — Notes: `+e.notes:``}`}).join(`
...
- Keep responses to 3-5 sentences unless a list is genuinely helpful`}function yr(e,t){let n=document.getElementById(`toto-messages`),r=document.createElement(`div`);r.className=`toto-msg ${e}`;let i=t.replace(/\n/g,`<br>`);e===`toto`?r.innerHTML=`<div class="toto-msg-avatar">🐕</div><div class="toto-msg-bubble">${i}</div>`:r.innerHTML=`<div class="toto-msg-bubble">${i}</div>`,n.appendChild(r),n.scrollTop=n.scrollHeight}function br(){let e=document.getElementById(`toto-messages`),t=`toto-typing-`+Date.now(),n=document.createElement(`div`);return n.className=`toto-msg toto`,n.id=t,n.innerHTML=`<div class="toto-msg-avatar">🐕</div><div class="toto-msg-bubble"><div class="toto-typing"><span></span><span></span><span></span></div></div>`,e.appendChild(n),e.scrollTop=e.scrollHeight,t}function xr(e){document.getElementById(e)?.remove()}`serviceWorker`in navigator&&navigator.serviceWorker.register(`/home-budget/sw.js`,{scope:`/home-budget/`}).catch(e=>console.warn(`SW registration failed:`,e));var Sr=[`Produce`,`Meat & Seafood`,`Dairy & Eggs`,`Pantry`,`Bakery`,`Frozen`,`Household`,`Other`],Cr={Produce:`🥦`,"Meat & Seafood":`🥩`,"Dairy & Eggs":`🥛`,Pantry:`🥫`,Bakery:`🍞`,Frozen:`🧊`,Household:`🏠`,Other:`🛒`},wr={food:{label:`Food`,emoji:`🛒`,color:`#dcfce7`,text:`#166534`,aisles:!0,priceEst:!0},clothes:{label:`Clothes`,emoji:`👕`,color:`#dbeafe`,text:`#1e40af`,aisles:!1,priceEst:!1},wishlist:{label:`Wishlist`,emoji:`🎁`,color:`#fce7f3`,text:`#9d174d`,aisles:!1,priceEst:!1},home:{label:`Home & Garden`,emoji:`🛠`,color:`#fef3c7`,text:`#92400e`,aisles:!0,priceEst:!1},pharmacy:{label:`Pharmacy`,emoji:`💊`,color:`#ede9fe`,text:`#5b21b6`,aisles:!0,priceEst:!1}},Tr={food:[{key:`produce`,emoji:`🥦`,label:`Produce`},{key:`dairy`,emoji:`🥛`,label:`Dairy & Eggs`},{key:`bakery`,emoji:`🍞`,label:`Bakery`},{key:`meat`,emoji:`🥩`,label:`Meat & Seafood`},{key:`pantry`,emoji:`🥫`,label:`Pantry`},{key:`frozen`,emoji:`🧊`,label:`Frozen`},{key:`health`,emoji:`🧴`,label:`Health & Beauty`},{key:`bathroom`,emoji:`🚿`,label:`Bathroom`},{key:`cleaning`,emoji:`🧹`,label:`Cleaning`},{key:`drinks`,emoji:`🍷`,label:`Drinks & Alcohol`},{key:`other`,emoji:`🛒`,label:`Uncategorised`}],home:[{key:`tools`,emoji:`🔨`,label:`Tools & Hardware`},{key:`garden`,emoji:`🌱`,label:`Garden`},{key:`cleaning`,emoji:`🧹`,label:`Cleaning`},{key:`other`,emoji:`🛒`,label:`Other`}],pharmacy:[{key:`medicine`,emoji:`💊`,label:`Medicine`},{key:`skincare`,emoji:`🧴`,label:`Skincare`},{key:`vitamins`,emoji:`💪`,label:`Vitamins`},{key:`other`,emoji:`🛒`,label:`Other`}]},Er={milk:`dairy`,cheese:`dairy`,butter:`dairy`,eggs:`dairy`,yoghurt:`dairy`,cream:`dairy`,bread:`bakery`,rolls:`bakery`,muffin:`bakery`,croissant:`bakery`,baguette:`bakery`,apple:`produce`,banana:`produce`,orange:`produce`,strawberry:`produce`,tomato:`produce`,lettuce:`produce`,spinach:`produce`,carrot:`produce`,broccoli:`produce`,potato:`produce`,onion:`produce`,garlic:`produce`,cucumber:`produce`,capsicum:`produce`,avocado:`produce`,lemon:`produce`,lime:`produce`,grapes:`produce`,mango:`produce`,pineapple:`produce`,watermelon:`produce`,chicken:`meat`,beef:`meat`,mince:`meat`,steak:`meat`,pork:`meat`,lamb:`meat`,salmon:`meat`,fish:`meat`,tuna:`meat`,prawn:`meat`,sausage:`meat`,bacon:`meat`,rice:`pantry`,pasta:`pantry`,flour:`pantry`,sugar:`pantry`,oil:`pantry`,vinegar:`pantry`,salt:`pantry`,pepper:`pantry`,sauce:`pantry`,stock:`pantry`,beans:`pantry`,lentils:`pantry`,chickpeas:`pantry`,cereal:`pantry`,oats:`pantry`,honey:`pantry`,jam:`pantry`,peanut:`pantry`,coffee:`pantry`,tea:`pantry`,biscuit:`pantry`,cracker:`pantry`,chocolate:`pantry`,chips:`pantry`,nuts:`pantry`,icecream:`frozen`,peas:`frozen`,corn:`frozen`,pizza:`frozen`,shampoo:`health`,conditioner:`health`,deodorant:`health`,sunscreen:`health`,moisturiser:`health`,makeup:`health`,lipstick:`health`,mascara:`health`,toothpaste:`bathroom`,toothbrush:`bathroom`,soap:`bathroom`,toilet:`bathroom`,razors:`bathroom`,tampons:`bathroom`,pads:`bathroom`,detergent:`cleaning`,bleach:`cleaning`,sponge:`cleaning`,dishwashing:`cleaning`,bins:`cleaning`,mop:`cleaning`,water:`drinks`,juice:`drinks`,beer:`drinks`,wine:`drinks`,spirits:`drinks`,softdrink:`drinks`,soda:`drinks`,kombucha:`drinks`};function Dr(e){for(var t=e.toLowerCase(),n=Object.keys(Er),r=0;r<n.length;r++)if(t.indexOf(n[r])!==-1)return Er[n[r]];return`other`}function Or(e){var t=e.trim(),n=1,r=`units`,i=t,a=t.match(/^(\d+(?:\.\d+)?)\s*(kg|g|L|l|ml|dozen|doz)\s+(.+)$/i);if(a)n=parseFloat(a[1]),r=a[2].toLowerCase()===`l`?`L`:a[2].toLowerCase()===`doz`?`dozen`:a[2],i=a[3];else{var o=t.match(/^(\d+(?:\.\d+)?)\s+(.+)$/);o?(n=parseFloat(o[1]),i=o[2]):/^dozen\s+/i.test(t)&&(n=1,r=`dozen`,i=t.replace(/^dozen\s+/i,``))}return i=i.charAt(0).toUpperCase()+i.slice(1),{qty:n,unit:r,name:i}}var kr=`food`,Ar=`selector`;Object.defineProperty(window,`_listsActiveType`,{get(){return kr},set(e){kr=e},configurable:!0}),Object.defineProperty(window,`_listsView`,{get(){return Ar},set(e){Ar=e},configurable:!0});function jr(e){let t=new Date,n=t.getDay()===0?-6:1-t.getDay(),r=new Date(t);return r.setDate(t.getDate()+n+(e||0)*7),r.toISOString().slice(0,10)}function Mr(e){let t=new Date(e+`T00:00:00`);return Array.from({length:7},(e,n)=>{let r=new Date(t);return r.setDate(t.getDate()+n),r})}function Nr(){window._mealView===`shopping`?Wr():Pr()}function Pr(){let e=jr(0),t=Mr(e),n=s.meals.plan[e]||{},r=new Date().toISOString().slice(0,10),i=[`Mon`,`Tue`,`Wed`,`Thu`,`Fri`,`Sat`,`Sun`],a=[{key:`b`,label:`Breakfast`},{key:`l`,label:`Lunch`},{key:`d`,label:`Dinner`}];t[0].toLocaleDateString(`en-AU`,{day:`numeric`,month:`short`}),t[6].toLocaleDateString(`en-AU`,{day:`numeric`,month:`short`});let o=[];for(let e=0;e<7;e++){let t=n[e]||{};[`b`,`l`,`d`].forEach(e=>{t[e]&&o.push(t[e])})}let c=``;a.forEach(i=>{c+=`<div class="meal-grid-label">${i.label}</div>`,t.forEach((t,a)=>{let o=(n[a]||{})[i.key]||``,l=t.toISOString().slice(0,10)===r;c+=`<div class="meal-cell${l?` today`:``}" onclick="openMealEdit('${e}',${a},'${i.key}')">
    ...
    style="padding:4px 10px;border-radius:99px;font-size:12px;cursor:pointer;white-space:nowrap;
      ...
      color:${n?`#0891b2`:`var(--text)`}">${t}</button>`}function zr(e,t){L[e]=t,document.querySelectorAll(`[data-filter="${e}"]`).forEach(e=>{let n=e.dataset.val===t;e.style.borderColor=n?`#0891b2`:`var(--border)`,e.style.background=n?`#ecfeff`:`var(--surface)`,e.style.color=n?`#0891b2`:`var(--text)`})}function Br(e){L.price=e;let t=document.getElementById(`meal-price-lbl`);t&&(t.textContent=e>0?`$${e}`:`Any`)}async function Vr(e){let t=N(`toto_ai_key`);if(!t)return;let n=document.getElementById(`meal-suggest-btn`),r=document.getElementById(`meal-suggest-out`);n&&(n.textContent=`⏳ Thinking…`,n.disabled=!0),r&&(r.innerHTML=``);let i={b:`Breakfast`,l:`Lunch`,d:`Dinner`},a=[L.cuisine===`Any`?``:`Cuisine: ${L.cuisine}`,L.price>0?`Budget: up to $${L.price} per serve`:``,L.dietary===`Any`?``:`Style: ${L.dietary}`].filter(Boolean).join(`, `)||`No specific filters`,o=`Suggest 8 ${i[e]} meal ideas. Filters: ${a}.
Return ONLY a JSON array of meal names, no other text: ["Meal 1","Meal 2",...]`;try{let e=(await(await fetch(CLAUDE_API,{method:`POST`,headers:{"x-api-key":t,"anthropic-version":`2023-06-01`,"content-type":`application/json`},body:JSON.stringify({model:`claude-haiku-4-5-20251001`,max_tokens:256,messages:[{role:`user`,content:o}]})})).json()).content[0].text.match(/\[[\s\S]*\]/);if(!e)throw Error(`No JSON`);let n=JSON.parse(e[0]);r&&(r.innerHTML=`<div style="display:flex;flex-wrap:wrap;gap:6px">
      ${n.map(e=>`<button style="padding:6px 12px;border-radius:99px;border:1px solid #0891b2;background:#ecfeff;color:#0891b2;font-size:12px;font-weight:500;cursor:pointer"
        onclick="document.getElementById('meal-input').value='${e.replace(/'/g,`\\'`)}'">${e}</button>`).join(``)}
    </div>`)}catch(e){r&&(r.innerHTML=`<span style="font-size:12px;color:var(--danger)">⚠ ${e.message}</span>`)}finally{n&&(n.textContent=`Get suggestions`,n.disabled=!1)}}function Hr(e,t,n,r){s.meals.plan[e]||(s.meals.plan[e]={}),s.meals.plan[e][t]||(s.meals.plan[e][t]={b:``,l:``,d:``}),s.meals.plan[e][t][n]=r,delete s.meals.plan[e][t][`cal_`+n],window.saveData(s),window.closeModal(),Pr(),r&&s.settings?.showCalories&&Ur(e,t,n,r)}async function Ur(e,t,n,r){let i=N(`toto_ai_key`);if(!(!i||!r))try{let a=await fetch(CLAUDE_API,{method:`POST`,headers:{"x-api-key":i,"anthropic-version":`2023-06-01`,"content-type":`application/json`},body:JSON.stringify({model:`claude-haiku-4-5-20251001`,max_tokens:50,messages:[{role:`user`,content:`Estimate the calories in this meal: "${r}". Return ONLY a number, nothing else. For example: 450`}]})});if(!a.ok)return;let o=await a.json(),c=parseInt(o.content[0].text.trim().replace(/[^0-9]/g,``));c>0&&c<5e3&&s.meals.plan[e]?.[t]&&(s.meals.plan[e][t][`cal_`+n]=c,window.saveData(s),Pr())}catch{}}function Wr(){let e=s.meals.shopping||[],t=e.filter(e=>e.checked).length,n=e.length-t,r={};Sr.forEach(e=>r[e]=[]),e.forEach(e=>{r[Sr.includes(e.cat)?e.cat:`Other`].push(e)});let i=``;Sr.forEach(e=>{let t=r[e];t.length&&(i+=`
      ...
      <div style="padding:14px 20px;display:flex;gap:10px;flex-wrap:wrap;align-items:flex-end">
        ...
        <div style="flex:1;min-width:120px">
          ...
          <select class="form-select" id="shop-cat">
            ${Sr.map(e=>`<option value="${e}">${Cr[e]} ${e}</option>`).join(``)}
          </select>
        </div>
    ...
    </div>`}function Gr(){let e=document.getElementById(`shop-name`)?.value.trim();if(!e)return;let t=document.getElementById(`shop-qty`)?.value.trim()||``,n=document.getElementById(`shop-cat`)?.value||`Other`,r=s.meals.shopping;r.push({id:r.length?Math.max(...r.map(e=>e.id))+1:1,name:e,qty:t,cat:n,checked:!1}),window.saveData(s),Wr()}function Kr(e,t){let n=s.meals.shopping.find(t=>t.id===e);n&&(n.checked=t,window.saveData(s))}function qr(e){s.meals.shopping=s.meals.shopping.filter(t=>t.id!==e),window.saveData(s),Wr()}function Jr(){s.meals.shopping=s.meals.shopping.filter(e=>!e.checked),window.saveData(s),Wr()}function Yr(e){var t=document.getElementById(`ls-toast`);t&&t.remove();var n=document.createElement(`div`);n.id=`ls-toast`,n.textContent=e,n.style.cssText=`position:fixed;bottom:96px;left:50%;transform:translateX(-50%) translateY(20px);background:#1a1814;color:#fff;padding:10px 18px;border-radius:99px;font-size:13px;font-weight:600;z-index:9999;opacity:0;transition:opacity .2s,transform .2s;pointer-events:none;white-space:nowrap;max-width:80vw;text-align:center;font-family:var(--sans,system-ui,sans-serif)`,document.body.appendChild(n),requestAnimationFrame(function(){n.style.opacity=`1`,n.style.transform=`translateX(-50%) translateY(0)`}),setTimeout(function(){n.style.opacity=`0`,n.style.transform=`translateX(-50%) translateY(10px)`,setTimeout(function(){n.parentNode&&n.remove()},300)},2400)}function Xr(e,t,n,r,i,a,o){s.lists||window._applyMigrations(s);var c=s.lists[e];if(c){if(c.items.find(function(e){return e.state===`active`&&e.name.toLowerCase()===t.toLowerCase()}))return Yr(g(t)+` is already on your list`),!1;var l={id:`li-`+Date.now()+`-`+Math.random().toString(36).slice(2,6),name:t,quantity:n||1,unit:r||`units`,notes:a||``,aisle:i||(e===`food`?Dr(t):`other`),state:`active`,addedBy:`user`,addedAt:new Date().toISOString(),stateChangedAt:null,mealTag:o||null,manualPrice:null,barcodeId:null};return c.items.push(l),ai(e,t),window.saveData(s),!0}}function Zr(e,t,n){if(!(!s.lists||!s.lists[e])){var r=s.lists[e].items.find(function(e){return e.id===t});r&&(r.state=n,r.stateChangedAt=new Date().toISOString(),window.saveData(s),ci())}}function Qr(e,t){!s.lists||!s.lists[e]||(s.lists[e].items=s.lists[e].items.filter(function(e){return e.id!==t}),window.saveData(s),ci())}function $r(e){var t=document.getElementById(`ls-quick-input`);if(t){var n=t.value.trim();if(n){var r=Or(n),i=e===`food`?Dr(r.name):`other`;if(Xr(e,r.name,r.qty,r.unit,i,``,null)!==!1){t.value=``;var a=document.getElementById(`ls-parse-preview`);a&&(a.innerHTML=``),ci()}}}}function ei(e,t){let n=e===`food`,r=s.lists?.[e]?.items||[],i=t?r.find(e=>e.id===t):null,a=n?PANTRY_CATS:[`Other`],o=[`units`,`kg`,`g`,`L`,`ml`,`dozen`];document.getElementById(`modal-title`).textContent=i?`Edit Item`:`Add Item`,document.getElementById(`modal-body`).innerHTML=`
    <div class="form-group">
      <label class="form-label">Item Name</label>
    ...
    <div style="display:grid;grid-template-columns:1fr 1fr;gap:12px">
      <div class="form-group">
        ...
        <select class="form-select" id="lf-cat">
          ${a.map(e=>`<option value="${e}"${i&&i.aisle===ti(e)?` selected`:``}>${e}</option>`).join(``)}
        </select>
      </div>
      <div class="form-group">
        <label class="form-label">Quantity <span style="font-weight:400;color:var(--text-muted)">(optional)</span></label>
    ...
    <button class="btn" onclick="window.closeModal()">Cancel</button>
    <button class="btn btn-primary" onclick="_listsSaveForm('${e}','${t||``}')">Save</button>`,document.getElementById(`modal-overlay`).classList.remove(`hidden`)}function ti(e){return{Fridge:`dairy`,Freezer:`frozen`,Pantry:`pantry`,"Fruit & Veg":`produce`,Spices:`pantry`,Drinks:`drinks`,Cleaning:`cleaning`,Other:`other`}[e]||`other`}function ni(e,t){let n=document.getElementById(`lf-name`)?.value.trim();if(!n)return;let r=document.getElementById(`lf-cat`)?.value||`Other`,i=document.getElementById(`lf-qty`)?.value.trim()||`1`,a=document.getElementById(`lf-unit`)?.value||`units`,o=document.getElementById(`lf-notes`)?.value.trim()||``,c=document.querySelector(`input[name="lf-state"]:checked`)?.value||`active`,l=parseFloat(i)||1,u=e===`food`?ti(r):`other`;s.lists||(s.lists={}),s.lists[e]||(s.lists[e]={items:[],weeklyBudget:0,budget:0,stores:[],favourites:[],history:[]});let d=s.lists[e].items;if(t){let e=d.find(e=>e.id===t);e&&(e.name=n,e.quantity=l,e.unit=a,e.notes=o,e.aisle=u,e.state=c)}else{if(d.find(e=>e.name.toLowerCase()===n.toLowerCase()&&e.state===`active`)&&!confirm(`"${n}" is already on your list. Add another?`))return;d.push({id:`si-`+Date.now(),name:n,quantity:l,unit:a,notes:o,aisle:u,state:c,addedBy:window._currentUser?.uid||`guest`,addedAt:new Date().toISOString(),mealTag:null,manualPrice:null,barcodeId:null}),ai(e,n)}window.saveData(s),window.closeModal(),ci()}function ri(e){if(!(!s.lists||!s.lists[e])){var t=s.lists[e].items.filter(function(e){return e.state===`got_it`});t.length&&confirm(`Remove `+t.length+` trolley item`+(t.length===1?``:`s`)+`?`)&&(s.lists[e].items=s.lists[e].items.filter(function(e){return e.state!==`got_it`}),window.saveData(s),ci())}}function ii(e){if(!(!s.lists||!s.lists[e])){var t=s.lists[e];t.history||(t.history=[]),t.history.push({archivedAt:new Date().toISOString(),items:JSON.parse(JSON.stringify(t.items))}),t.items=[],window.saveData(s),Yr(`Shop archived!`),ci()}}function ai(e,t){if(!(!s.lists||!s.lists[e])){var n=s.lists[e].favourites,r=n.find(function(e){return e.name.toLowerCase()===t.toLowerCase()});r?r.addedCount=(r.addedCount||0)+1:n.push({name:t,addedCount:1,pinned:!1})}}function oi(e){if(!(!s.lists||!s.lists[e])){var t=s.lists[e].favourites,n=t.filter(function(e){return e.pinned}),r=n.length?n:t.sort(function(e,t){return(t.addedCount||0)-(e.addedCount||0)}).slice(0,5),i=0;r.forEach(function(t){s.lists[e].items.find(function(e){return e.state===`active`&&e.name.toLowerCase()===t.name.toLowerCase()})||(Xr(e,t.name,1,`units`,e===`food`?Dr(t.name):`other`,``,null),i++)}),i?ci():Yr(`All usual items are already on the list`)}}function si(){var e=document.getElementById(`ls-quick-input`),t=document.getElementById(`ls-parse-preview`);if(!(!e||!t)){var n=e.value.trim();if(!n){t.innerHTML=``;return}var r=Or(n),i=``;(r.qty!==1||r.unit!==`units`)&&(i+=`<span class="ls-parse-chip">`+g(String(r.qty))+` `+g(r.unit)+`</span>`),i+=`<span class="ls-parse-chip">`+g(r.name)+`</span>`,t.innerHTML=i}}function ci(){var e=document.getElementById(`lists-content`);e&&(s.lists||window._applyMigrations(s),Ar===`selector`?li(e):ui(e,kr))}function li(e){var t=`<div class="ls-screen">`;t+=`<div style="font-size:22px;font-weight:800;color:var(--ink,#1a1814);margin-bottom:4px">My Lists</div>`,t+=`<div style="font-size:13px;color:var(--muted,#8c8880);margin-bottom:20px">Tap a list to open it</div>`,t+=`<div class="ls-type-grid">`,Object.keys(wr).forEach(function(e){var n=wr[e],r=(s.lists&&s.lists[e]?s.lists[e]:{items:[]}).items.filter(function(e){return e.state===`active`}).length;t+=`<div class="ls-type-card" onclick="_listsActiveType='`+e+`';_listsView='list';renderLists()">`,t+=`<div class="ls-type-icon" style="background:`+n.color+`;color:`+n.text+`">`+n.emoji+`</div>`,t+=`<div class="ls-type-label">`+g(n.label)+`</div>`,t+=`<div class="ls-type-count">`+(r>0?r+` item`+(r===1?``:`s`):`Empty`)+`</div>`,t+=`</div>`}),t+=`</div>`,t+=`</div>`,e.innerHTML=t}function ui(e,t){var n=wr[t],r=s.lists&&s.lists[t]?s.lists[t]:{items:[],weeklyBudget:0,favourites:[]},i=r.items||[],a=i.filter(function(e){return e.state===`active`}),o=i.filter(function(e){return e.state===`got_it`}),c=i.filter(function(e){return e.state===`not_found`}),l=`<div class="ls-screen">`;if(l+=`<button class="ls-back-btn" onclick="_listsView='selector';renderLists()">← Lists</button>`,l+=`<div class="ls-header">`,l+=`<div style="font-size:22px;margin-right:6px">`+n.emoji+`</div>`,l+=`<div class="ls-header-title">`+g(n.label)+`</div>`,l+=`<div class="ls-sync-dot"></div>`,l+=`<div class="ls-header-count">`+a.length+` to get</div>`,l+=`</div>`,r.weeklyBudget>0){var u=i.filter(function(e){return e.state===`got_it`&&e.manualPrice}).reduce(function(e,t){return e+(t.manualPrice||0)},0),d=Math.min(100,Math.round(u/r.weeklyBudget*100)),f=d>100?`over`:d>80?`warn`:``;l+=`<div class="ls-budget-bar-wrap">`,l+=`<div class="ls-budget-bar-meta"><span>$`+u.toFixed(0)+` spent</span><span>$`+r.weeklyBudget+` budget</span></div>`,l+=`<div class="ls-budget-bar"><div class="ls-budget-fill `+f+`" style="width:`+d+`%"></div></div>`,l+=`</div>`}var p=(r.favourites||[]).filter(function(e){return!a.find(function(t){return t.name.toLowerCase()===e.name.toLowerCase()})}).sort(function(e,t){return(t.addedCount||0)-(e.addedCount||0)}).slice(0,5);if(p.length>0&&(l+=`<div class="ls-fav-chips">`,p.forEach(function(e){l+=`<button class="ls-fav-chip" onclick="_listsAddItem('`+t+`','`+g(e.name).replace(/'/g,`\\'`)+`',1,'units','`+(t===`food`?Dr(e.name):`other`)+`','',null);renderLists()">+ `+g(e.name)+`</button>`}),l+=`</div>`),(r.favourites||[]).length>0&&(l+=`<button class="ls-usual-btn" onclick="_listsAddUsual('`+t+`')">The usual →</button>`),l+=`<div class="ls-quick-add">`,l+=`<div class="ls-quick-add-row">`,l+=`<input class="ls-quick-input" id="ls-quick-input" type="text" placeholder="Add item…" autocomplete="off" oninput="_listsUpdateParsePreview()" onkeydown="if(event.key==='Enter')_listsQuickAdd('`+t+`')">`,l+=`<button class="ls-quick-add-btn" onclick="_listsQuickAdd('`+t+`')">Add</button>`,l+=`<button class="ls-quick-add-btn" style="background:var(--purple-soft);color:var(--iris-1);min-width:36px;padding:0 10px" onclick="_listsOpenAddForm('`+t+`')">⋯</button>`,l+=`</div>`,l+=`<div class="ls-parse-preview" id="ls-parse-preview"></div>`,l+=`</div>`,a.length>0)if(n.aisles){var m=Tr[t]||[{key:`other`,emoji:`🛒`,label:`Other`}],h={};m.forEach(function(e){h[e.key]=[]}),a.forEach(function(e){var t=e.aisle&&h[e.aisle]!==void 0?e.aisle:`other`;h[t]||(h[t]=[]),h[t].push(e)}),m.forEach(function(e){!h[e.key]||!h[e.key].length||(l+=`<div class="ls-aisle-header">`+e.emoji+` `+g(e.label)+`</div>`,h[e.key].forEach(function(e){l+=di(t,e)}))})}else a.forEach(function(e){l+=di(t,e)});else l+=`<div style="text-align:center;padding:32px 0;color:var(--muted,#8c8880);font-size:14px">Nothing to get yet — add something above</div>`;o.length>0&&(l+=`<div class="ls-aisle-header">🛒 In the trolley</div>`,o.forEach(function(e){l+=di(t,e)})),c.length>0&&(l+=`<div class="ls-aisle-header">🚫 Not found</div>`,c.forEach(function(e){l+=di(t,e)})),l+=`<div class="ls-footer-row">`,o.length>0&&(l+=`<button class="ls-footer-btn" onclick="_listsClearTrolley('`+t+`')">Clear trolley (`+o.length+`)</button>`),a.length===0&&i.length>0&&(l+=`<button class="ls-footer-btn" style="background:var(--iris-2,#6366f1);color:#fff;border-color:var(--iris-2,#6366f1)" onclick="_listsArchive('`+t+`')">Archive this shop</button>`),l+=`</div>`,l+=`</div>`,e.innerHTML=l}function di(e,t){var n=t.state===`got_it`?`got-it`:t.state===`not_found`?`not-found`:``,r=t.state===`got_it`?`✓`:``,i=t.state===`active`?`got_it`:`active`,a=t.quantity&&t.unit&&t.unit!==`units`?t.quantity+` `+t.unit:t.quantity&&t.quantity!==1?`x`+t.quantity:``;g(t.id);var o=g(t.name),s=`<div class="ls-item `+n+`">`;return s+=`<button class="ls-item-check" onclick="_listsSetState('`+e+`','`+t.id+`','`+i+`')">`+r+`</button>`,s+=`<div class="ls-item-body">`,s+=`<div class="ls-item-name">`+o+`</div>`,a&&(s+=`<div class="ls-item-qty">`+g(a)+`</div>`),t.notes&&(s+=`<div class="ls-item-notes">`+g(t.notes)+`</div>`),s+=`</div>`,t.state===`active`?s+=`<button class="ls-item-notfound-btn" title="Not found" onclick="_listsSetState('`+e+`','`+t.id+`','not_found')">🚫</button>`:t.state===`not_found`&&(s+=`<button class="ls-item-notfound-btn" title="Mark active again" onclick="_listsSetState('`+e+`','`+t.id+`','active')">↩</button>`),s+=`<button class="ls-item-notfound-btn" title="Edit" onclick="_listsOpenAddForm('`+e+`','`+t.id+`')">✏️</button>`,s+=`<button class="ls-item-del" onclick="_listsDeleteItem('`+e+`','`+t.id+`')">×</button>`,s+=`</div>`,s}async function fi(e){let t=s.meals.plan[e]||{},n=[];for(let e=0;e<7;e++){let r=t[e]||{};[`b`,`l`,`d`].forEach(e=>{r[e]&&n.push(r[e])})}if(!n.length)return;let r=N(`toto_ai_key`);if(!r){kr=`food`,Ar=`list`,activateTab(`lists`);return}let i=document.getElementById(`gen-shop-btn`);i&&(i.textContent=`⏳ Generating…`,i.disabled=!0);let a=`Generate a grocery shopping list for these meals: ${n.join(`, `)}.
...
Combine quantities where sensible. No duplicates. No other text.`,o={Produce:`produce`,"Meat & Seafood":`meat`,"Dairy & Eggs":`dairy`,Pantry:`pantry`,Bakery:`bakery`,Frozen:`frozen`,Household:`cleaning`,Other:`other`};try{let e=(await(await fetch(CLAUDE_API,{method:`POST`,headers:{"x-api-key":r,"anthropic-version":`2023-06-01`,"content-type":`application/json`},body:JSON.stringify({model:`claude-haiku-4-5-20251001`,max_tokens:1024,messages:[{role:`user`,content:a}]})})).json()).content[0].text.match(/\[[\s\S]*\]/);if(!e)throw Error(`No JSON`);let t=JSON.parse(e[0]);s.lists||(s.lists={}),s.lists.food||(s.lists.food={items:[],weeklyBudget:200,budget:0,stores:[],favourites:[],history:[]});let n=s.lists.food.items,i=0;t.forEach(e=>{n.some(t=>t.name.toLowerCase()===e.name.toLowerCase()&&t.state===`active`)||n.push({id:`si-meal-`+Date.now()+`-`+ i++,name:e.name,quantity:1,unit:`units`,notes:e.qty||``,aisle:o[e.cat]||(Dr?Dr(e.name):`other`),state:`active`,addedBy:`meals`,addedAt:new Date().toISOString(),mealTag:`Meal plan`,manualPrice:null,barcodeId:null})}),window.saveData(s),kr=`food`,Ar=`list`,activateTab(`lists`)}catch{i&&(i.textContent=`🛒 Generate shopping list`,i.disabled=!1)}}var pi=[`Living Room`,`Dining Room`,`Kitchen`,`Master Bedroom`,`Bedroom 2`,`Bedroom 3`,`Study / Office`,`Bathroom`,`Laundry`,`Outdoor / Alfresco`,`Other`];function mi(e={}){let t=e.deliveryDate?(()=>{let[t,n,r]=e.deliveryDate.split(`-`);return`${r}/${n}/${t}`})():``;return`
    <div class="form-row">
      ...
      <div class="form-group">
        ...
        <select class="form-select" id="f-furn-room">
          ...
          ${pi.map(t=>`<option value="${t}" ${e.room===t?`selected`:``}>${t}</option>`).join(``)}
        </select>
      </div>
    </div>
    <div class="form-row">
  ...
  `}function hi(e){return{id:e,name:document.getElementById(`f-furn-name`).value.trim(),room:document.getElementById(`f-furn-room`).value,vendor:document.getElementById(`f-furn-vendor`).value.trim(),price:parseFloat(document.getElementById(`f-furn-price`).value)||0,status:document.getElementById(`f-furn-status`).value,funding:document.getElementById(`f-furn-funding`).value,deliveryDate:document.getElementById(`f-exp-duedate`).value||null,notes:document.getElementById(`f-furn-notes`).value.trim()}}function gi(){window.openModal(`Add Furniture Item`,mi(),()=>{let e=hi(k(s.furniture));e.name&&(window.logActivity(`Added furniture`,e.name),s.furniture.push(e),window.saveData(s),window.closeModal(),window.renderAll())})}function _i(e){let t=s.furniture.find(t=>t.id===e);window.openModal(`Edit Furniture Item`,mi(t),()=>{let n=hi(e);window.logActivity(`Edited furniture`,n.name||t.name),Object.assign(t,n),window.saveData(s),window.closeModal(),window.renderAll()})}function vi(e){if(!confirm(`Delete this item?`))return;let t=s.furniture.find(t=>t.id===e);window.logActivity(`Deleted furniture`,t?t.name:``),s.furniture=s.furniture.filter(t=>t.id!==e),window.saveData(s),window.renderAll()}function yi(e={}){let t=e.deliveryDate?(()=>{let[t,n,r]=e.deliveryDate.split(`-`);return`${r}/${n}/${t}`})():``;return`
    <div class="form-row">
      <div class="form-group">
        <label class="form-label">Item Name</label>
  ...
  `}function bi(e){return{id:e,name:document.getElementById(`f-appl-name`).value.trim(),room:document.getElementById(`f-appl-room`).value,vendor:document.getElementById(`f-appl-vendor`).value.trim(),price:parseFloat(document.getElementById(`f-appl-price`).value)||0,status:document.getElementById(`f-appl-status`).value,funding:document.getElementById(`f-appl-funding`).value,deliveryDate:document.getElementById(`f-exp-duedate`).value||null,notes:document.getElementById(`f-appl-notes`).value.trim()}}function xi(){window.openModal(`Add Appliance`,yi(),()=>{let e=bi(k(s.appliances));e.name&&(window.logActivity(`Added appliance`,e.name),s.appliances.push(e),window.saveData(s),window.closeModal(),window.renderAll())})}function Si(e){let t=s.appliances.find(t=>t.id===e);window.openModal(`Edit Appliance`,yi(t),()=>{let n=bi(e);window.logActivity(`Edited appliance`,n.name||t.name),Object.assign(t,n),window.saveData(s),window.closeModal(),window.renderAll()})}function Ci(e){if(!confirm(`Delete this item?`))return;let t=s.appliances.find(t=>t.id===e);window.logActivity(`Deleted appliance`,t?t.name:``),s.appliances=s.appliances.filter(t=>t.id!==e),window.saveData(s),window.renderAll()}function wi(){let[e,t]=window.selectedBudgetMonth.split(`-`).map(Number),n=new Date(e,t-2,1);window.selectedBudgetMonth=`${n.getFullYear()}-${String(n.getMonth()+1).padStart(2,`0`)}`,window.safeRender(Ei),window.safeRender(renderBudget)}function Ti(){let[e,t]=window.selectedBudgetMonth.split(`-`).map(Number),n=new Date(e,t,1);window.selectedBudgetMonth=`${n.getFullYear()}-${String(n.getMonth()+1).padStart(2,`0`)}`,window.safeRender(Ei),window.safeRender(renderBudget)}function Ei(){let e=window.getMonthData(window.selectedBudgetMonth),t=e.income,n=e.expenses,r=O(t),i=O(n),a=r-i,o=r>0?Math.round(a/r*100):0,c={};n.forEach(e=>{let t=e.category||`Other`;c[t]=(c[t]||0)+D(e)});let l=Object.entries(c).sort((e,t)=>t[1]-e[1]),u=s.budget.actuals[window.selectedBudgetMonth]||{},d=n.reduce((e,t)=>e+window.getActual(t.id,window.selectedBudgetMonth),0),f=d>0,p=`
  ...
  `,document.getElementById(`money-content`).innerHTML=p}function Di(){if(s.setupProgressDismissed)return``;let e=window.setupProgressTasks(),t=e.filter(e=>e.done),n=e.filter(e=>!e.done),r=t.length,i=e.length,a=Math.round(r/i*100);if(r===i)return`<div class="td-card td-card-win" style="display:flex;align-items:center;gap:12px;margin-bottom:10px">
      <span style="font-size:22px">🎉</span>
      <div style="flex:1">
    ...
    </div>`}).join(``)+`</div>`}function Fi(e){let t=e.onclick?e.onclick:e.tab?`activateTab('${e.tab}')`:``,n=Ai(e),r=[`red`,`amber`,`green`,`blue`].includes(e.cls)?e.cls:`grey`;return`<div class="brief-row"${t?` onclick="${t}"`:``}>
    ...
    <svg class="brief-arrow" width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><polyline points="9 18 15 12 9 6"/></svg>
      ...
      </div>`;d.push({type:`priority`,urgency:f.length>0?3:h?2:0,html:`<div style="display:flex;gap:12px;margin-bottom:12px">${t}${n}</div>`})}let _=o===`evening`||o===`night`?new Date(t.getTime()+864e5).toISOString().slice(0,10):i,v=_===i?`Today`:`Tomorrow`,y=Ut?Ut(_):[];if(y.length>0){let e=t.getHours()*60+t.getMinutes(),n=y.slice(0,4).map((t,n,r)=>{let a=t.allDay||!t.time?`All day`:Wt?Wt(t.time):t.time,o=Bt?Bt(t):``,s=zt?zt(t):{dot:`var(--iris-2)`},c=P?P[t.category]||P.other:{emoji:`📅`,label:``},l=t.time?parseInt(t.time.split(`:`)[0])*60+parseInt(t.time.split(`:`)[1]):-1,u=_===i&&l>=0&&e>=l&&e<l+90,d=n===Math.min(y.length,4)-1,f=c.color||`#f1f5f9`,p=c.text||`#475569`;return p.replace(/^#/,``),`<div class="pl-agenda-ev" style="margin-bottom:${d?`0`:`8px`}">
        <div class="pl-agenda-time-col">
    ...
    ${ce}
    ${le}
  `,s.settings?.typeAMode&&window._checkMissionEscalation(),typeof Gi==`function`){let e=typeof window.calcFinancialHealth==`function`?window.calcFinancialHealth():null,n=typeof _mealWeekKey==`function`?_mealWeekKey(0):null,r=n&&s.meals?.plan?.[n]?.[t.getDay()===0?6:t.getDay()-1]||{};Gi(d.map(e=>({title:e.type})),C,T,r,e)}}function Li(){let e=(s.bills||[]).map(e=>({...e,days:n(e)})).filter(e=>e.days!==null&&e.days<=2).sort((e,t)=>e.days-t.days);if(!e.length)return;let t=e.reduce((e,t)=>e+(parseFloat(t.amount)||0),0);zi(`Heads Up`,e.map(e=>{let t=e.days===0?`Due today`:e.days===1?`Tomorrow`:`In ${e.days} days`,n=e.days===0?`background:#FEF2F2;color:#b91c1c`:e.days===1?`background:#FFF4EE;color:#c2410c`:`background:var(--paper);color:var(--muted)`,r=e.amount?`$${parseFloat(e.amount).toLocaleString(`en-AU`,{minimumFractionDigits:2,maximumFractionDigits:2})}`:``,i=e.days===0?`box-shadow:0 0 0 3px rgba(239,68,68,.2)`:e.days===1?`box-shadow:0 0 0 3px rgba(249,115,22,.2)`:``;return`<div style="display:flex;align-items:center;padding:13px 18px;border-bottom:1px solid var(--hairline);gap:14px">
  ...
  </div>`)}function Ri(){let e=new Date,t=[];(s.documents||[]).forEach(n=>{n.expiryDate&&new Date(n.expiryDate)<e&&t.push({label:n.name,sub:`Documents`,badge:`Expired`,cls:`alert`,tab:`documents`})}),(s.maintenance||[]).forEach(e=>{let n=window._maintDaysUntil(e);n!==null&&n<0&&t.push({label:e.name,sub:`Maintenance`,badge:`${Math.abs(n)}d overdue`,cls:`watch`,tab:`maintenance`})}),(s.vehicles||[]).forEach(n=>{n.regoExpiry&&new Date(n.regoExpiry)<e&&t.push({label:n.name+` rego`,sub:`Vehicles`,badge:`Expired`,cls:`alert`,tab:`vehicles`})}),t.length&&zi(`Slipping`,t.map(e=>{let t=e.cls===`alert`?`#ef4444`:`var(--ember)`,n=e.cls===`alert`?`box-shadow:0 0 0 3px rgba(239,68,68,.15)`:`box-shadow:0 0 0 3px rgba(249,115,22,.18)`,r=e.cls===`alert`?`background:#FEF2F2;color:#b91c1c`:`background:#FFF4EE;color:#c2410c`;return`<div onclick="activateTab('${e.tab}');_tdCloseSheet()" style="display:flex;align-items:center;padding:13px 18px;border-bottom:1px solid var(--hairline);gap:14px;cursor:pointer">
  ...
  </div>`)}function zi(e,t){let n=document.getElementById(`td-sheet-overlay`);n||(n=document.createElement(`div`),n.id=`td-sheet-overlay`,n.style.cssText=`position:fixed;inset:0;z-index:1200;display:flex;flex-direction:column;justify-content:flex-end;background:rgba(0,0,0,.4)`,n.onclick=e=>{e.target===n&&Bi()},document.body.appendChild(n)),n.innerHTML=`
  ...
  </div>`).join(``);typeof window.openModal==`function`&&window.openModal(`💡 Insights`,`<div style="padding:0 4px">${i}</div>`,null)}var Ui=``,Wi=``;async function Gi(e,t,n,r,i){let a=N(`toto_ai_key`);if(!a)return;let o=new Date().toISOString().slice(0,10);if(Ui===o&&Wi){let e=document.getElementById(`today-briefing-text`);e&&(e.textContent=Wi);return}let c=e.filter(e=>e.cls===`red`).map(e=>e.title),l=e.filter(e=>e.cls===`amber`).map(e=>e.title),u=`You are Toto, a friendly family personal assistant app. Write a 2-sentence daily briefing for the user based on this context:
...
- Don't start with "Here's" or "Today"`;try{let e=await fetch(CLAUDE_API,{method:`POST`,headers:{"x-api-key":a,"anthropic-version":`2023-06-01`,"content-type":`application/json`},body:JSON.stringify({model:`claude-haiku-4-5-20251001`,max_tokens:100,messages:[{role:`user`,content:u}]})});if(!e.ok)return;let t=(await e.json()).content[0].text.trim().replace(/^["']|["']$/g,``);Wi=t,Ui=o;let n=document.getElementById(`today-briefing-text`);n&&(n.textContent=t)}catch{}}function Ki(){let e=s,r=e.buildContract,i=(e.netWorth.assets||[]).reduce((e,t)=>e+(parseFloat(t.value)||0),0),a=(e.netWorth.liabilities||[]).reduce((e,t)=>e+(parseFloat(t.value)||0),0),o=i-a,c=e.netWorth.snapshots||[],l=``;if(c.length>=2){let e=o-c[c.length-2].netWorth;l=`<span class="${e>=0?`up`:`dn`}">${e>=0?`+`:``}${fmtNW(e)}</span> vs last snapshot`}let u=window.selectedBudgetMonth,d=window.getMonthData(u),f=O(d.income),p=O(d.expenses),m=f-p,h=(e.subscriptions||[]).reduce((e,t)=>e+subMonthlyAmount(t),0),_=(e.bills||[]).filter(e=>{let t=n(e);return t>=0&&t<=31}).reduce((e,t)=>e+(parseFloat(t.amount)||0),0),v=[...e.bills||[]].filter(e=>n(e)>=-1).sort((e,t)=>n(e)-n(t)).slice(0,5),y=(e.goals||[]).filter(e=>e.status!==`achieved`).slice(0,4),x=r.stages.filter(e=>e.paid).reduce((e,t)=>e+t.amount,0),S=(r.variations||[]).filter(e=>e.status===`approved`).reduce((e,t)=>e+(t.amount||0),0),C=r.total+S,w=Math.round(x/C*100),T=r.stages.find(e=>!e.paid),ee=((e.kids||{}).completions||[]).filter(e=>e.status===`pending`).length+((e.kids||{}).redemptions||[]).filter(e=>e.status===`pending`).length,E=window.getLast6Months(),te=O(d.expenses),D=E.map(e=>{let t=Object.values(s.budget.actuals[e]||{}).reduce((e,t)=>e+t,0);return{label:window.monthShortLabel(e),budget:te,actual:t}}),k=te>0||D.some(e=>e.actual>0),A=window.calcFinancialHealth(),ne=251.3,re=(A.total/100*ne).toFixed(1),ie=A.dimensions.map(e=>{let t=Math.round(e.score/e.max*100),n=t>=75?`#10b981`:t>=50?`#f59e0b`:`#ef4444`;return`
    ...
    ${ae}
  `;document.getElementById(`dashboard-content`).innerHTML=oe}function qi(){function e(e){return e.replace(/[^a-z0-9]/gi,`_`)}function t(t,n,r,i){let a=`${t}_${e(n)}`;return`
      <div class="color-row">
      ...
      </div>`}let n=N(window.STORAGE_KEY),r=n?(n.length/1024).toFixed(1):0,i=n?`<span style="color:var(--success);font-weight:600">✓ Data found in device storage (${r} KB)</span>`:`<span style="color:var(--danger);font-weight:600">⚠ No data in device storage</span>`,a=s.activityLog||[];function o(e){let t=new Date(e);return t.toLocaleDateString(`en-AU`,{day:`numeric`,month:`short`,year:`numeric`})+` `+t.toLocaleTimeString(`en-AU`,{hour:`2-digit`,minute:`2-digit`})}function c(e){return e.photo?`<img src="${e.photo}" style="width:28px;height:28px;border-radius:50%;object-fit:cover;flex-shrink:0" onerror="this.style.display='none'">`:`<div style="width:28px;height:28px;border-radius:50%;background:var(--primary);color:#fff;display:flex;align-items:center;justify-content:center;font-size:12px;font-weight:700;flex-shrink:0">${(e.name||`?`)[0].toUpperCase()}</div>`}let l={Added:`#10b981`,Edited:`#3b82f6`,Deleted:`#ef4444`,Updated:`#f59e0b`,Imported:`#8b5cf6`,Removed:`#ef4444`};function u(e){return l[e.split(` `)[0]]||`#94a3b8`}function d(e,t,n,r,i){let a=window._settingsOpen.has(e);return`
      <div class="sacc-item">
  ...
  </div>`,h+=`<div class="settings-save-bar" id="settings-save-bar" style="display:${window._settingsDirty?`flex`:`none`}">
  ...
  </div>`,document.getElementById(`settings-content`).innerHTML=h}function Ji(){if(confirm(`This will permanently delete ALL household data — budget, kids, goals, everything — from this device and the cloud.

This cannot be undone. Are you sure?`)&&confirm(`Last chance. All data will be gone. Continue?`)){if(window._currentUser&&fbStore){let e=window._getHouseholdDocRef();e&&e.delete().catch(()=>{})}Ze(window.STORAGE_KEY),window._secureClear(HOUSEHOLD_OWNER_KEY),window._fsUnsubscribe&&(window._fsUnsubscribe(),window._fsUnsubscribe=null),fbAuth.signOut().then(()=>{window.location.reload()})}}function Yi(){let e=new Blob([JSON.stringify(s,null,2)],{type:`application/json`}),t=document.createElement(`a`);t.href=URL.createObjectURL(e),t.download=`home-budget-backup-${new Date().toISOString().slice(0,10)}.json`,t.click(),URL.revokeObjectURL(t.href)}function Xi(e){let t=e.target.files[0];if(!t)return;let n=new FileReader;n.onload=e=>{try{let t=JSON.parse(e.target.result);if(!t.budget){alert(`Invalid backup file — missing budget data.`);return}if(!confirm(`This will replace ALL current data with the backup. Continue?`))return;Xe(window.STORAGE_KEY,JSON.stringify(t)),location.reload()}catch(e){alert(`Failed to read backup file: `+e.message)}},n.readAsText(t)}function Zi(e){let t=document.getElementById(`grp-body-${e}`),n=document.getElementById(`grp-arrow-${e}`);if(!t)return;let r=t.style.display!==`none`;t.style.display=r?`none`:`block`,n&&(n.textContent=r?`▼`:`▲`)}function Qi(e){window.budgetViewMode=e,renderBudget()}function $i(){let e=s.buildContract,t=e.stages.filter(e=>e.paid).reduce((e,t)=>e+t.amount,0);e.total-t;let n=e.stages.filter(e=>e.paid).length,r=e.stages.length,i=e.variations||[],a=i.filter(e=>e.status===`approved`).reduce((e,t)=>e+(t.amount||0),0),o=i.filter(e=>e.status===`pending`).reduce((e,t)=>e+(t.amount||0),0),c=e.total+a,l=`
  ...
  `,document.getElementById(`build-content`).innerHTML=l}function ea(){return s.expenseCategories||DEFAULT_DATA.expenseCategories}function ta(){return s.incomeCategories||DEFAULT_DATA.incomeCategories}function na(e){if(e.type===`spending`){let t=Object.keys(s.budget.actuals).sort().reverse(),n=null;for(let r of t){let t=(window.getMonthData(r).expenses||[]).filter(t=>(t.category||`Other`)===e.category).reduce((e,t)=>e+(s.budget.actuals[r]?.[t.id]||0),0);if(t>0){n=t;break}}let r=e.targetMonthly||0;if(n===null)return{pct:null,label:`No actuals yet`,actual:null,target:r};let i=r>0?Math.max(0,Math.min(100,n/r*100)):0,a=n<=r;return{pct:i,label:`${b(n)} actual vs ${b(r)} limit`,actual:n,target:r,ok:a}}if(e.type===`savings`){let t=e.currentSaved||0,n=e.targetTotal||1;return{pct:Math.min(100,t/n*100),label:`${b(t)} of ${b(n)} saved`,ok:t>=n}}if(e.type===`income`){let t=monthlyTotal(window.getMonthData(window.selectedBudgetMonth).income),n=e.targetMonthly||1;return{pct:Math.min(100,t/n*100),label:`${b(t)}/mo of ${b(n)}/mo target`,ok:t>=n}}return{pct:0,label:``,ok:!1}}function ra(){let e=s.goals,t=e.filter(e=>e.status===`active`),n=e.filter(e=>e.status===`achieved`),r=``;e.length>0&&(r+=`
    ...
    <button class="btn btn-primary" onclick="openAddGoal()">+ New Goal</button>
      ...
      <div style="background:#fff;border:2px dashed #cbd5e1;border-radius:16px;padding:36px 24px;text-align:center;margin-top:8px">
        ...
        </div>`});document.getElementById(`goals-content`).innerHTML=r}function ia(e={}){let t=e.type||`spending`;return`
    <div class="form-group">
  ...
  `}function aa(){let e=document.getElementById(`f-goal-type`).value;document.getElementById(`goal-spending-fields`).style.display=e===`spending`?``:`none`,document.getElementById(`goal-savings-fields`).style.display=e===`savings`?``:`none`,document.getElementById(`goal-income-fields`).style.display=e===`income`?``:`none`}function oa(e){let t=document.getElementById(`f-goal-type`).value,n={id:e,name:document.getElementById(`f-goal-name`).value.trim(),type:t,status:document.getElementById(`f-goal-status`).value,deadline:document.getElementById(`f-goal-deadline`).value||null,notes:document.getElementById(`f-goal-notes`).value.trim()};return t===`spending`&&(n.category=document.getElementById(`f-goal-category`).value,n.targetMonthly=parseFloat(document.getElementById(`f-goal-target-monthly`).value)||0),t===`savings`&&(n.targetTotal=parseFloat(document.getElementById(`f-goal-target-total`).value)||0,n.currentSaved=parseFloat(document.getElementById(`f-goal-current-saved`).value)||0),t===`income`&&(n.targetMonthly=parseFloat(document.getElementById(`f-goal-target-monthly-inc`).value)||0),n}function sa(){window.openModal(`New Goal`,ia(),()=>{let e=oa(k(s.goals));e.name&&(s.goals.push(e),window.saveData(s),window.closeModal(),ra())})}function ca(e){let t=s.goals.find(t=>t.id===e);window.openModal(`Edit Goal`,ia(t),()=>{Object.assign(t,oa(e)),window.saveData(s),window.closeModal(),ra()})}function la(e){confirm(`Delete this goal?`)&&(s.goals=s.goals.filter(t=>t.id!==e),window.saveData(s),ra())}function ua(e){let t=s.goals.find(t=>t.id===e);t&&(t.status=`achieved`,window.saveData(s),ra())}var da=null,fa=[{value:`add-income`,label:`Add income source`,icon:`💰`},{value:`remove-income`,label:`Remove income source`,icon:`➖`},{value:`reduce-income`,label:`Reduce income amount`,icon:`📉`},{value:`add-expense`,label:`Add new expense`,icon:`➕`},{value:`remove-expense`,label:`Remove expense`,icon:`✂️`},{value:`reduce-expense`,label:`Reduce expense amount`,icon:`📉`}];function pa(e){let t=window.getMonthData(window.selectedBudgetMonth),n=JSON.parse(JSON.stringify(t.income)),r=JSON.parse(JSON.stringify(t.expenses));return(e.adjustments||[]).forEach(e=>{if(e.type===`add-income`)n.push({id:-(e.id||0),name:e.name,amount:e.amount||0,frequency:e.frequency||`monthly`});else if(e.type===`remove-income`)n=n.filter(t=>t.id!==e.itemId);else if(e.type===`reduce-income`){let t=n.find(t=>t.id===e.itemId);t&&(t.amount=e.changeType===`percent`?Math.max(0,t.amount*(1-e.changeAmount/100)):Math.max(0,t.amount-(e.changeAmount||0)))}else if(e.type===`add-expense`)r.push({id:-(e.id||0),name:e.name,amount:e.amount||0,frequency:e.frequency||`monthly`,category:e.category||`Other`});else if(e.type===`remove-expense`)r=r.filter(t=>t.id!==e.itemId);else if(e.type===`reduce-expense`){let t=r.find(t=>t.id===e.itemId);t&&(t.amount=e.changeType===`percent`?Math.max(0,t.amount*(1-e.changeAmount/100)):Math.max(0,t.amount-(e.changeAmount||0)))}}),{income:n,expenses:r,totalIncome:O(n),totalExpenses:O(r),surplus:O(n)-O(r)}}function ma(){let e=s.scenarios,t=window.getMonthData(window.selectedBudgetMonth),n=O(t.income),r=O(t.expenses),i=n-r,a=`<div style="display:flex;justify-content:flex-end;margin-bottom:16px">
    <button class="btn btn-primary" onclick="openAddScenario()">+ New Scenario</button>
  </div>`;e.length===0?a+=`<div class="empty"><div class="empty-icon">🔬</div>No scenarios yet. Create one to model income changes, expense cuts, or lifestyle adjustments.</div>`:e.forEach(e=>{let t=pa(e),o=t.totalIncome-n,s=t.totalExpenses-r,c=t.surplus-i,l=da===e.id;a+=`
        ...
        </div>`}),document.getElementById(`scenarios-content`).innerHTML=a}function ha(e){da=da===e?null:e,ma()}function ga(e={}){return`
    <div class="form-group">
  ...
  `}function _a(e,t={}){return{id:e,name:document.getElementById(`f-sc-name`).value.trim(),description:document.getElementById(`f-sc-desc`).value.trim(),adjustments:t.adjustments||[]}}function va(){window.openModal(`New Scenario`,ga(),()=>{let e=_a(k(s.scenarios));e.name&&(s.scenarios.push(e),window.saveData(s),window.closeModal(),ma())})}function ya(e){let t=s.scenarios.find(t=>t.id===e);window.openModal(`Edit Scenario`,ga(t),()=>{let n=_a(e,t);Object.assign(t,n),window.saveData(s),window.closeModal(),ma()})}function ba(e){confirm(`Delete this scenario?`)&&(s.scenarios=s.scenarios.filter(t=>t.id!==e),da===e&&(da=null),window.saveData(s),ma())}function xa(e){let t=window.getMonthData(window.selectedBudgetMonth),n=t.income.map(e=>`<option value="${e.id}">${g(e.name)} (${b(D(e))}/mo)</option>`).join(``),r=t.expenses.map(e=>`<option value="${e.id}">${g(e.name)} (${b(D(e))}/mo)</option>`).join(``);return`
    <div class="form-group">
      <label class="form-label">Adjustment Type</label>
      <select class="form-select" id="f-adj-type" onchange="toggleAdjFields()">
        ${fa.map(e=>`<option value="${e.value}">${e.icon} ${e.label}</option>`).join(``)}
      </select>
    </div>
  ...
  `}function Sa(){let e=document.getElementById(`f-adj-type`).value;document.getElementById(`adj-add-income`).style.display=e===`add-income`||e===`add-expense`?``:`none`,document.getElementById(`adj-add-income-extra`).style.display=e===`add-income`||e===`add-expense`?``:`none`,document.getElementById(`adj-cat-wrap`).style.display=e===`add-expense`?``:`none`,document.getElementById(`adj-remove-income`).style.display=e===`remove-income`?``:`none`,document.getElementById(`adj-reduce-income`).style.display=e===`reduce-income`?``:`none`,document.getElementById(`adj-remove-expense`).style.display=e===`remove-expense`?``:`none`,document.getElementById(`adj-reduce-expense`).style.display=e===`reduce-expense`?``:`none`}function Ca(e){let t=s.scenarios.find(t=>t.id===e);t&&window.openModal(`Add Adjustment`,xa(t),()=>{let e=document.getElementById(`f-adj-type`).value,n=window.getMonthData(window.selectedBudgetMonth),r={id:k(t.adjustments||[]),type:e};if(e===`add-income`){if(r.name=document.getElementById(`f-adj-name`).value.trim(),r.amount=parseFloat(document.getElementById(`f-adj-amount`).value)||0,r.frequency=document.getElementById(`f-adj-freq`).value,!r.name)return}else if(e===`add-expense`){if(r.name=document.getElementById(`f-adj-name`).value.trim(),r.amount=parseFloat(document.getElementById(`f-adj-amount`).value)||0,r.frequency=document.getElementById(`f-adj-freq`).value,r.category=document.getElementById(`f-adj-cat`).value,!r.name)return}else if(e===`remove-income`){let e=document.getElementById(`f-adj-inc-sel`);r.itemId=parseInt(e.value),r.itemName=e.options[e.selectedIndex]?.text||``}else if(e===`reduce-income`){let e=document.getElementById(`f-adj-inc-reduce-sel`);r.itemId=parseInt(e.value),r.itemName=n.income.find(e=>e.id===r.itemId)?.name||``,r.changeAmount=parseFloat(document.getElementById(`f-adj-change-amount`).value)||0,r.changeType=document.getElementById(`f-adj-change-type-inc`).value}else if(e===`remove-expense`){let e=document.getElementById(`f-adj-exp-sel`);r.itemId=parseInt(e.value),r.itemName=e.options[e.selectedIndex]?.text||``}else if(e===`reduce-expense`){let e=document.getElementById(`f-adj-exp-reduce-sel`);r.itemId=parseInt(e.value),r.itemName=n.expenses.find(e=>e.id===r.itemId)?.name||``,r.changeAmount=parseFloat(document.getElementById(`f-adj-change-amount-exp`).value)||0,r.changeType=document.getElementById(`f-adj-change-type-exp`).value}t.adjustments||(t.adjustments=[]),t.adjustments.push(r),window.saveData(s),window.closeModal(),ma()})}function wa(e,t){let n=s.scenarios.find(t=>t.id===e);n&&(n.adjustments=(n.adjustments||[]).filter(e=>e.id!==t),window.saveData(s),ma())}var Ta=`home_finance_colors_v1`,Ea={expense:{"Mortgage / Rent":`#6366f1`,Insurance:`#8b5cf6`,Utilities:`#06b6d4`,Groceries:`#10b981`,Transport:`#f59e0b`,"Childcare / Education":`#3b82f6`,Health:`#ef4444`,Entertainment:`#f97316`,Subscriptions:`#84cc16`,"Dining Out":`#14b8a6`,Clothing:`#ec4899`,"Personal Care":`#a855f7`,"Savings / Investment":`#22c55e`,Other:`#94a3b8`},income:`#10b981`,build:{contract:`#3b82f6`,extras:`#f59e0b`,furniture:`#8b5cf6`,appliances:`#ef4444`}};function Da(){try{let e=N(Ta);if(!e)return JSON.parse(JSON.stringify(Ea));let t=JSON.parse(e);return t.expense||(t.expense={}),t.build||(t.build={}),t.income||(t.income=Ea.income),ea().forEach(e=>{t.expense[e]||(t.expense[e]=Ea.expense[e]||`#94a3b8`)}),Object.keys(Ea.build).forEach(e=>{t.build[e]||(t.build[e]=Ea.build[e])}),t}catch{return JSON.parse(JSON.stringify(Ea))}}function Oa(e){Xe(Ta,JSON.stringify(e))}var ka=Da();Object.defineProperty(window,`colors`,{get(){return ka},set(e){ka=e},configurable:!0});function Aa(e,t,n){e===`expense`?ka.expense[t]=n:e===`income`?ka.income=n:e===`build`&&(ka.build[t]=n),Oa(ka),renderBudget(),renderBuild(),renderDashboard()}function ja(){return(s.householdProfile.members||[]).filter(e=>e.role===`adult`).length||1}function Ma(){return(s.householdProfile.members||[]).filter(e=>e.role===`child`).length}function Na(e){s.householdProfile.members.push({role:e||`adult`,age:null}),go(),renderSettings()}function Pa(e){let t=s.householdProfile.members[e];if(!t)return;let n=t.name||(t.role===`child`?`this child`:`this adult`);if(confirm(`Remove ${n} from the household?\n\nThis cannot be undone.`)){if(t.role===`child`&&t.name){let e=(s.kids?.profiles||[]).find(e=>e.name===t.name);e&&(s.kids.profiles=s.kids.profiles.filter(t=>t.id!==e.id),s.kids.chores=s.kids.chores.filter(t=>t.assignedTo!==e.id),s.kids.completions=s.kids.completions.filter(t=>t.kidId!==e.id),s.kids.redemptions=s.kids.redemptions.filter(t=>t.kidId!==e.id),s.meals?.lunchbox?.profiles&&(s.meals.lunchbox.profiles=s.meals.lunchbox.profiles.filter(t=>t.id!==e.id)),String(window.getDeviceProfile())===String(e.id)&&window.setDeviceProfile(`adult`))}s.householdProfile.members.splice(e,1),window.saveData(s),window.renderAll()}}function Fa(e,t,n){let r=s.householdProfile.members[e];r&&(r[t]=n,go(),(t===`role`||t===`name`)&&renderSettings())}function Ia(e){s.householdProfile.pets.push({type:e||`dog`,name:``}),go(),renderSettings()}function La(e){s.householdProfile.pets.splice(e,1),go(),renderSettings()}function Ra(e,t,n){let r=s.householdProfile.pets[e];r&&(r[t]=n,go())}function za(e){s.householdProfile.cars=e,go()}function Ba(e,t,n){let r=t+n;return[{category:`Mortgage / Rent`,min:e*.2,max:e*.3,label:`20–30% of income`,source:`ABS / MoneySmart`,needs:!0},{category:`Groceries`,min:380+(t-1)*260+n*160,max:560+(t-1)*360+n*220,label:`$${Math.round(380+(t-1)*260+n*160)}–$${Math.round(560+(t-1)*360+n*220)}/month for ${r} ${r===1?`person`:`people`}`,source:`ABS HES 2022`,needs:!0},{category:`Transport`,min:e*.08,max:e*.15,label:`8–15% of income`,source:`ABS HES 2022`,needs:!0},{category:`Utilities`,min:180+t*25+n*15,max:360+t*40+n*25,label:`$${Math.round(180+t*25+n*15)}–$${Math.round(360+t*40+n*25)}/month`,source:`AER / ABS`,needs:!0},{category:`Insurance`,min:180+t*40+n*20,max:420+t*60+n*30,label:`$${Math.round(180+t*40+n*20)}–$${Math.round(420+t*60+n*30)}/month`,source:`APRA industry avg`,needs:!0},{category:`Health`,min:60*t+30*n,max:180*t+60*n,label:`$${60*t+30*n}–$${180*t+60*n}/month`,source:`AIHW / ABS`,needs:!0},...n>0?[{category:`Childcare / Education`,min:700*n,max:2200*n,label:`$700–$2,200/month per child (before subsidies)`,source:`ACCC Childcare Report`,needs:!0}]:[],{category:`Dining Out`,min:e*.02,max:e*.05,label:`2–5% of income`,source:`MoneySmart`,needs:!1},{category:`Entertainment`,min:e*.02,max:e*.05,label:`2–5% of income`,source:`MoneySmart`,needs:!1},{category:`Subscriptions`,min:30,max:120,label:`$30–$120/month`,source:`Industry average`,needs:!1},{category:`Clothing`,min:50*t+30*n,max:150*t+80*n,label:`$${50*t+30*n}–$${150*t+80*n}/month`,source:`ABS HES 2022`,needs:!1},{category:`Savings / Investment`,min:e*.1,max:e*.2,label:`10–20% of income (aim for 20%)`,source:`50/30/20 rule`,needs:!1}]}function Va(e,t,n){return e<t*.9?`under`:e>n*1.1?`over`:`within`}var Ha=`home_finance_ai_key`;function Ua(){return N(`home_finance_ai_key`)||``}function Wa(e){Xe(Ha,e)}function Ga(){let e=window.getLast6Months(),t={};return e.forEach(e=>{let n=window.getMonthData(e),r={},i={};n.expenses.forEach(t=>{let n=t.category||`Other`;r[n]=(r[n]||0)+D(t);let a=window.getActual(t.id,e);a>0&&(i[n]=(i[n]||0)+a)}),new Set([...Object.keys(r),...Object.keys(i)]).forEach(n=>{t[n]||(t[n]=[]),t[n].push({mo:e,budget:r[n]||0,actual:i[n]||0,hasActual:(i[n]||0)>0})})}),t}function Ka(e){let t=[];return Object.entries(e).forEach(([e,n])=>{let r=n.filter(e=>e.hasActual);if(r.length<2)return;let i=r.filter(e=>e.budget>0&&e.actual>e.budget*1.05).length,a=r.filter(e=>e.budget>0&&e.actual<e.budget*.92).length,o=r.reduce((e,t)=>e+(t.actual-t.budget),0)/r.length,s=n.slice(-3).filter(e=>e.hasActual),c=n.slice(0,3).filter(e=>e.hasActual),l=s.length?s.reduce((e,t)=>e+t.actual,0)/s.length:0,u=c.length?c.reduce((e,t)=>e+t.actual,0)/c.length:0,d=u>50?(l-u)/u:0;i>=3&&o>20?t.push({cat:e,level:`warning`,icon:`⚠️`,title:`Consistently over on ${e}`,body:`Over budget ${i}/${r.length} months, avg +${b(Math.abs(o))}/mo. Consider raising the budget or cutting back.`,months:r}):a>=4&&n[n.length-1]?.budget>0?t.push({cat:e,level:`good`,icon:`✅`,title:`Consistently under on ${e}`,body:`Under budget ${a}/${r.length} months, avg ${b(Math.abs(o))}/mo less. You may be able to reallocate this budget elsewhere.`,months:r}):d>.25&&l>50?t.push({cat:e,level:`warning`,icon:`📈`,title:`${e} trending up`,body:`Spending up ${Math.round(d*100)}% over recent months — now averaging ${b(l)}/mo. Worth keeping an eye on.`,months:r}):d<-.25&&u>50&&t.push({cat:e,level:`good`,icon:`📉`,title:`${e} trending down`,body:`Down ${Math.round(Math.abs(d)*100)}% recently — now ${b(l)}/mo. Nice improvement.`,months:r})}),t.sort((e,t)=>[`warning`,`alert`,`good`,`info`].indexOf(e.level)-[`warning`,`alert`,`good`,`info`].indexOf(t.level)).slice(0,6)}function qa(){let e=window.getMonthData(window.selectedBudgetMonth);if(e.expenses.length===0)return``;let t={};e.expenses.forEach(e=>{let n=e.category||`Other`;t[n]||(t[n]={budget:0,actual:0}),t[n].budget+=D(e),t[n].actual+=window.getActual(e.id,window.selectedBudgetMonth)});let n=Object.entries(t).filter(([,e])=>e.budget>0||e.actual>0).sort((e,t)=>Math.max(t[1].budget,t[1].actual)-Math.max(e[1].budget,e[1].actual));if(!n.length)return``;let r=Math.max(...n.flatMap(([,e])=>[e.budget,e.actual]),1),i=n.map(([e,t])=>{let n=t.actual>0,i=(t.budget/r*100).toFixed(1),a=(t.actual/r*100).toFixed(1),o=t.actual-t.budget,s=n?o>5?`over`:o<-5?`under`:``:``,c=n?o>5?`<span class="spi-over">+${b(o)}</span>`:o<-5?`<span class="spi-under">${b(o)}</span>`:`<span class="spi-on">on track</span>`:`<span class="spi-no-actual">no actuals</span>`;return`<div class="spi-cat-row">
    ...
    ${i}
  </div>`}function Ja(){let e=Ka(Ga()),t=qa(),n={warning:{bg:`#fffbeb`,border:`#fcd34d`,title:`#92400e`},good:{bg:`#ecfeff`,border:`#86efac`,title:`#166534`},alert:{bg:`#fef2f2`,border:`#fca5a5`,title:`#991b1b`},info:{bg:`#f8fafc`,border:`#cbd5e1`,title:`#475569`}};return`<div class="spi-section">
...
Reply with ONLY the JSON array, no other text.`;try{let e=await fetch(Xa,{method:`POST`,headers:{"Content-Type":`application/json`},body:JSON.stringify({model:`claude-haiku-4-5-20251001`,max_tokens:1024,messages:[{role:`user`,content:d}]})});if(!e.ok){let t=await e.json().catch(()=>({}));throw Error(t.error?.message||`API error ${e.status}`)}let t=(await e.json()).content[0].text.trim();t=t.replace(/^```[a-z]*\n?/i,``).replace(/\n?```$/,``).trim(),Qa(JSON.parse(t),!0)}catch(e){let t=e.message;(t.includes(`Failed to fetch`)||t.includes(`NetworkError`)||t.includes(`CORS`))&&(t=`CORS blocked — the browser can't call the Anthropic API directly. We need a small proxy (Cloudflare Worker). Ask me to set it up — it takes 5 minutes and is free.`),document.getElementById(`ai-output`).innerHTML=`
    ...
    <div style="display:flex;flex-direction:column;gap:12px">
      ...
      </div>`}let h=i.filter(e=>r[e.category]!==void 0||e.category===`Savings / Investment`).map(e=>{let t=r[e.category]||0,n=Va(t,e.min,e.max),i=n===`under`?`#3b82f6`:n===`within`?`#10b981`:`#ef4444`,a=n===`under`?`Below avg`:n===`within`?`On track`:`Above avg`,o=e.max>0?Math.min(t/(e.max*1.5)*100,100):0,s=e.max>0?Math.min(e.max/(e.max*1.5)*100,100):0;return`
      <tr>
        <td style="border-left:3px solid ${ka.expense[e.category]||`#94a3b8`};font-weight:500">${e.category}</td>
  ...
  `}function to(e){let t={good:{bg:`#ecfeff`,border:`#86efac`,text:`#166534`},ok:{bg:`#eff6ff`,border:`#93c5fd`,text:`#1e40af`},warning:{bg:`#fffbeb`,border:`#fcd34d`,text:`#92400e`},alert:{bg:`#fef2f2`,border:`#fca5a5`,text:`#991b1b`},info:{bg:`#f8fafc`,border:`#cbd5e1`,text:`#475569`}};return e.map(e=>{let n=t[e.level]||t.info;return`
      ...
      </div>`}).join(``)}function no(){let[e,t]=window.selectedBudgetMonth.split(`-`).map(Number),n=new Date(e,t-2,1);window.selectedBudgetMonth=`${n.getFullYear()}-${String(n.getMonth()+1).padStart(2,`0`)}`,window.safeRender(eo),window.safeRender(renderMoneyDashboard),window.safeRender(renderBudget)}function ro(){let[e,t]=window.selectedBudgetMonth.split(`-`).map(Number),n=new Date(e,t,1);window.selectedBudgetMonth=`${n.getFullYear()}-${String(n.getMonth()+1).padStart(2,`0`)}`,window.safeRender(eo),window.safeRender(renderMoneyDashboard),window.safeRender(renderBudget)}var io=`🏠.🏡.🏗️.🔑.💡.🔌.🚿.🛋️.🛏️.🪴.🍽️.🍕.🍔.🛒.🥗.🍷.☕.🍰.🥩.🧃.🚗.🚙.🚌.✈️.⛽.🚕.🏎️.🚲.🛵.🚂.👨‍👩‍👧.👶.📚.🏫.💊.🏥.💅.💆.🧴.👕.🎮.🎬.🎵.🏋️.📺.🎲.🏄.🎯.🎨.🎭.💰.💳.🏦.📈.💸.🪙.💎.📊.🏆.💼.📦.🛍️.🎁.🔧.🛠️.📱.💻.🧹.🧺.🖨️.🐕.🐈.🐠.🌱.☀️.❄️.🎄.🎂.⚽.🧸`.split(`.`);function ao(e,t){let n=document.createElement(`div`);n.style.cssText=`position:fixed;inset:0;background:rgba(0,0,0,0.45);z-index:9999;display:flex;align-items:center;justify-content:center`,n.innerHTML=`
  ...
  `,document.body.appendChild(n),n.querySelector(`#emoji-picker-close`).onclick=()=>document.body.removeChild(n),n.querySelectorAll(`[data-emoji]`).forEach(e=>{e.onclick=()=>{t(e.dataset.emoji),document.body.removeChild(n)}}),n.addEventListener(`click`,e=>{e.target===n&&document.body.removeChild(n)})}function oo(){window.openModal(`Add Category Group`,`
    <div class="form-row">
  ...
  `,()=>{let e=document.getElementById(`f-grp-icon`).value||`📦`,t=document.getElementById(`f-grp-name`).value.trim();if(!t)return;let n=k(s.categoryGroups);s.categoryGroups.push({id:n,name:t,icon:e,categories:[]}),window.logActivity(`Added category group`,t),window.saveData(s),window.closeModal(),renderSettings()}),document.getElementById(`f-grp-icon-btn`).addEventListener(`click`,()=>{let e=document.getElementById(`f-grp-icon`).value;ao(e,e=>{document.getElementById(`f-grp-icon`).value=e,document.getElementById(`f-grp-icon-btn`).textContent=e})})}function so(e){let t=s.categoryGroups.find(t=>t.id===e);t&&ao(t.icon,t=>{lo(e,`icon`,t);let n=document.getElementById(`grp-icon-btn-${e}`);n&&(n.textContent=t)})}function co(e){let t=s.categoryGroups.find(t=>t.id===e);confirm(`Delete group "${t?t.name:``}"? Categories will become unassigned.`)&&(s.categoryGroups=s.categoryGroups.filter(t=>t.id!==e),window.logActivity(`Deleted category group`,t?t.name:``),window.saveData(s),renderSettings())}function lo(e,t,n){let r=s.categoryGroups.find(t=>t.id===e);r&&(r[t]=n,window.saveData(s))}function uo(e,t){if(!t)return;ea().includes(t)||(s.expenseCategories||(s.expenseCategories=ea().slice()),s.expenseCategories.push(t)),s.categoryGroups.forEach(e=>{e.categories=e.categories.filter(e=>e!==t)});let n=s.categoryGroups.find(t=>t.id===e);n&&n.categories.push(t),window.saveData(s),renderSettings()}function fo(e){let t=ea(),n=new Set((s.categoryGroups||[]).filter(t=>t.id!==e).flatMap(e=>e.categories)),r=s.categoryGroups.find(t=>t.id===e),i=new Set(r?r.categories:[]),a=t.filter(e=>!i.has(e)&&!n.has(e));window.openModal(`Add Category to Group`,`
    ${a.length>0?`
  ...
  `,()=>{let t=document.getElementById(`f-cat-selected`).value,n=document.getElementById(`f-cat-new`).value.trim()||t;n&&(uo(e,n),window.closeModal())})}function po(e,t){let n=s.categoryGroups.find(t=>t.id===e);n&&(n.categories=n.categories.filter(e=>e!==t),window.saveData(s),renderSettings())}var mo=!1,ho=null;function go(){mo||(ho=JSON.parse(JSON.stringify(s))),mo=!0;let e=document.getElementById(`settings-save-bar`);e&&(e.style.display=`flex`)}function _o(){mo=!1,ho=null,window.saveData(s);let e=document.getElementById(`settings-save-bar`);e&&(e.style.display=`none`);let t=document.getElementById(`settings-save-btn`);if(t){let e=t.textContent;t.textContent=`Saved`,t.style.background=`#10b981`,setTimeout(()=>{t.textContent=e,t.style.background=``},1500)}window.renderAll()}function vo(){ho&&Object.assign(s,ho),mo=!1,ho=null;let e=document.getElementById(`settings-save-bar`);e&&(e.style.display=`none`),renderSettings()}function yo(e){mo&&(confirm(`You have unsaved changes in Settings. Save before leaving?`)?_o():vo())}function bo(e,t){s.settings||(s.settings={}),s.settings[e]=t,go()}function xo(){let e=document.getElementById(`settings-api-key`)?.value.trim();if(!e)return;Xe(`toto_ai_key`,e),Xe(`toto_ai_key_meta`,JSON.stringify({addedAt:new Date().toISOString(),prefix:e.slice(0,10),suffix:e.slice(-4)}));let t=document.getElementById(`api-key-status`);t&&(t.textContent=`✓ Key saved!`,t.style.color=`var(--success)`,setTimeout(()=>{t.textContent=``,t.style.color=``},2e3));let n=document.getElementById(`api-key-summary`);n&&(n.outerHTML=Co())}function So(){confirm(`Remove saved API key? Toto and cost estimation will stop working.`)&&(Ze(`toto_ai_key`),Ze(`toto_ai_key_meta`),renderSettings())}function Co(){let e=N(`toto_ai_key`);if(!e)return`<div id="api-key-summary"></div>`;let t=JSON.parse(N(`toto_ai_key_meta`)||`{}`),n=t.addedAt?new Date(t.addedAt).toLocaleDateString(`en-AU`,{day:`numeric`,month:`short`,year:`numeric`}):`Unknown date`;return`
    ...
    </div>`;let p=ko({income:e,expenses:t});if(p.segments.length){let e=p.segments.map(e=>`<div style="background:${e.color};flex:${e.pct.toFixed(2)}"></div>`).join(``);p.segments.map(e=>`<div class="alloc-row">
        <div class="tdot" style="background:${e.color}"><svg viewBox="0 0 24 24"><use href="#${Mo(e.name)}"/></svg></div>
        <div class="body">
          <div class="ticker">${jo(e.name)}</div>
  ...
  `;let m=[`all`,...Array.from(new Set(t.map(e=>e.category||`Other`))).sort()],h=window.expenseFilterCat===`all`?t:t.filter(e=>(e.category||`Other`)===window.expenseFilterCat),_=h.reduce((e,t)=>e+D(t),0),v=h.reduce((e,t)=>e+window.getActual(t.id,window.selectedBudgetMonth),0),y=_-v,S=window.expenseFilterCat!==`all`;f+=`
    <div class="section">
      ...
      <div style="padding:16px 20px">
      ${window.budgetViewMode===`grouped`?Oo(t):`
  ...
  `,f+=`</div></div>`,document.getElementById(`budget-content`).innerHTML=f}catch(e){console.error(`renderBudget error:`,e);let t=document.getElementById(`budget-content`);t&&(t.innerHTML=`<div style="padding:24px;color:var(--alert);font-family:var(--mono);font-size:13px">Render error: ${g(e.message)}<br><small>${g(e.stack||``)}</small></div>`)}}var Po=!1;function Fo(){Po=!Po;let e=document.getElementById(`budget-detail`),t=document.getElementById(`budget-expand-label`),n=document.getElementById(`budget-expand-chevron`),r=e&&e.parentElement;e&&(e.classList.toggle(`collapsed`,!Po),e.classList.toggle(`expanded`,Po)),r&&(r.style.marginBottom=Po?`16px`:`0`),t&&(t.textContent=Po?`Hide details ▲`:`See breakdown ▼`),n&&(n.textContent=Po?`▲`:`▼`)}function R(e,t,n){document.getElementById(`modal-title`).textContent=e,document.getElementById(`modal-body`).innerHTML=t,document.getElementById(`modal-footer`).innerHTML=`
    <button class="btn btn-ghost" onclick="closeModal()">Cancel</button>
    <button class="btn btn-primary" id="modal-save-btn">Save</button>
  `,window._modalSaveHandler=n,document.getElementById(`modal-save-btn`).onclick=()=>window._modalSaveHandler?.(),document.getElementById(`modal-overlay`).classList.remove(`hidden`)}function Io(){window._pendingLogEntry=null,window._actualEditorRefresh=null,window._csvSuggestions=null,window._csvSuggestNames=null,document.getElementById(`modal-body`).innerHTML=``,document.getElementById(`modal-footer`).innerHTML=``,document.getElementById(`modal-overlay`).classList.add(`hidden`)}document.getElementById(`modal-overlay`).addEventListener(`click`,e=>{e.target===document.getElementById(`modal-overlay`)&&Io()});function Lo(){R(`Edit Contract Total`,`
    <div class="form-group">
  ...
  `,()=>{let e=parseFloat(document.getElementById(`f-contract-total`).value);!isNaN(e)&&e>0&&(window.logActivity(`Updated contract total`,b(e)),s.buildContract.total=e,window.saveData(s),Io(),window.renderAll())})}function Ro(e={}){return`
    <div class="form-row">
      <div class="form-group">
  ...
  `}function zo(e){return{id:e,name:document.getElementById(`f-stage-name`).value.trim(),amount:parseFloat(document.getElementById(`f-stage-amount`).value)||0,paid:document.getElementById(`f-stage-paid`).checked,expectedDate:document.getElementById(`f-stage-expected`).value,paidDate:document.getElementById(`f-stage-paiddate`).value,invoiceRef:document.getElementById(`f-stage-ref`).value.trim(),funding:document.getElementById(`f-stage-funding`).value,notes:document.getElementById(`f-stage-notes`).value.trim()}}function Bo(){R(`Add Contract Stage`,Ro(),()=>{let e=zo(k(s.buildContract.stages));e.name&&(window.logActivity(`Added build stage`,e.name),s.buildContract.stages.push(e),window.saveData(s),Io(),window.renderAll())})}function Vo(e){let t=s.buildContract.stages.find(t=>t.id===e);R(`Edit Stage`,Ro(t),()=>{let n=zo(e);window.logActivity(`Edited build stage`,n.name||t.name),Object.assign(t,n),window.saveData(s),Io(),window.renderAll()})}function Ho(e){if(!confirm(`Delete this stage?`))return;let t=s.buildContract.stages.find(t=>t.id===e);window.logActivity(`Deleted build stage`,t?t.name:``),s.buildContract.stages=s.buildContract.stages.filter(t=>t.id!==e),window.saveData(s),window.renderAll()}function Uo(e={}){return`
    <div class="form-row">
      <div class="form-group">
  ...
  `}function Wo(e){return{id:e,ref:document.getElementById(`f-var-ref`).value.trim(),name:document.getElementById(`f-var-name`).value.trim(),amount:parseFloat(document.getElementById(`f-var-amount`).value)||0,status:document.getElementById(`f-var-status`).value,funding:document.getElementById(`f-var-funding`).value,dateRaised:document.getElementById(`f-var-raised`).value,dateApproved:document.getElementById(`f-var-approved`).value,notes:document.getElementById(`f-var-notes`).value.trim()}}function Go(){R(`Add Variation`,Uo(),()=>{let e=Wo(k(s.buildContract.variations));e.name&&(window.logActivity(`Added variation`,`${e.ref?e.ref+` · `:``}${e.name}`),s.buildContract.variations.push(e),window.saveData(s),renderBuild())})}function Ko(e){R(`Edit Variation`,Uo(s.buildContract.variations.find(t=>t.id===e)),()=>{let t=Wo(e);if(!t.name)return;window.logActivity(`Edited variation`,`${t.ref?t.ref+` · `:``}${t.name}`);let n=s.buildContract.variations.findIndex(t=>t.id===e);n!==-1&&(s.buildContract.variations[n]=t),window.saveData(s),renderBuild()})}function qo(e){if(!confirm(`Delete this variation?`))return;let t=s.buildContract.variations.find(t=>t.id===e);window.logActivity(`Deleted variation`,t?t.name:``),s.buildContract.variations=s.buildContract.variations.filter(t=>t.id!==e),window.saveData(s),renderBuild()}var Jo=[{value:`not-quoted`,label:`Not Quoted`},{value:`quoted`,label:`Quoted`},{value:`approved`,label:`Approved`},{value:`partial`,label:`Partially Paid`},{value:`paid`,label:`Paid`}];function Yo(e={}){let t=e.status||(e.totalAmount?`approved`:`not-quoted`);return`
    <div class="form-row">
      <div class="form-group">
        <label class="form-label">Item Name</label>
  ...
  `}function Xo(e){return{id:e,name:document.getElementById(`f-extra-name`).value.trim(),vendor:document.getElementById(`f-extra-vendor`).value.trim(),status:document.getElementById(`f-extra-status`).value,funding:document.getElementById(`f-extra-funding`).value,totalAmount:parseFloat(document.getElementById(`f-extra-total`).value)||0,amountPaid:parseFloat(document.getElementById(`f-extra-paid`).value)||0,dueDate:document.getElementById(`f-extra-due`).value,notes:document.getElementById(`f-extra-notes`).value.trim()}}function Zo(){R(`Add Outside Contract Item`,Yo(),()=>{let e=Xo(k(s.extras));e.name&&(window.logActivity(`Added extra item`,e.name),s.extras.push(e),window.saveData(s),Io(),window.renderAll())})}function Qo(e){let t=s.extras.find(t=>t.id===e);R(`Edit Item`,Yo(t),()=>{let n=Xo(e);window.logActivity(`Edited extra item`,n.name||t.name),Object.assign(t,n),window.saveData(s),Io(),window.renderAll()})}function $o(e){if(!confirm(`Delete this item?`))return;let t=s.extras.find(t=>t.id===e);window.logActivity(`Deleted extra item`,t?t.name:``),s.extras=s.extras.filter(t=>t.id!==e),window.saveData(s),window.renderAll()}function es(e={}){let t=e.dueDate?(()=>{let[t,n,r]=e.dueDate.split(`-`);return`${r}/${n}/${t}`})():``,n=[`weekly`,`fortnightly`,`monthly`,`quarterly`,`annually`,`custom`],r=e.frequency===`custom`;return`
    <div class="form-group">
  ...
  `}function os(e){let t=document.getElementById(`f-exp-freq`)?document.getElementById(`f-exp-freq`).value:`monthly`,n=document.getElementById(`f-exp-recurring`),r={id:e,name:document.getElementById(`f-exp-name`).value.trim(),category:document.getElementById(`f-exp-cat`).value,vendor:(document.getElementById(`f-exp-vendor`)?.value||``).trim()||null,amount:parseFloat(document.getElementById(`f-exp-amount`).value)||0,frequency:t,dueDate:document.getElementById(`f-exp-duedate`).value||null,recurring:n?n.checked:!0};return t===`custom`&&(r.customEvery=parseInt(document.getElementById(`f-exp-custom-n`).value)||1,r.customUnit=document.getElementById(`f-exp-custom-unit`).value),r}function ss(e){let t=document.getElementById(`f-${e}-freq`).value,n=document.getElementById(`f-${e}-custom-wrap`);n&&(n.style.display=t===`custom`?`flex`:`none`)}function cs(){R(`Add Expense`,as(),()=>{let e=os(k(window.getMonthData(window.selectedBudgetMonth).expenses));e.name&&(window.logActivity(`Added expense`,`${e.name} (${e.category||`Other`})`),window.confirmScope(()=>{let t=window.ensureMonthOverride(window.selectedBudgetMonth);e.id=k(t.expenses),t.expenses.push(e),window.saveData(s),window.renderAll()},()=>{if(e.id=k(s.budget.expenses),s.budget.expenses.push(e),window.isMonthCustomized(window.selectedBudgetMonth)){let t=s.budget.months[window.selectedBudgetMonth];t.expenses.push({...e,id:k(t.expenses)})}window.saveData(s),window.renderAll()}))})}function ls(e){let t=window.getMonthData(window.selectedBudgetMonth).expenses.find(t=>t.id===e);R(`Edit Expense`,as(t),()=>{let n=os(e);window.logActivity(`Edited expense`,`${n.name||t&&t.name||``} (${n.category||`Other`})`),window.confirmScope(()=>{let t=window.ensureMonthOverride(window.selectedBudgetMonth),r=t.expenses.find(t=>t.id===e);r?Object.assign(r,n):t.expenses.push(n),window.saveData(s),window.renderAll()},()=>{let t=s.budget.expenses.find(t=>t.id===e);if(t&&Object.assign(t,n),window.isMonthCustomized(window.selectedBudgetMonth)){let t=s.budget.months[window.selectedBudgetMonth].expenses.find(t=>t.id===e);t&&Object.assign(t,n)}window.saveData(s),window.renderAll()})});let n=document.createElement(`button`);n.className=`btn btn-danger`,n.textContent=`Delete`,n.style.marginRight=`auto`,n.onclick=()=>{Io(),us(e)};let r=document.getElementById(`modal-footer`);r.insertBefore(n,r.firstChild)}function us(e){let t=window.getMonthData(window.selectedBudgetMonth).expenses.find(t=>t.id===e),n=t?t.name:`this expense`;window.logActivity(`Deleted expense`,n),window._scopePending={onThisMonth:()=>{let t=window.ensureMonthOverride(window.selectedBudgetMonth);t.expenses=t.expenses.filter(t=>t.id!==e),window.saveData(s),window.renderAll()},onAllMonths:()=>{s.budget.expenses=s.budget.expenses.filter(t=>t.id!==e),s.budget.months&&Object.values(s.budget.months).forEach(t=>{t.expenses=t.expenses.filter(t=>t.id!==e)}),window.saveData(s),window.renderAll()}},document.getElementById(`modal-title`).textContent=`Delete expense`,document.getElementById(`modal-body`).innerHTML=`
    ...
    <button class="btn btn-secondary" onclick="window.doScopeAll()">Remove from all months</button>
    <button class="btn btn-danger" onclick="window.doScopeMonth()">Delete from ${window.monthLabel(window.selectedBudgetMonth)}</button>
  `,document.getElementById(`modal-overlay`).classList.remove(`hidden`)}function ds(){return document.getElementById(`f-exp-duedate`)||document.getElementById(`f-inc-duedate`)}function fs(e){e.stopPropagation();let t=document.getElementById(`dp-popup`);if(!t)return;let n=(ds()||{}).value||``;if(n)[window.dpViewYear,window.dpViewMonth]=n.split(`-`).map(Number);else{let e=new Date;window.dpViewYear=e.getFullYear(),window.dpViewMonth=e.getMonth()+1}t.classList.remove(`hidden`),ps();function r(e){let n=document.getElementById(`dp-wrap`);n&&!n.contains(e.target)?t.classList.add(`hidden`):document.addEventListener(`click`,r,{once:!0})}document.addEventListener(`click`,r,{once:!0})}function ps(){let e=document.getElementById(`dp-popup`);if(!e)return;let t=window.dpViewYear,n=window.dpViewMonth,r=new Date(t,n-1,1).getDay(),i=new Date(t,n,0).getDate(),a=new Date,o=(ds()||{}).value||``,s=`
    <div class="dp-nav">
  ...
  `;for(let e=0;e<r;e++)s+=`<div class="dp-day dp-other"></div>`;for(let e=1;e<=i;e++){let r=`${t}-${String(n).padStart(2,`0`)}-${String(e).padStart(2,`0`)}`,i=[`dp-day`,a.getFullYear()===t&&a.getMonth()+1===n&&a.getDate()===e?`dp-today`:``,o===r?`dp-selected`:``].filter(Boolean).join(` `);s+=`<div class="${i}" onclick="dpSelectDate('${r}',event)">${e}</div>`}s+=`</div>`,o&&(s+=`<div class="dp-clear"><button class="dp-clear-btn" onclick="dpClearDate(event)">Clear date</button></div>`),e.innerHTML=s}function ms(e){e.stopPropagation(),window.dpViewMonth===1?(window.dpViewMonth=12,window.dpViewYear--):window.dpViewMonth--,ps()}function hs(e){e.stopPropagation(),window.dpViewMonth===12?(window.dpViewMonth=1,window.dpViewYear++):window.dpViewMonth++,ps()}function gs(e,t){t&&t.stopPropagation();let n=ds();n&&(n.value=e);let[r,i,a]=e.split(`-`);document.getElementById(`dp-display`).textContent=`${a}/${i}/${r}`,document.getElementById(`dp-trigger`).classList.add(`has-value`),document.getElementById(`dp-popup`).classList.add(`hidden`);let o=document.getElementById(`dp-repeats-wrap`);o&&(o.style.display=``)}function _s(e){e.stopPropagation();let t=ds();t&&(t.value=``),document.getElementById(`dp-display`).textContent=`Select a date`,document.getElementById(`dp-trigger`).classList.remove(`has-value`),document.getElementById(`dp-popup`).classList.add(`hidden`);let n=document.getElementById(`dp-repeats-wrap`);n&&(n.style.display=`none`)}var z=[],B=[];function vs(){document.getElementById(`modal-title`).textContent=`Import Bank Transactions`,document.getElementById(`modal-body`).innerHTML=`
    ...
    <button class="btn" onclick="window.closeModal()">Cancel</button>
    ${e?`<button class="btn btn-primary" onclick="runCsvCategorise()">Categorise with AI →</button>`:`<button class="btn btn-primary" onclick="_renderCsvReview(null)">Assign Manually →</button>`}`}async function Ss(){let e=window._secureGet(`toto_ai_key`);if(!e){Cs(null);return}document.getElementById(`modal-body`).innerHTML=`
...
[{"idx":0,"expenseId":3},{"idx":1,"expenseId":-1,"suggest":"Dining Out"},{"idx":2,"expenseId":-1}]`;try{let t=await fetch(CLAUDE_API,{method:`POST`,headers:{"x-api-key":e,"anthropic-version":`2023-06-01`,"content-type":`application/json`},body:JSON.stringify({model:`claude-haiku-4-5-20251001`,max_tokens:4096,messages:[{role:`user`,content:o}]})});if(!t.ok)throw Error(`API error ${t.status}`);let n=(await t.json()).content[0].text.replace(/```[\w]*\n?/g,``).trim().match(/\[[\s\S]*\]/);if(!n)throw Error(`No JSON in response`);let r=JSON.parse(n[0]);Cs(i(r))}catch(e){document.getElementById(`modal-body`).innerHTML=`
    ...
    <div style="height:max(12px,env(safe-area-inset-bottom))"></div>`,requestAnimationFrame(()=>document.getElementById(`qah-text`)?.focus())}function Ms(e){As(),setTimeout(()=>{if(e===`event`)activateTab(`planner`),setTimeout(()=>openPlannerModal(null,new Date().toISOString().slice(0,10)),300);else if(e===`expense`){let e=window.getMonthData(window.selectedBudgetMonth).expenses,t=parseInt(window._secureGet(`toto_qa_last`)||`0`);_qaExpenseId=e.find(e=>e.id===t)?.id??e[0]?.id??null,_qaAmount=``,Fs(e),document.getElementById(`qa-overlay`).classList.add(`open`),requestAnimationFrame(()=>document.getElementById(`qa-sheet`).classList.add(`open`))}else e===`income`?(activateTab(`budget`),setTimeout(()=>openAddIncome(),300)):e===`bill`?(activateTab(`bills`),setTimeout(()=>openBillModal(),300)):e===`chore`?(activateTab(`kids`),setTimeout(()=>{typeof renderChoreMgmt==`function`&&renderChoreMgmt()},300)):e===`shopping`?(window._listsActiveType=`food`,window._listsView=`list`,activateTab(`lists`)):e===`ai`&&typeof toggleTotoAssistant==`function`&&toggleTotoAssistant()},320)}async function Ns(){let e=document.getElementById(`qah-text`)?.value.trim();if(!e){Ms(`ai`);return}let t=document.querySelector(`.qah-ai-send`);t&&(t.textContent=`…`,t.disabled=!0);let n=`Today is ${new Date().toISOString().slice(0,10)}. The user typed: "${e}"
...
- If genuinely unclear → unknown`;try{let t=s.settings?.claudeApiKey,r=(await(await fetch(CLAUDE_API,{method:`POST`,headers:{"x-api-key":t,"anthropic-version":`2023-06-01`,"content-type":`application/json`},body:JSON.stringify({model:`claude-haiku-4-5-20251001`,max_tokens:150,messages:[{role:`user`,content:n}]})})).json()).content?.[0]?.text?.trim()||`{"type":"unknown"}`,i=JSON.parse(r.replace(/```[\w]*\n?/g,``).trim());As(),await Ps(i,e)}catch{As(),setTimeout(()=>{typeof toggleTotoAssistant==`function`&&toggleTotoAssistant(),setTimeout(()=>{let t=document.getElementById(`toto-input`)||document.querySelector(`.toto-msg-input`);t&&(t.value=e,t.focus())},400)},320)}}async function Ps(e,t){let n=e=>new Promise(t=>setTimeout(t,e));if(e.type===`event`){activateTab(`planner`),await n(320),openPlannerModal(null,e.date||new Date().toISOString().slice(0,10)),await n(200);let r=document.getElementById(`pe-title`),i=document.getElementById(`pe-time`);if(r&&(r.value=e.title||t),i&&e.time&&(i.value=e.time),e.date){let t=document.getElementById(`pe-date`),n=document.getElementById(`pm-start-display`);t&&(t.value=e.date),n&&typeof _pmFmtDateShort==`function`&&(n.textContent=_pmFmtDateShort(e.date))}}else if(e.type===`expense`){let t=window.getMonthData(window.selectedBudgetMonth).expenses,n=parseInt(window._secureGet(`toto_qa_last`)||`0`);_qaExpenseId=t.find(e=>e.id===n)?.id??t[0]?.id??null,_qaAmount=e.amount?String(e.amount):``,Fs(t),document.getElementById(`qa-overlay`).classList.add(`open`),requestAnimationFrame(()=>{document.getElementById(`qa-sheet`).classList.add(`open`);let t=document.getElementById(`qa-note`);t&&e.note&&(t.value=e.note)})}else if(e.type===`income`){activateTab(`budget`),await n(320),openAddIncome(),await n(200);let t=document.getElementById(`inc-name`)||document.querySelector(`#modal-body [id*="name"]`),r=document.getElementById(`inc-amount`)||document.querySelector(`#modal-body [id*="amount"]`);t&&e.name&&(t.value=e.name),r&&e.amount&&(r.value=e.amount)}else if(e.type===`bill`){activateTab(`bills`),await n(320),openBillModal(),await n(200);let t=document.getElementById(`bill-name`),r=document.getElementById(`bill-amount`),i=document.getElementById(`bill-due`);t&&e.name&&(t.value=e.name),r&&e.amount&&(r.value=e.amount),i&&e.dueDate&&(i.value=e.dueDate)}else if(e.type===`chore`)activateTab(`kids`),await n(320);else if(e.type===`shopping`){window._listsActiveType=`food`,window._listsView=`list`,activateTab(`lists`),await n(320);let t=document.getElementById(`ls-quick-input`);t&&e.name&&(t.value=e.name,t.focus())}else{typeof toggleTotoAssistant==`function`&&toggleTotoAssistant(),await n(400);let e=document.getElementById(`toto-input`)||document.querySelector(`.toto-msg-input`);e&&(e.value=t,e.focus())}}function Fs(e){let t=e||window.getMonthData(window.selectedBudgetMonth).expenses,n=_qaAmount?`$${_qaAmount}`:`$0`,r=!_qaAmount,i=t.length?t.map(e=>`<button class="qa-cat${e.id===_qaExpenseId?` selected`:``}" onclick="_qaSelectCat(${e.id})">${g(e.name)}</button>`).join(``):`<span style="color:var(--text-muted);font-size:13px;padding:6px 4px">Add budget expenses first</span>`,a=[`1`,`2`,`3`,`4`,`5`,`6`,`7`,`8`,`9`,`.`,`0`,`⌫`];document.getElementById(`qa-sheet`).innerHTML=`
    ...
    </div>`}function Is(e){if(e===`⌫`)_qaAmount=_qaAmount.slice(0,-1);else if(e===`.`)_qaAmount.includes(`.`)||(_qaAmount+=(_qaAmount?``:`0`)+`.`);else{let t=_qaAmount.split(`.`);if(t[1]!==void 0&&t[1].length>=2||_qaAmount.replace(`.`,``).length>=6)return;_qaAmount===`0`&&e!==`.`?_qaAmount=e:_qaAmount+=e}let t=document.getElementById(`qa-display`);if(!t)return;let n=!_qaAmount;t.textContent=_qaAmount?`$${_qaAmount}`:`$0`,t.className=`qa-amount-display${n?` zero`:``}`}function Ls(e){_qaExpenseId=e,document.querySelectorAll(`.qa-cat`).forEach(t=>{t.classList.toggle(`selected`,parseInt(t.getAttribute(`onclick`).match(/\d+/)[0])===e)})}function Rs(){let e=parseFloat(_qaAmount);if(!e||e<=0){let e=document.getElementById(`qa-display`);e&&(e.style.color=`var(--danger)`,setTimeout(()=>e.style.color=``,600));return}if(!_qaExpenseId){let e=document.getElementById(`qa-cats`);e&&(e.style.outline=`2px solid var(--danger)`,e.style.borderRadius=`8px`,setTimeout(()=>{e.style.outline=``},600));return}let t=document.getElementById(`qa-note`)?.value.trim()||``,n=new Date().toISOString().slice(0,10);s.budget.actuals[window.selectedBudgetMonth]||(s.budget.actuals[window.selectedBudgetMonth]={});let r=window.getActualEntries(_qaExpenseId,window.selectedBudgetMonth),i=r.length?Math.max(...r.map(e=>e.id))+1:1;r.push({id:i,amount:e,date:n,note:t}),s.budget.actuals[window.selectedBudgetMonth][_qaExpenseId]=r,window._secureSet(`toto_qa_last`,String(_qaExpenseId)),window.saveData(s),As(),window.renderAll();let a=document.getElementById(`qa-fab`);a&&(a.textContent=`✓`,a.style.background=`#10b981`,setTimeout(()=>{a.textContent=`+`,a.style.background=``},1800))}window.addEventListener(`resize`,()=>{document.querySelectorAll(`.section-pills-wrap`).forEach(_updatePillsOverflow)}),document.querySelectorAll(`.nav-item, .nav-text-item`).forEach(e=>{e.addEventListener(`click`,()=>activateTab(e.dataset.tab))});function zs(){let e=window._activeProfile?.role===`child`;document.body.classList.toggle(`kid-mode`,e);let t=document.getElementById(`kid-banner-label`);t&&e&&(t.textContent=`${window._activeProfile.emoji||`😊`} ${window._activeProfile.name}'s view`);let n=window.getDeviceProfile(),r=document.getElementById(`header-switch-profile`);if(r){let t=n&&n!==`adult`;if(r.style.display=t?``:`none`,t)if(e)r.textContent=`👨‍👩‍👧 Parent`;else{let e=(s.kids?.profiles||[]).find(e=>e.id===n);r.textContent=e?`Back to ${e.name}`:`Switch`}}}function Bs(){let e=window.getDeviceProfile();if(window._activeProfile?.role===`child`)window.clearKidSession(),window._activeProfile=null,zs(),window.renderAll();else if(e===`shared`)window.showProfileSelector();else if(e&&e!==`adult`){let t=(s.kids?.profiles||[]).find(t=>t.id===e);t&&(t.pinHash?(window._pinTargetId=t.id,window._pinBuffer=``,window._pinAttempts=0,window._showPinScreen(t)):(window._activeProfile={id:t.id,name:t.name,emoji:t.emoji,role:`child`},window.setKidSession(t.id),window._applyActiveProfile()))}}async function Vs(e,t){let n=(s.kids?.profiles||[]).find(t=>String(t.id)===String(e));!n||t.length!==4||(n.pinHash=await window._hashPin(t,window._getHouseholdOwnerUID()),window.saveData(s))}function Hs(e){let t=(s.kids?.profiles||[]).find(t=>String(t.id)===String(e));t&&(delete t.pinHash,window.saveData(s))}function Us(e){(s.householdProfile?.members||[]).filter(e=>e.role===`adult`&&e.name)[e]&&(Gs=e,qs=``,V=``,Ks=`enter`,Js(),document.getElementById(`adult-pin-modal`).classList.remove(`hidden`))}function Ws(e){let t=(s.householdProfile?.members||[]).filter(e=>e.role===`adult`&&e.name);t[e]&&(delete t[e].pinHash,window.saveData(s),renderSettings())}var Gs=0,Ks=`enter`,qs=``,V=``;function Js(){let e=(s.householdProfile?.members||[]).filter(e=>e.role===`adult`&&e.name)[Gs];if(!e)return;let t=Ks===`enter`,n=t?e.pinHash?`Change your PIN 🔢`:`Set your PIN 🔢`:`Confirm your PIN ✅`,r=t?`Pick 4 numbers — used on shared devices`:`Enter it again to confirm`,i=[0,1,2,3].map(e=>{let t=e<V.length;return`<div style="width:52px;height:60px;border:2px solid ${t?`#0891b2`:`#e2e8f0`};border-radius:10px;background:${t?`#ecfeff`:`#f8fafc`};display:flex;align-items:center;justify-content:center;font-size:24px;font-weight:700;color:#0891b2">${t?`●`:``}</div>`}).join(``),a=[1,2,3,4,5,6,7,8,9,``,0,`⌫`].map(e=>e===``?`<div></div>`:`<div onclick="_adultPinKey('${e}')" style="height:52px;border:1.5px solid #e2e8f0;border-radius:10px;display:flex;align-items:center;justify-content:center;font-size:20px;font-weight:600;color:#374151;cursor:pointer;background:#fff;-webkit-tap-highlight-color:transparent;user-select:none">${e}</div>`).join(``);document.getElementById(`adult-pin-modal-body`).innerHTML=`
    ...
    <button onclick="document.getElementById('adult-pin-modal').classList.add('hidden')" style="font-size:13px;color:#94a3b8;background:none;border:none;cursor:pointer">Cancel</button>`}function Ys(e){if(e===`⌫`){V=V.slice(0,-1),Js();return}V.length>=4||(V+=e,Js(),V.length===4&&Xs())}async function Xs(){if(Ks===`enter`)qs=V,V=``,Ks=`confirm`,Js();else{if(V!==qs){V=``,qs=``,Ks=`enter`,Js();let e=document.getElementById(`adult-pin-error`);e&&(e.textContent=`Those didn't match — try again`);return}let e=(s.householdProfile?.members||[]).filter(e=>e.role===`adult`&&e.name);e[Gs].pinHash=await window._hashPin(V,window._getHouseholdOwnerUID()),window.saveData(s),document.getElementById(`adult-pin-modal`).classList.add(`hidden`),renderSettings()}}function Zs(e){let t=Number(e.age||0);return t<5?`tiny-tots`:t<8?`early-reader`:t<12?`independent`:`tween`}function Qs(){let e=new Date().getHours();return e<12?`Good morning`:e<17?`Good afternoon`:`Good evening`}function $s(e){if(!e.triggerTime)return!0;let[t,n]=e.triggerTime.split(`:`).map(Number),r=new Date,i=r.getHours()*60+r.getMinutes(),a=t*60+(n||0),o=a+360;return i>=a&&i<o}function ec(e){if(!e.triggerTime)return``;let[t]=e.triggerTime.split(`:`).map(Number);return t<12?`Available this morning`:t<17?`Available this afternoon`:`Available tonight`}function tc(){let e=document.getElementById(`child-view-overlay`);if(!e)return;let t=document.getElementById(`cv-confetti-wrap`);t&&t.remove(),t=document.createElement(`div`),t.id=`cv-confetti-wrap`,t.className=`cv2-confetti-wrap`,e.appendChild(t);let n=[`#5B4CF5`,`#7C3AED`,`#F59E0B`,`#10B981`,`#F43F5E`,`#FBBF24`];for(let e=0;e<60;e++){let r=document.createElement(`div`);r.className=`cv2-confetti-particle`,r.style.cssText=`
    ...
    </div>`}let{events:k}=dc(t,p),A=[...k].sort((e,t)=>(e.time||`99:99`).localeCompare(t.time||`99:99`)),ne=``;A.length&&!a&&(ne=`<div class="cv2-group">
      <div class="cv2-group-heading">📅 Today's Events</div>
      <div class="cv2-card">${A.map(e=>`
      <div class="cv2-event-row">
        <span class="cv2-event-time">${e.time?fc(e.time):``}</span>
    ...
    ${ne}${O}${ie}`:(M=ae,i!==`tiny-tots`&&!c&&(M+=`<button class="cv2-week-shortcut" onclick="_cvSwitchTab('calendar','${t.id}')">📅 See my week →</button>`),m.length&&(M+=`<div class="cv2-group">
      ...
      </div>`),M+=ne,M+=O,M+=ie),document.getElementById(`cv-content`).innerHTML=M,l.classList.remove(`hidden`),l.style.display=`flex`,oe&&!c&&tc()}function rc(e){let t=document.getElementById(`cv-prizes-badge`);if(!t)return;let n=s.kids,r=kidBalance(n,e.id),i=(n.notifications||[]).filter(t=>t.kidId===e.id&&!t.read).length,a=(n.prizes||[]).filter(e=>r>=e.pointCost).length,o=i+(a>0&&i===0?a:0);o>0?(t.textContent=o>9?`9+`:String(o),t.style.display=``):t.style.display=`none`}function ic(e){let t=s.kids,n=kidBalance(t,e.id),r=Zs(e)===`tiny-tots`,i=_cvReadOnly,a=t.prizes||[],o=!1;(t.notifications||[]).filter(t=>t.kidId===e.id&&!t.read).forEach(e=>{e.read=!0,o=!0}),o&&window.saveData(s),rc(e);let c=r?`${`⭐`.repeat(Math.min(n,10))}${n>10?`+`:``}`:`${n}`,l=`<div class="cv2-prizes-balance">
    ...
    <span class="cv2-prizes-balance-emoji">🏆</span>
      ...
      </div>`}),l+=`</div>`);let d=document.getElementById(`cv-content`);d&&(d.innerHTML=l)}var ac=null,oc=`today`,sc=`7day`,cc=null,lc=new Set;function uc(e,t){t&&(ac=t);let n=(s.kids?.profiles||[]).find(e=>String(e.id)===String(ac));if(!n)return;let r=Zs(n),i=document.getElementById(`cv-nav`);i&&(i.style.display=r===`tiny-tots`?`none`:``),document.getElementById(`cv-nav-today`)?.classList.toggle(`active`,e===`today`),document.getElementById(`cv-nav-calendar`)?.classList.toggle(`active`,e===`calendar`),document.getElementById(`cv-nav-prizes`)?.classList.toggle(`active`,e===`prizes`),e===`today`?nc(ac):e===`prizes`?ic(n):(cc=null,vc(n))}function dc(e,t){let n=(s.routineAssignments||[]).filter(t=>t.childId===e.id).map(e=>{let n=(s.routines||[]).find(t=>t.id===e.routineId);return n&&_routineMatchesDate(n,t)?{type:`routine`,routine:n,assignment:e,label:n.name,emoji:n.emoji,color:`#7C3AED`,tag:`Routine`,time:n.triggerTime||null}:null}).filter(Boolean),r=(s.childEvents||[]).filter(n=>{let r=Array.isArray(n.assignedTo)?n.assignedTo:[n.assignedTo];return r.includes(e.id)||r.includes(`all`)||n.isHouseholdWide?n.recurrence?Et(n.recurrence,t):n.date===t:!1}).map(e=>({type:`event`,label:e.title,emoji:e.emoji||`📅`,color:`#10b981`,tag:`Event`,notes:e.notes,time:e.time||null})),i=(s.planner?.events||[]).filter(e=>Rt(e).includes(`everyone`)?e.recurrence&&e.recurrence.type!==`one_time`?Et(e.recurrence,t):e.endDate&&e.endDate>e.date?t>=e.date&&t<=e.endDate:e.date===t:!1).map(e=>({type:`event`,label:e.title,emoji:P[e.category]?.emoji||`📅`,color:`#10b981`,tag:`Event`,notes:e.notes||``,time:e.time||null}));return{routines:n,events:[...r,...i],chores:(s.kids?.chores||[]).filter(t=>(t.assignedTo===e.id||t.assignedTo===`all`)&&!t._isRoutine).map(e=>({type:`chore`,label:e.name,emoji:e.emoji||`📋`,color:`#ec4899`,tag:`Chore`,time:null}))}}function fc(e){if(!e)return``;let[t,n]=e.split(`:`).map(Number);return`${t%12||12}:${String(n).padStart(2,`0`)} ${t>=12?`pm`:`am`}`}function pc(e){lc.has(e)?lc.delete(e):lc.add(e);let t=(s.kids?.profiles||[]).find(e=>String(e.id)===String(ac));t&&(sc===`7day`||Zs(t)===`early-reader`?hc(t,new Date().toISOString().slice(0,10)):hc(t,cc))}function mc(e,t,n){if(_cvReadOnly)return;let r=_routineGetAssignment(e,ac);if(!r)return;r.completionState||(r.completionState={});let i=n;r.completionState[i]||(r.completionState[i]=[]);let a=r.completionState[i].indexOf(t),o=a===-1;o?r.completionState[i].push(t):r.completionState[i].splice(a,1);let c=new Date().toISOString().slice(0,10);if(o&&n===c){let n=(s.routines||[]).find(t=>String(t.id)===String(e));if(n){let e=n.steps.find(e=>e.id===t);(e?.points||0)>0&&_routineAwardStepPoints(n,e,ac);let a=n.steps.length;r.completionState[i].length===a&&a>0&&(n.pointsPerCompletion||0)>0&&_routineAwardPoints(n,ac)}}window.saveData(s);let l=(s.kids?.profiles||[]).find(e=>String(e.id)===String(ac));l&&hc(l,n)}function hc(e,t){let n=sc===`month`?`cv-day-panel`:`cv-schedule-panel`,r=document.getElementById(n);r&&(r.innerHTML=gc(e,t))}function gc(e,t){if(!t)return``;let{routines:n,events:r,chores:i}=dc(e,t),a=t;if(!n.length&&!r.length&&!i.length)return`<div style="text-align:center;padding:28px 0;color:#A1A1AA;font-size:13px">Nothing scheduled</div>`;let o=[...n.map(e=>({...e,sortKey:e.time||`23:59`})),...r.map(e=>({...e,sortKey:e.time||`23:59`}))].sort((e,t)=>e.sortKey.localeCompare(t.sortKey)),c=``;return o.length&&(c+=`<div class="cv-sched-section-hdr">Schedule</div>`,o.forEach(e=>{e.type===`routine`?c+=_c(e.routine,e.assignment,a):c+=`<div class="cv-sched-item">
          <div class="cv-sched-row">
            <span class="cv-sched-time">${e.time?fc(e.time):``}</span>
      ...
      </div>`})),c}function _c(e,t,n){let r=t.completionState?.[n]||[],i=e.steps.length,a=i>0?Math.round(r.length/i*100):0,o=r.length===i&&i>0,s=lc.has(e.id),c=i>0?`
    ...
    <div class="cv-sched-row" style="cursor:pointer" onclick="_cvToggleRoutineExpand(${JSON.stringify(e.id)})">
      <span class="cv-sched-time">${e.triggerTime?fc(e.triggerTime):``}</span>
      <div class="cv-sched-color-bar" style="background:#7C3AED"></div>
      <span class="cv-sched-emoji">${e.emoji}</span>
      <div class="cv-sched-body">
        <div class="cv-sched-title">${g(e.name)}</div>
        ${o?`<div class="cv-sched-sub" style="color:#5B4CF5;font-weight:700">✓ Complete</div>`:e.triggerTime?`<div class="cv-sched-sub">${fc(e.triggerTime)}</div>`:``}
      </div>
      ${c}
      <button class="cv-sched-expand-btn">${s?`▲`:`▼`}</button>
    </div>
    ${l}
  </div>`}function vc(e){if(!e)return;let t=Zs(e)===`early-reader`,n=t?`7day`:sc,r=new Date().toISOString().slice(0,10),i=document.getElementById(`cv-content`);if(!i)return;let a=``;t||(a+=`<div style="display:flex;gap:8px;margin-bottom:14px">
    ...
    </div>`),a+=n===`7day`?bc(e,r):Sc(e,r),i.innerHTML=a}function yc(e,t){sc=e,cc=null;let n=(s.kids?.profiles||[]).find(e=>String(e.id)===String(t));n&&vc(n)}function bc(e,t){let n=[`S`,`M`,`T`,`W`,`T`,`F`,`S`],r=new Date(t+`T12:00:00`),i=`<div class="cv-week-strip">`;for(let a=0;a<7;a++){let o=new Date(r);o.setDate(r.getDate()+a);let s=o.toISOString().slice(0,10),c=s===t,{routines:l,events:u,chores:d}=dc(e,s),f=[...l,...u,...d].slice(0,3).map(e=>`<div class="cv-cal-dot" style="background:${e.color}"></div>`).join(``);i+=`<div class="cv-week-cell ${c?`cv-today`:``}" onclick="_cvWeekDayTap('${s}','${e.id}')">
    ...
    <div id="cv-day-panel">${gc(e,l)}</div>
  </div>`,u}function Cc(e,t){cc=e,lc.clear();let n=(s.kids?.profiles||[]).find(e=>String(e.id)===String(t));n&&vc(n)}function wc(e,t){Cc(e,t)}function Tc(e,t){let n=(s.kids?.notifications||[]).find(t=>t.id===e);n&&(n.read=!0),window.saveData(s),nc(t)}function Ec(e,t){let n=(s.kids?.prizes||[]).find(e=>String(e.id)===String(t));if(!n)return;let r=`<div class="cv2-confirm">
    ...
    <div style="display:flex;align-items:center;justify-content:space-between;gap:12px;padding:12px 0;border-bottom:1px solid var(--hairline-soft)">
      ...
      </div>`;return}(!Bc||!n.find(e=>e.id===Bc))&&(Bc=n[0].id);let r=n.find(e=>e.id===Bc),i=_mealWeekKey(0),a=_mealWeekDates(i).slice(0,5),o=(t.plans[i]||{})[Bc]||{},c=new Date().toISOString().slice(0,10),l=[`Mon`,`Tue`,`Wed`,`Thu`,`Fri`],u=n.map(e=>`<button class="lb-kid-tab${e.id===Bc?` active`:``}" onclick="_lbActiveKid=${e.id};renderLunchbox()">${g(e.name)}</button>`).join(``);a[0].toLocaleDateString(`en-AU`,{day:`numeric`,month:`short`}),a[4].toLocaleDateString(`en-AU`,{day:`numeric`,month:`short`});let d=``;Vc.forEach(e=>{d+=`<div class="lb-label">${e.label}</div>`,a.forEach((t,n)=>{let r=(o[n]||{})[e.key]||``,a=t.toISOString().slice(0,10)===c;d+=`<div class="lb-cell${a?` today`:``}" onclick="openLunchboxEdit('${i}',${Bc},${n},'${e.key}')">
    ...
    </div>`}function Wc(e){let t=e?(s.meals.lunchbox.profiles||[]).find(t=>t.id===e):null;document.getElementById(`modal-title`).textContent=t?`Edit ${g(t.name)}`:`Add Child`,document.getElementById(`modal-body`).innerHTML=`
    <div class="form-group">
    ...
    <div class="form-group">
      ...
      <div id="lb-allergies" style="display:flex;flex-wrap:wrap;gap:6px">
        ${Hc.map(e=>{let n=t&&(t.allergies||[]).includes(e);return`<button type="button" class="lb-tag${n?` allergy`:``}" style="cursor:pointer;${n?``:`background:var(--surface2);color:var(--text-muted);border:1px solid var(--border)`}"
    ...
    <button class="btn btn-primary" onclick="_saveLbSlot('${e}',${t},${n},'${r}',document.getElementById('lb-input').value.trim())">Save</button>`,document.getElementById(`modal-overlay`).classList.remove(`hidden`),setTimeout(()=>{let e=document.getElementById(`lb-input`);e&&(e.focus(),e.select())},80)}function Jc(e,t,n,r,i){s.meals.lunchbox.plans[e]||(s.meals.lunchbox.plans[e]={}),s.meals.lunchbox.plans[e][t]||(s.meals.lunchbox.plans[e][t]={}),s.meals.lunchbox.plans[e][t][n]||(s.meals.lunchbox.plans[e][t][n]={}),s.meals.lunchbox.plans[e][t][n][r]=i,delete s.meals.lunchbox.plans[e][t][n][`cal_`+r],window.saveData(s),window.closeModal(),Uc(),i&&s.settings?.showCalories&&Yc(e,t,n,r,i)}async function Yc(e,t,n,r,i){let a=localStorage.getItem(`toto_ai_key`);if(!(!a||!i))try{let o=await fetch(CLAUDE_API,{method:`POST`,headers:{"x-api-key":a,"anthropic-version":`2023-06-01`,"content-type":`application/json`},body:JSON.stringify({model:`claude-haiku-4-5-20251001`,max_tokens:50,messages:[{role:`user`,content:`Estimate the calories in this school lunch item: "${i}". Return ONLY a number, nothing else. For example: 250`}]})});if(!o.ok)return;let c=await o.json(),l=parseInt(c.content[0].text.trim().replace(/[^0-9]/g,``));l>0&&l<3e3&&s.meals.lunchbox.plans[e]?.[t]?.[n]&&(s.meals.lunchbox.plans[e][t][n][`cal_`+r]=l,window.saveData(s),Uc())}catch{}}async function Xc(e,t){let n=localStorage.getItem(`toto_ai_key`);if(!n)return;let r=(s.meals.lunchbox.profiles||[]).find(e=>e.id===t);if(!r)return;let i=document.getElementById(`lb-ai-btn`);i&&(i.textContent=`Planning...`,i.disabled=!0);let a=`Plan 5 days (Monday to Friday) of school lunches for a child:
...
No markdown, no code fences, just raw JSON.`;try{let r=await fetch(CLAUDE_API,{method:`POST`,headers:{"x-api-key":n,"anthropic-version":`2023-06-01`,"content-type":`application/json`},body:JSON.stringify({model:`claude-haiku-4-5-20251001`,max_tokens:1024,messages:[{role:`user`,content:a}]})});if(!r.ok)throw Error(`API ${r.status}`);let i=(await r.json()).content[0].text.replace(/```[\w]*\n?/g,``).trim().match(/\[[\s\S]*\]/);if(!i)throw Error(`No JSON`);let o=JSON.parse(i[0]);s.meals.lunchbox.plans[e]||(s.meals.lunchbox.plans[e]={}),s.meals.lunchbox.plans[e][t]||(s.meals.lunchbox.plans[e][t]={}),o.forEach(n=>{let r={main:n.main||``,snack:n.snack||``,fruit:n.fruit||``,drink:n.drink||``};n.cal_main&&(r.cal_main=n.cal_main),n.cal_snack&&(r.cal_snack=n.cal_snack),n.cal_fruit&&(r.cal_fruit=n.cal_fruit),n.cal_drink&&(r.cal_drink=n.cal_drink),s.meals.lunchbox.plans[e][t][n.day]=r}),window.saveData(s),Uc()}catch(e){console.error(`Lunchbox AI error:`,e),i&&(i.textContent=`Plan this week`,i.disabled=!1)}}function Zc(e,t){let n=(s.meals.lunchbox.plans[e]||{})[t]||{},r=new Set;Object.values(n).forEach(e=>{e.main&&r.add(e.main),e.snack&&r.add(e.snack),e.fruit&&r.add(e.fruit)}),s.lists||(s.lists={}),s.lists.food||(s.lists.food={items:[],weeklyBudget:200,budget:0,stores:[],favourites:[],history:[]});let i=s.lists.food.items,a=0;r.forEach(e=>{i.some(t=>t.name.toLowerCase()===e.toLowerCase()&&t.state===`active`)||(i.push({id:`si-lb-`+Date.now()+`-`+a,name:e,quantity:1,unit:`units`,notes:``,aisle:_inferAisle?_inferAisle(e):`other`,state:`active`,addedBy:`lunchbox`,addedAt:new Date().toISOString(),mealTag:`Lunchbox`,manualPrice:null,barcodeId:null}),a++)}),a>0&&(window.saveData(s),_listsActiveType=`food`,_listsView=`list`,activateTab(`lists`));let o=document.querySelector(`[onclick^="lbToShoppingList"]`);if(o){let e=o.textContent;o.textContent=`Added ${a} items`,o.style.color=`var(--success)`,setTimeout(()=>{o.textContent=e,o.style.color=``},2e3)}}function Qc(e){let t=document.querySelector(`.health-popover`);if(t){t.remove();return}let n=window._lastHealth;if(!n)return;let r=window._lastHealthGuides||[`savings`,`tracking`,`netWorth`,`emergency`,`goals`],i={A:`#10b981`,B:`#0891b2`,C:`#f59e0b`,D:`#f97316`,F:`#ef4444`},a={A:`Excellent`,B:`Good`,C:`Fair — some areas need work`,D:`Needs attention`,F:`Critical — take action`},o={"Savings Rate":n.dimensions[0].score>=15?`Healthy savings rate`:n.dimensions[0].score>=8?`Could save more each month`:`Very little being saved`,"Budget Tracking":n.dimensions[1].score>=15?`Consistently logging actuals`:n.dimensions[1].score>=8?`Some months tracked`:`Not tracking actual spending`,"Net Worth":n.dimensions[2].score>=15?`Assets outweigh liabilities`:n.dimensions[2].score>=8?`Building slowly`:`Liabilities are high or no data`,"Emergency Buffer":n.dimensions[3].score>=15?`3+ months of expenses covered`:n.dimensions[3].score>=8?`Some buffer building`:`Less than 1 month covered`,Goals:n.dimensions[4].score>=15?`Goals set and progressing`:n.dimensions[4].score>=8?`Goals need more progress`:`No active goals set`},s=``;n.dimensions.forEach((e,t)=>{let n=Math.round(e.score/e.max*100),i=n>=75?`#10b981`:n>=50?`#f59e0b`:`#ef4444`,a=o[e.label]||``,c=r[t],l=e.score<15&&HEALTH_GUIDES[c];s+=`<div class="health-dim">
    ...
    ${s}`,document.body.appendChild(c);let l=e.getBoundingClientRect(),u=l.bottom+10,d=l.left+l.width/2-150;d<12&&(d=12),d+300>window.innerWidth-12&&(d=window.innerWidth-312),u+c.offsetHeight>window.innerHeight-12&&(u=l.top-c.offsetHeight-10),c.style.top=u+`px`,c.style.left=d+`px`,setTimeout(()=>{let t=n=>{!c.contains(n.target)&&n.target!==e&&(c.remove(),document.removeEventListener(`click`,t))};document.addEventListener(`click`,t)},10)}var $c=[],el=0,tl=null;function nl(e){$c=e,el=0,rl()}function rl(){if(sl(),el>=$c.length){ol();return}let e=$c[el];e.tab&&_activeTab()!==e.tab?(activateTab(e.tab),setTimeout(()=>il(e),500)):setTimeout(()=>il(e),100)}function il(e){let t=e.el?typeof e.el==`string`?document.querySelector(e.el):e.el:null,n=document.createElement(`div`);n.className=`guide-overlay`,n.onclick=()=>ol(),document.body.appendChild(n);let r=null,i=null;t&&(t.scrollIntoView({behavior:`smooth`,block:`center`}),i=t.getBoundingClientRect(),r=document.createElement(`div`),r.className=`guide-spotlight guide-pulse`,r.style.cssText=`top:${i.top-6}px;left:${i.left-6}px;width:${i.width+12}px;height:${i.height+12}px;`,document.body.appendChild(r));let a=document.createElement(`div`);if(a.className=`guide-tooltip`,a.innerHTML=`
    <div class="guide-tooltip-step">Step ${el+1} of ${$c.length}</div>
    ...
    <div class="guide-tooltip-actions">
      ...
      ${el<$c.length-1?`<button class="guide-tooltip-btn" style="background:#0891b2;color:#fff" onclick="nextGuideStep()">Next</button>`:`<button class="guide-tooltip-btn" style="background:#0891b2;color:#fff" onclick="endGuide()">Done</button>`}
    </div>`,a.onclick=e=>e.stopPropagation(),document.body.appendChild(a),i){let e=a.offsetWidth,t=a.offsetHeight,n=i.bottom+14,r=i.left+i.width/2-e/2;r<12&&(r=12),r+e>window.innerWidth-12&&(r=window.innerWidth-e-12),n+t>window.innerHeight-12&&(n=i.top-t-14),a.style.top=n+`px`,a.style.left=r+`px`}else a.style.top=`50%`,a.style.left=`50%`,a.style.transform=`translate(-50%, -50%)`;if(tl=()=>{n.remove(),r&&r.remove(),a.remove()},e.watchClick&&t){let e=()=>{t.removeEventListener(`click`,e),setTimeout(()=>al(),400)};t.addEventListener(`click`,e)}}function al(){el++,rl()}function ol(){sl(),$c=[],el=0}function sl(){tl&&(tl(),tl=null)}var cl={emergency:[{tab:`goals`,title:`Create an Emergency Fund`,text:`An emergency fund covers 3-6 months of expenses. Let's set one up as a savings goal.`,el:null},{tab:`goals`,title:`Tap "+ New Goal"`,text:`This button creates a new savings goal. Set the name to "Emergency Fund" and the target to your 3-month expenses.`,el:`[onclick*="openAddGoal"]`,watchClick:!0}],savings:[{tab:`budget`,title:`Improve your savings rate`,text:`Your savings rate is the gap between income and expenses. To improve it, you can either reduce expenses or increase income.`,el:null},{tab:`budget`,title:`Review your expenses`,text:`Look through your expenses below. Are there any you could reduce? Tap any expense to edit it.`,el:`#budget-content`}],tracking:[{tab:`budget`,title:`Track your actual spending`,text:`Recording what you actually spend helps you stay on track. You can import your bank statement or log spends manually.`,el:null},{tab:`budget`,title:`Import transactions`,text:`Tap "Import Transactions" to upload a bank CSV, or use the + button to log individual spends.`,el:`[onclick*="openCsvImport"]`,watchClick:!0}],netWorth:[{tab:`networth`,title:`Build your net worth picture`,text:`Add what you own (property, savings, super) as assets. Add what you owe (mortgage, loans, credit cards) as liabilities.`,el:null},{tab:`networth`,title:`Start adding`,text:`Use the buttons below to add your first asset or liability. Even rough numbers help build the picture.`,el:`#networth-content`}],goals:[{tab:`goals`,title:`Set a financial goal`,text:`Goals keep you motivated — a holiday fund, debt payoff, or savings target. Let's create your first one.`,el:null},{tab:`goals`,title:`Tap "+ New Goal"`,text:`Choose a goal type, set a target amount, and start tracking your progress.`,el:`[onclick*="openAddGoal"]`,watchClick:!0}]};function ll(e){let t=cl[e];t&&nl(t)}var ul=!1;function dl(e){ul=!0;let t=document.getElementById(`cv-readonly-bar`),n=document.getElementById(`cv-signout-btn`);t&&(t.style.display=``),n&&(n.style.display=`none`),window.showChildView(e)}function fl(e){ul=!0;let t=document.getElementById(`cv-readonly-bar`),n=document.getElementById(`cv-signout-btn`);t&&(t.style.display=``),n&&(n.style.display=`none`),window.showChildView(e),setTimeout(()=>{_cvSwitchTab(`calendar`,e)},50)}function pl(){let e=document.getElementById(`child-view-overlay`);e.classList.add(`hidden`),e.style.display=``;let t=document.getElementById(`cv-readonly-bar`),n=document.getElementById(`cv-signout-btn`);t&&(t.style.display=`none`),n&&(n.style.display=``);let r=ul;ul=!1,r||(window._activeProfile=null,window.clearKidSession(),window.switchProfile())}function ml(){let e=Number(s.settings?.routineResetHour??0),t=new Date,n=new Date(t);n.setHours(e,0,0,0),t<n&&n.setDate(n.getDate()-1),n.toISOString().slice(0,10);let r=!1;(s.routineAssignments||[]).forEach(e=>{e.completionState&&Object.keys(e.completionState).forEach(e=>{})});let i=t.toISOString().slice(0,10);if(s.settings?.lastRoutineResetCheck!==i){if(s.settings||(s.settings={}),s.settings.lastRoutineResetCheck=i,s.kids?.completions){let e=s.kids.completions.length;s.kids.completions=s.kids.completions.filter(e=>e.status===`pending`?new Date(e.completedAt||e.ts||0).toISOString().slice(0,10)>=i:!0),s.kids.completions.length!==e&&(r=!0)}r=!0}r&&window.saveData(s)}function hl(e,t){return!e||(e.pausePeriods||[]).some(e=>t>=e.from&&(!e.to||t<=e.to))||(e.skippedDates||[]).includes(t)?!1:Et(e.recurrence||null,t)}var gl={morning:[{label:`Make bed`,emoji:`🛏`,durationMin:2},{label:`Shower`,emoji:`🚿`,durationMin:10},{label:`Breakfast`,emoji:`🍳`,durationMin:15},{label:`Exercise`,emoji:`💪`,durationMin:20},{label:`Meditate`,emoji:`🧘`,durationMin:10},{label:`Plan the day`,emoji:`📋`,durationMin:5},{label:`Read`,emoji:`📚`,durationMin:15},{label:`Vitamins`,emoji:`💊`,durationMin:1},{label:`Walk`,emoji:`🚶`,durationMin:20},{label:`Journaling`,emoji:`✍️`,durationMin:10}],evening:[{label:`Tidy kitchen`,emoji:`🍽`,durationMin:10},{label:`Prep tomorrow`,emoji:`👔`,durationMin:5},{label:`Family time`,emoji:`👨‍👩‍👧`,durationMin:30},{label:`Read`,emoji:`📚`,durationMin:20},{label:`Lights out`,emoji:`💤`,durationMin:0},{label:`Stretch`,emoji:`🤸`,durationMin:10},{label:`Review the day`,emoji:`🪞`,durationMin:5},{label:`Skincare`,emoji:`🧴`,durationMin:5},{label:`No screens`,emoji:`📵`,durationMin:0},{label:`Brush teeth`,emoji:`🦷`,durationMin:3}],general:[{label:`Drink water`,emoji:`💧`,durationMin:1},{label:`Check messages`,emoji:`📱`,durationMin:5},{label:`Quick tidy`,emoji:`🧹`,durationMin:5},{label:`Gratitude`,emoji:`🙏`,durationMin:3}]};function _l(e){let t=e||new Date;return`${t.getFullYear()}-${String(t.getMonth()+1).padStart(2,`0`)}-${String(t.getDate()).padStart(2,`0`)}`}function H(){return _l(new Date)}function vl(){return window._currentUser?.uid||`guest`}function yl(){let e=vl();return(s.routines||[]).filter(t=>t.ownerType===`adult`&&(t.ownerId===e||(t.sharedWith||[]).includes(e)))}function bl(){return(s.routines||[]).filter(e=>e.ownerType===`household`)}function xl(e){return e.ownerId===vl()}function Sl(){return s.kids?.profiles||[]}function Cl(){return Math.max(0,...(s.routines||[]).map(e=>e.id))+1}function wl(){return vl(),(s.householdProfile?.authorizedUsers||[]).filter(e=>e.uid!==window.uid)}function Tl(e,t){return(s.routineAssignments||[]).find(n=>n.routineId===e&&n.childId===t)}function El(e){return(e?.completionState?.[H()]||[]).length}function Dl(e,t){if(!e)return 0;let n=0,r=new Date;for(;(e.completionState?.[_l(r)]||[]).length===t&&t>0;)n++,r.setDate(r.getDate()-1);return n}function Ol(e,t,n){let r=[],i=new Date;for(let a=n-1;a>=0;a--){let n=new Date(i);n.setDate(i.getDate()-a);let o=_l(n),s=(e?.completionState?.[o]||[]).length;r.push({key:o,label:n.getDate(),done:s,total:t})}return r}function kl(e){return(e.completions?.[H()]||[]).length}function Al(e){let t=0,n=new Date;for(;(e.completions?.[_l(n)]||[]).length===e.steps.length&&e.steps.length>0;)t++,n.setDate(n.getDate()-1);return t}function jl(e,t){let n=[],r=new Date;for(let i=t-1;i>=0;i--){let t=new Date(r);t.setDate(r.getDate()-i);let a=_l(t),o=(e.completions?.[a]||[]).length;n.push({key:a,label:t.getDate(),done:o,total:e.steps.length})}return n}function Ml(e){let t=new Set(e.steps.map(e=>e.label.toLowerCase())),n=e.name.toLowerCase(),r;return r=n.includes(`morning`)?[...gl.morning,...gl.general]:n.includes(`evening`)||n.includes(`night`)||n.includes(`bed`)?[...gl.evening,...gl.general]:[...gl.morning,...gl.evening,...gl.general],r.filter(e=>!t.has(e.label.toLowerCase()))}function Nl(e){if(e.ownerType===`household`){if(e.ownerId!==`household`)throw Error(`Scope violation: household routine has ownerId="${e.ownerId}"`)}else if(e.ownerType===`adult`){if(!e.ownerId||e.ownerId===`household`)throw Error(`Scope violation: adult routine has ownerId="${e.ownerId}"`)}else throw Error(`Scope violation: unknown ownerType="${e.ownerType}"`)}function Pl(e){(s.routines||[]).forEach(t=>{try{Nl(t)}catch(n){console.error(`[Routines] ${e}:`,n.message,t)}}),window.saveData(s)}function Fl(){let e=yl(),t=new Date().getHours();for(let n of e){let e=kl(n),r=n.steps.length;if(!r)continue;let[i]=(n.triggerTime||`00:00`).split(`:`).map(Number),a=Al(n);if(t>=i+1&&e===0)return{icon:n.emoji,text:`Your ${n.name} routine hasn't been started yet.`};if(a>0&&a%7==0)return{icon:`🔥`,text:`${a}-day streak on your ${n.name} routine — keep it up!`};if(e>0&&e===r-1)return{icon:n.emoji,text:`One step left in your ${n.name} routine!`}}return null}function Il(){let e=new Date().toISOString().slice(0,10),t=yl().filter(t=>hl(t,e));if(!t.length)return``;let n=new Date().getHours(),r=t.map(e=>{let t=kl(e),r=e.steps.length,i=r>0?Math.round(t/r*100):0,[a]=(e.triggerTime||`00:00`).split(`:`).map(Number),o=n>=a+1&&t===0&&r>0;return`<div class="routine-today-row">
    ...
    </div>`}).join(``),i=Fl();return`${i?`<div class="routine-intel-nudge" onclick="activateTab('routines')" style="margin-bottom:8px">
  ...
  </div>`}var Ll=`mine`;Object.defineProperty(window,`_routineActiveTab`,{get(){return Ll},set(e){Ll=e},configurable:!0});function U(){let e=document.getElementById(`routines-content`);if(!e)return;let t=Sl(),n=t.length>0,r=`<div class="page-header" style="margin-bottom:4px">
    <h1>Routines</h1>
  ...
  <div style="display:flex;justify-content:flex-end;margin-bottom:16px">
    <button class="btn btn-primary btn-sm" onclick="_routineCreate()">＋ New routine</button>
      ...
      <button class="routine-tab${Ll===`mine`?` active`:``}" onclick="_routineSetTab('mine')">My Routines</button>
      <button class="routine-tab${Ll===`children`?` active`:``}" onclick="_routineSetTab('children')">Children's Routines</button>
    </div>`),r+=Ll===`children`&&n?ql(t):zl(),e.innerHTML=r}function Rl(e){Ll=e,U()}function zl(){let e=vl(),t=!1;e!==`guest`&&((s.routines||[]).forEach(n=>{n.ownerType===`adult`&&n.ownerId===`guest`&&(n.ownerId=e,t=!0)}),t&&Pl(`lazy-uid-claim`));let n=new Date().toISOString().slice(0,10),r=yl().filter(e=>hl(e,n)),i=H(),a=``;return r.length?(r.forEach(e=>{let t=xl(e),n=e.linkedType===`join`,r=!t&&(e.sharedWith||[]).includes(window.uid),o=e.completions?.[i]||[],s=e.steps.length,c=s>0?Math.round(o.length/s*100):0,l=o.length===s&&s>0,u=Al(e),d=e.steps.reduce((e,t)=>e+(t.durationMin||0),0),f=t&&!n,p=``;n?p=`<span class="routine-owner-badge routine-joined-badge">🔗 Joined</span>`:e.linkedType===`duplicate`?p=`<span class="routine-owner-badge">📋 Duplicated</span>`:r&&(p=`<span class="routine-owner-badge routine-shared-badge">👥 Shared with you</span>`);let m=e.steps.map((t,n)=>{let r=o.includes(t.id);return`<div class="routine-step"${f?` draggable="true" data-routine="${e.id}" data-step="${t.id}" data-idx="${n}"
      ...
      <button class="btn btn-sm btn-secondary" onclick="_routineShowHistory(${e.id},null)">📅 History</button>`;f?v+=`<button class="btn btn-sm btn-secondary" onclick="_routineShareMenu(${e.id})">👥 Share</button>`:n?v+=`<button class="btn btn-sm btn-secondary" onclick="_routineDuplicateFromJoined(${e.id})">📋 Duplicate to mine</button>
        <button class="btn btn-sm btn-ghost" style="color:#ef4444;margin-left:auto" onclick="_routineLeave(${e.id})">Leave</button>`:r&&(v+=`<button class="btn btn-sm btn-secondary" onclick="_routineDuplicateTo(${e.id})">📋 Duplicate to mine</button>
    ...
    <div class="routine-new-card" onclick="_routineCreate()">
      <span style="font-size:22px">＋</span>
      <span class="routine-new-card-label">Create new routine</span>
    </div>`}var Bl={},Vl={};function Hl(e){Bl[e]=!Bl[e],U()}function Ul(e){Vl[e]=!0,U()}function Wl(e){let t=Ml(e);if(!t.length)return``;let n=!Bl[e.id],r=!!Vl[e.id],i=r?t:t.slice(0,3),a=t.length-3,o=e.ownerType===`household`,s=i.map(t=>`
  ...
  </div>`}function Gl(e,t,n,r){window.openModal(`Add step`,`
  ...
  `,()=>{Kl(e,document.getElementById(`rse-label`)?.value.trim()||t,document.getElementById(`rse-emoji`)?.value.trim()||n,parseInt(document.getElementById(`rse-dur`)?.value)||0,parseInt(document.getElementById(`rse-pts`)?.value)||0,document.getElementById(`rse-notes`)?.value.trim()||``)})}function Kl(e,t,n,r,i,a){let o=(s.routines||[]).find(t=>String(t.id)===String(e));if(!o)return;let c=new Set([...o.steps.map(e=>e.id),...Object.values(o.completions||{}).flat(),...(s.routineAssignments||[]).filter(t=>t.routineId===e).flatMap(e=>Object.values(e.completionState||{}).flat())]),l=Math.max(0,...c)+1;for(;c.has(l);)l++;o.steps.push({id:l,label:t,emoji:n,durationMin:r||0,points:i||0,notes:a||``}),du(o.id,o.steps[o.steps.length-1]),window.saveData(s),U()}function ql(e){let t=s.routineAssignments||[],n=bl(),r=H();if(!n.length)return`<div style="text-align:center;padding:32px 20px;color:var(--text-muted)">
    ...
    <div class="routine-new-card" onclick="_routineCreate('child')">
      ...
      <span class="routine-new-card-label">Create children's routine</span>
        ...
        ${e.emoji||`👤`} ${g(e.name)}
      </span>`}).join(``),l=o.length?o.map(({kid:e,assignment:t})=>{let n=t.completionState?.[r]||[],i=s>0?Math.round(n.length/s*100):0,a=Dl(t,s),o=n.length===s&&s>0;return`<div style="display:flex;align-items:center;gap:8px;padding:4px 0">
  ...
  `,()=>{let e=document.getElementById(`rp-from`)?.value;if(!e)return;let n=document.getElementById(`rp-to`)?.value||void 0,r=document.getElementById(`rp-reason`)?.value.trim()||void 0;t.pausePeriods.push({from:e,to:n,reason:r}),window.saveData(s),window.closeModal(),U()})}function nu(e,t){let n=(s.routines||[]).find(t=>String(t.id)===String(e));!n||!n.pausePeriods||(n.pausePeriods.splice(t,1),window.saveData(s),window.closeModal(),U())}function ru(e,t){let n=Tl(e,t);n&&(n.completionState&&delete n.completionState[H()],window.saveData(s),U())}function iu(e){let t=H();(s.routineAssignments||[]).filter(t=>t.routineId===e).forEach(e=>{e.completionState&&delete e.completionState[t]}),window.saveData(s),U()}var au=null;Object.defineProperty(window,`_routineDragIdx`,{get(){return au},set(e){au=e},configurable:!0});function ou(e){au=parseInt(e.currentTarget.dataset.idx),e.currentTarget.classList.add(`dragging`),e.dataTransfer.effectAllowed=`move`}function su(e){e.preventDefault(),e.dataTransfer.dropEffect=`move`,document.querySelectorAll(`.routine-step.drag-over`).forEach(e=>e.classList.remove(`drag-over`)),e.currentTarget.classList.add(`drag-over`)}function cu(e,t){e.preventDefault();let n=parseInt(e.currentTarget.dataset.idx);if(au===null||au===n)return;let r=(s.routines||[]).find(e=>String(e.id)===String(t));if(!r)return;let i=r.steps.splice(au,1)[0];r.steps.splice(n,0,i),au=null,window.saveData(s),U()}function lu(e){e.currentTarget.classList.remove(`dragging`),document.querySelectorAll(`.routine-step.drag-over`).forEach(e=>e.classList.remove(`drag-over`)),au=null}function uu(e){let t=(s.routines||[]).find(t=>String(t.id)===String(e));if(!t)return;let n=t.ownerType===`household`;window.openModal(`Add step`,`
    ...
    </div>`,()=>{let r=document.getElementById(`rs-label`)?.value.trim();if(!r)return;let i=new Set([...t.steps.map(e=>e.id),...Object.values(t.completions||{}).flat(),...(s.routineAssignments||[]).filter(t=>t.routineId===e).flatMap(e=>Object.values(e.completionState||{}).flat())]),a=Math.max(0,...i)+1;for(;i.has(a);)a++;let o={id:a,label:r,emoji:document.getElementById(`rs-emoji`)?.value.trim()||`✅`,durationMin:parseInt(document.getElementById(`rs-dur`)?.value)||0,points:n&&parseInt(document.getElementById(`rs-pts`)?.value)||0,notes:document.getElementById(`rs-notes`)?.value.trim()||``};t.steps.push(o),du(t.id,o),window.saveData(s),window.closeModal(),U()}),setTimeout(()=>document.getElementById(`rs-label`)?.focus(),100)}function du(e,t){(s.routines||[]).forEach(n=>{if(n.linkedType===`join`&&n.linkedFrom===e&&!n.steps.some(e=>e.label===t.label)){let e=Math.max(0,...n.steps.map(e=>e.id),0)+1;n.steps.push({...t,id:e})}})}function fu(e,t){(s.routines||[]).forEach(n=>{n.linkedType===`join`&&n.linkedFrom===e&&(n.steps=n.steps.filter(e=>e.label!==t))})}function pu(e){let t=(s.routines||[]).find(t=>String(t.id)===String(e));if(!t)return;let n=t.ownerType===`household`,r=t.steps.map(t=>`
  ...
  `,()=>{let e=document.getElementById(`re-name`)?.value.trim(),n=document.getElementById(`re-emoji`)?.value.trim(),r=document.getElementById(`re-time`)?.value;if(e&&(t.name=e),n&&(t.emoji=n),r&&(t.triggerTime=r),t.ownerType===`household`){let e=parseInt(document.getElementById(`re-pts`)?.value);isNaN(e)||(t.pointsPerCompletion=e)}t.recurrence=xu(),t.lastEditedBy=vl(),t.lastEditedAt=new Date().toISOString(),window.saveData(s),window.closeModal(),U()}),setTimeout(()=>{bu()},100)}function mu(e,t,n){let r=(s.routines||[]).find(t=>String(t.id)===String(e));if(!r)return;let i=r.steps.find(e=>e.id===t);r.steps=r.steps.filter(e=>e.id!==t),Object.keys(r.completions||{}).forEach(e=>{r.completions[e]=r.completions[e].filter(e=>e!==t)}),(s.routineAssignments||[]).filter(t=>t.routineId===e).forEach(e=>{Object.keys(e.completionState||{}).forEach(n=>{e.completionState[n]=e.completionState[n].filter(e=>e!==t)})}),i&&fu(r.id,i.label),window.saveData(s),n?(window.closeModal(),pu(e)):(window.closeModal(),U())}function hu(e,t){let n=(s.routines||[]).find(t=>String(t.id)===String(e));if(!n)return;let r=n.steps.find(e=>e.id===t);if(!r)return;let i=n.ownerType===`household`;window.openModal(`Edit step`,`
    <div class="form-group" style="display:flex;gap:12px">
      <div style="flex:1"><label class="form-label">Label</label>
  ...
  `,()=>{let e=document.getElementById(`rst-label`)?.value.trim();e&&(r.label=e,r.emoji=document.getElementById(`rst-emoji`)?.value.trim()||r.emoji,r.durationMin=parseInt(document.getElementById(`rst-dur`)?.value)||0,r.notes=document.getElementById(`rst-notes`)?.value.trim()||``,i&&(r.points=parseInt(document.getElementById(`rst-pts`)?.value)||0),window.saveData(s),window.closeModal(),U())})}function gu(e){let t=(s.routines||[]).find(t=>String(t.id)===String(e));if(!t)return;let n=Sl();if(!n.length){window.openModal(`Assign`,`<p style="color:var(--text-muted);font-size:13px">No children in this household yet.</p>`,()=>window.closeModal());return}let r=s.routineAssignments||[],i=n.map(t=>{let n=r.some(n=>n.routineId===e&&n.childId===t.id);return`<div style="display:flex;align-items:center;gap:12px;padding:10px 0;border-bottom:1px solid var(--border)">
    ...
    ${i}
  `,()=>window.closeModal())}function _u(e,t){s.routineAssignments||(s.routineAssignments=[]);let n=Tl(e,t);n?(n.archivedCompletionState=JSON.parse(JSON.stringify(n.completionState||{})),n.archivedAt=new Date().toISOString(),s.routineAssignments=s.routineAssignments.filter(n=>!(n.routineId===e&&n.childId===t))):s.routineAssignments.push({id:window.uid(),routineId:e,childId:t,assignedBy:vl(),assignedAt:new Date().toISOString(),completionState:{},archivedCompletionState:null,childIds:[t]}),window.saveData(s),U()}function vu(e){let t=e?.type||`daily`,n=e?.startDate||new Date().toISOString().slice(0,10),r=e?.endDate||``,i=e?.intervalDays||2,a=e?.days||[];return`
    <div class="form-group">
    ...
    <div id="rn-rec-summary" style="font-size:12px;color:var(--primary);font-weight:600;padding:8px 12px;background:var(--primary-light);border-radius:8px;margin-bottom:4px"></div>`}function yu(){let e=document.getElementById(`rn-rec-type`)?.value;e&&(document.getElementById(`rn-rec-days`).style.display=e===`specific_days`?`flex`:`none`,document.getElementById(`rn-rec-interval`).style.display=e===`interval`?`flex`:`none`,bu())}function bu(){let e=document.getElementById(`rn-rec-summary`);if(!e)return;let t=document.getElementById(`rn-rec-type`)?.value,n={daily:`Every day`,weekdays:`Mon – Fri`,weekends:`Sat & Sun`,one_time:`Once only`};if(n[t]){e.textContent=n[t];return}if(t===`specific_days`){let t=[...document.querySelectorAll(`input[name="rn-rec-dow"]:checked`)].map(e=>[`Sun`,`Mon`,`Tue`,`Wed`,`Thu`,`Fri`,`Sat`][e.value]);e.textContent=t.length?t.join(`, `):`No days selected`}else t===`interval`&&(e.textContent=`Every ${document.getElementById(`rn-rec-interval-days`)?.value||2} days`)}function xu(){let e=document.getElementById(`rn-rec-type`)?.value||`daily`,t=document.getElementById(`rn-rec-start`)?.value||new Date().toISOString().slice(0,10),n=document.getElementById(`rn-rec-end`)?.value||void 0,r={type:e,startDate:t};return n&&(r.endDate=n),e===`specific_days`&&(r.days=[...document.querySelectorAll(`input[name="rn-rec-dow"]:checked`)].map(e=>e.value)),e===`interval`&&(r.intervalDays=parseInt(document.getElementById(`rn-rec-interval-days`)?.value)||2),r}function Su(e){let t=e===`child`;window.openModal(`New routine`,`
    <div class="form-group">
    ...
    ${vu(null)}
  `,()=>{let e=document.getElementById(`rn-name`)?.value.trim();if(!e)return;let t=document.querySelector(`input[name="rn-type"]:checked`)?.value||`adult`,n=document.getElementById(`rn-emoji`)?.value.trim()||`📋`,r=document.getElementById(`rn-time`)?.value||(t===`child`?`19:30`:`08:00`),i=parseInt(document.getElementById(`rn-pts`)?.value)||0,a=vl(),o=Cl(),c=xu();if(t===`adult`){let t={id:o,name:e,emoji:n,triggerTime:r,steps:[],completions:{},assignedTo:[],ownerType:`adult`,ownerId:a,sharedWith:[],linkedFrom:null,linkedType:null,pointsPerCompletion:0,recurrence:c,skippedDates:[],pausePeriods:[]};Nl(t),s.routines.push(t),Pl(`create:adult`),window.closeModal(),Ll=`mine`}else{let t={id:o,name:e,emoji:n,triggerTime:r,steps:[],completions:{},assignedTo:[],ownerType:`household`,ownerId:`household`,sharedWith:[],linkedFrom:null,linkedType:null,pointsPerCompletion:i,createdBy:a,lastEditedBy:a,lastEditedAt:new Date().toISOString(),recurrence:c,skippedDates:[],pausePeriods:[]};Nl(t),s.routines.push(t),Pl(`create:child`),window.closeModal(),Ll=`children`}U()}),setTimeout(()=>{document.getElementById(`rn-name`)?.focus(),bu()},100)}function Cu(e){document.querySelectorAll(`input[name="rn-type"]`).forEach(t=>t.checked=t.value===e),document.getElementById(`rn-type-adult`)?.classList.toggle(`selected`,e===`adult`),document.getElementById(`rn-type-child`)?.classList.toggle(`selected`,e===`child`);let t=document.getElementById(`rn-time`);t&&(t.value=e===`child`?`19:30`:`08:00`);let n=document.getElementById(`rn-pts-group`);n&&(n.style.display=e===`child`?`block`:`none`)}function wu(e){let t=(s.routines||[]).find(t=>String(t.id)===String(e));t&&confirm(`Delete "${t.name}"? This cannot be undone.`)&&(s.routines=s.routines.filter(t=>t.id!==e),s.routineAssignments=(s.routineAssignments||[]).filter(t=>t.routineId!==e),window.saveData(s),U())}function Tu(e){let t=(s.routines||[]).find(t=>String(t.id)===String(e));t&&confirm(`Leave "${t.name}"? Your completion history will be removed.`)&&(s.routines=s.routines.filter(t=>t.id!==e),window.saveData(s),U())}function Eu(e,t){if(!confirm(`Unassign this child? Their completion history will be archived.`))return;let n=Tl(e,t);n&&(n.archivedCompletionState=JSON.parse(JSON.stringify(n.completionState||{})),n.archivedAt=new Date().toISOString()),s.routineAssignments=(s.routineAssignments||[]).filter(n=>!(n.routineId===e&&n.childId===t)),(s.routineAssignments||[]).some(t=>t.routineId===e)||(s.routines=s.routines.filter(t=>t.id!==e)),window.saveData(s),U()}function Du(e){let t=(s.routines||[]).find(t=>String(t.id)===String(e));if(!t)return;let n=wl();if(!n.length){window.openModal(`Share routine`,`<p style="color:var(--text-muted);font-size:13px">No other adults have joined yet. Invite them via Settings → Household first.</p>`,()=>window.closeModal());return}let r=t.sharedWith||[],i=n.map(t=>{let n=r.includes(t.uid);return`<div style="display:flex;align-items:center;gap:10px;padding:8px 0;border-bottom:1px solid var(--border)">
    ...
    ${i}
  `,()=>window.closeModal())}function Ou(e,t){let n=(s.routines||[]).find(t=>String(t.id)===String(e));if(!n)return;n.sharedWith||(n.sharedWith=[]);let r=n.sharedWith.indexOf(t);r===-1?n.sharedWith.push(t):n.sharedWith.splice(r,1),window.saveData(s),window.closeModal(),Du(e)}function ku(e){let t=(s.routines||[]).find(t=>t.id===e);if(!t)return;vl();let n={...JSON.parse(JSON.stringify(t)),id:Cl(),ownerId:window.uid,ownerType:`adult`,sharedWith:[],completions:{},linkedFrom:e,linkedType:`duplicate`};s.routines.push(n),window.saveData(s),window.closeModal(),U()}function Au(e){let t=(s.routines||[]).find(t=>t.id===e);if(!t)return;if(vl(),(s.routines||[]).some(t=>t.linkedFrom===e&&t.linkedType===`join`&&t.ownerId===window.uid)){alert(`You have already joined this routine.`);return}let n={...JSON.parse(JSON.stringify(t)),id:Cl(),ownerId:window.uid,ownerType:`adult`,sharedWith:[],completions:{},linkedFrom:e,linkedType:`join`};s.routines.push(n),window.saveData(s),window.closeModal(),U()}function ju(e){let t=(s.routines||[]).find(t=>t.id===e);t?.linkedFrom&&ku(t.linkedFrom)}function Mu(e,t){let n=(s.routines||[]).find(t=>String(t.id)===String(e));if(!n)return;let r=n.steps.length,i,a,o;if(t){let s=Tl(e,t),c=Sl().find(e=>e.id===t);i=Ol(s,r,90),a=Dl(s,r),o=`${n.emoji} ${n.name} — ${c?.name||`Child`}`}else i=jl(n,90),a=Al(n),o=`${n.emoji} ${n.name} — History`;let c=i.filter(e=>e.done===e.total&&e.total>0).length,l=i.filter(e=>e.done>0&&e.done<e.total).length,u=i.map(e=>{let t=e.done===e.total&&e.total>0?`full`:e.done>0?`partial`:`empty`,n=e.done===e.total&&e.total>0?`✓`:e.done>0?`${e.done}`:``;return`<div class="routine-history-dot ${t}" title="${e.key}: ${e.done}/${e.total}">${n}</div>`}).join(``);window.openModal(o,`
  ...
  `,()=>window.closeModal())}var Nu=`parent`,Pu=`overview`;Object.defineProperty(window,`kidsView`,{get(){return Nu},set(e){Nu=e},configurable:!0}),Object.defineProperty(window,`kidsParentTab`,{get(){return Pu},set(e){Pu=e},configurable:!0});function Fu(){return Date.now().toString(36)+Math.random().toString(36).slice(2)}function W(){let e=document.getElementById(`kids-content`);if(!e)return;let t=s.kids;if(Nu===`parent`)Lu(e,t,t.completions.filter(e=>e.status===`pending`).length+t.redemptions.filter(e=>e.status===`pending`).length);else{let n=t.profiles.find(e=>e.id===Nu);n?Hu(e,t,n):(Nu=`parent`,W())}}function Iu(e,t){return e.completions.filter(e=>e.kidId===t&&e.status===`approved`).reduce((t,n)=>t+(e.chores.find(e=>e.id===n.choreId)?.points||0),0)-e.redemptions.filter(e=>e.kidId===t&&e.status===`approved`).reduce((t,n)=>t+(e.prizes.find(e=>e.id===n.prizeId)?.pointCost||0),0)}function Lu(e,t,n){let r=`
    <div style="display:flex;justify-content:flex-end;margin-bottom:16px">
    ...
    <div style="text-align:center;padding:60px 20px">
    ...
    </div>`;let t=`<div class="kids-grid">`;return e.profiles.forEach(n=>{let r=Iu(e,n.id),i=e.chores.filter(e=>e.assignedTo===n.id||e.assignedTo===`all`).length,a=e.completions.filter(e=>e.kidId===n.id&&e.status===`pending`).length;t+=`
      ...
      </div>`}),t+=`</div>`,t}function zu(e){let t=`<div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:16px">
    ...
    </div>`}),t}function Bu(e){let t=`<div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:16px">
    ...
    <button class="btn btn-primary btn-sm" onclick="openPrizeModal()">+ Add Prize</button>
      ...
      </div>`)})),r}function Hu(e,t,n){let r=Iu(t,n.id),i=t.chores.filter(e=>e.assignedTo===n.id||e.assignedTo===`all`),a=new Set(t.completions.filter(e=>e.kidId===n.id&&e.status===`pending`).map(e=>e.choreId)),o=new Date().toDateString(),s=new Set(t.completions.filter(e=>e.kidId===n.id&&e.status===`approved`&&new Date(e.completedAt).toDateString()===o).map(e=>e.choreId)),c=`
    ...
    <div style="font-size:15px;font-weight:700;margin-bottom:12px">Prize Shelf 🎁</div>`,t.prizes.length?t.prizes.forEach(e=>{let i=r>=e.pointCost,a=t.redemptions.some(t=>t.prizeId===e.id&&t.kidId===n.id&&t.status===`pending`);c+=`<div class="prize-card ${i&&!a?`can-afford`:``} ${a?`pending-redemption`:``}">
      ...
      </div>`}):c+=`<div style="color:#64748b;font-size:13px;padding:20px;text-align:center;background:#f8fafc;border-radius:10px">No prizes on the shelf yet!</div>`,c+=`</div></div>`,e.innerHTML=c}function Uu(){let e=s.childEvents||[],t=s.kids?.profiles||[],n=`<div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:16px">
    ...
    </div>`}var Wu=[`📅`,`⚽`,`🏊`,`🎓`,`🎂`,`🎨`,`🏃`,`🎭`,`🚌`,`🎡`,`🏖`,`🎪`];function Gu(e){let t=e?(s.childEvents||[]).find(t=>t.id===e):null,n=s.kids?.profiles||[];window.openModal(t?`Edit Event`:`Add Event`,`
  ...
  `,()=>{let e=document.getElementById(`ce-title`)?.value.trim();if(!e)return;let t=document.getElementById(`ce-emoji`)?.value.trim()||`📅`,n=document.getElementById(`ce-date`)?.value,r=document.getElementById(`ce-time`)?.value||void 0,i=document.getElementById(`ce-who`)?.value,a=document.getElementById(`ce-notes`)?.value.trim()||``,o=_routineRecurrenceCollect(),c=document.getElementById(`ce-id`)?.value;if(s.childEvents||(s.childEvents=[]),c){let l=s.childEvents.find(e=>e.id===c);l&&Object.assign(l,{title:e,emoji:t,date:n,time:r,notes:a,recurrence:o,isHouseholdWide:i===`all`,assignedTo:i===`all`?`all`:[i]})}else s.childEvents.push({id:Fu(),title:e,emoji:t,date:n,time:r,notes:a,recurrence:o,assignedTo:i===`all`?`all`:[i],isHouseholdWide:i===`all`,createdBy:_routineCurrentUserId()});window.saveData(s),window.closeModal(),W()}),setTimeout(()=>{_routineRecurrenceSummaryUpdate()},100)}function Ku(e){confirm(`Delete this event?`)&&(s.childEvents=(s.childEvents||[]).filter(t=>t.id!==e),window.saveData(s),W())}function qu(e,t){s.kids.completions.push({id:Fu(),choreId:e,kidId:t,completedAt:Date.now(),status:`pending`}),window.saveData(s),W()}function Ju(e){let t=s.kids.completions.find(t=>t.id===e);t&&(t.status=`approved`,t.approvedAt=Date.now()),window.saveData(s),W()}function Yu(e){let t=s.kids.completions.find(t=>t.id===e);t&&(t.status=`rejected`),window.saveData(s),W()}function Xu(e,t){s.kids.redemptions.push({id:Fu(),prizeId:e,kidId:t,requestedAt:Date.now(),status:`pending`}),window.saveData(s),W()}function Zu(e){let t=s.kids.redemptions.find(t=>t.id===e);if(t){t.status=`approved`,t.approvedAt=Date.now();let e=(s.kids.prizes||[]).find(e=>e.id===t.prizeId);s.kids.notifications||(s.kids.notifications=[]),s.kids.notifications.push({id:Fu(),kidId:t.kidId,type:`prize_approved`,prizeId:t.prizeId,prizeName:e?.name||`Prize`,prizeEmoji:e?.emoji||`🎁`,ts:Date.now(),read:!1})}window.saveData(s),W()}function Qu(e){let t=s.kids.redemptions.find(t=>t.id===e);if(t){t.status=`rejected`;let e=(s.kids.prizes||[]).find(e=>e.id===t.prizeId);s.kids.notifications||(s.kids.notifications=[]),s.kids.notifications.push({id:Fu(),kidId:t.kidId,type:`prize_declined`,prizeId:t.prizeId,prizeName:e?.name||`Prize`,prizeEmoji:e?.emoji||`🎁`,ts:Date.now(),read:!1})}window.saveData(s),W()}function $u(e){s.kids.chores=s.kids.chores.filter(t=>t.id!==e),window.saveData(s),W()}function ed(e){s.kids.prizes=s.kids.prizes.filter(t=>t.id!==e),window.saveData(s),W()}var td=[`😊`,`🦁`,`🐯`,`🐻`,`🦊`,`🐸`,`🐧`,`🦋`,`🌟`,`🎈`,`🚀`,`⚡`],nd=[`🧹`,`🍽`,`🐕`,`🛏`,`📚`,`🌿`,`🧺`,`🗑`,`🧽`,`🚿`,`🛒`,`🪴`],rd=[`🍦`,`🎬`,`🎮`,`🍕`,`🎁`,`💰`,`🏖`,`🎡`,`🎨`,`👟`,`📱`,`🎭`];function id(e,t){return`<div style="display:flex;flex-wrap:wrap;gap:6px" id="emoji-pick">
  ...
  </div>`}function ad(e){e.closest(`#emoji-pick`).querySelectorAll(`button`).forEach(e=>{e.style.borderColor=`var(--border)`,e.style.background=`none`}),e.style.borderColor=`#0891b2`,e.style.background=`#ecfeff`}function od(e){let t=document.querySelector(`#emoji-pick button[style*="#0891b2"]`);return t?t.dataset.e:e}function sd(){cd(null)}function cd(e){let t=e?s.kids.profiles.find(t=>String(t.id)===String(e)):null;document.getElementById(`modal-title`).textContent=t?`Edit Kid`:`Add Kid`,document.getElementById(`modal-body`).innerHTML=`
    ...
    <div class="form-group"><label class="form-label">Avatar</label>${id(td,t?.emoji||`😊`)}</div>
    <div class="form-group">
  ...
  `,document.getElementById(`modal-footer`).innerHTML=`<button class="btn btn-ghost" onclick="window.closeModal()">Cancel</button><button class="btn btn-primary" onclick="saveKid()">Save</button>`,document.getElementById(`modal-overlay`).classList.remove(`hidden`)}function ld(){let e=document.getElementById(`k-name`).value.trim();if(!e)return;let t=parseInt(document.getElementById(`k-age`).value)||null,n=od(`😊`),r=document.getElementById(`k-id`)?.value;if(r){let i=s.kids.profiles.find(e=>String(e.id)===String(r));i&&Object.assign(i,{name:e,emoji:n,age:t})}else s.kids.profiles.push({id:Fu(),name:e,emoji:n,age:t,savings:0});window.saveData(s),window.closeModal(),W()}function ud(e){let t=s.kids.profiles.find(t=>String(t.id)===String(e));t&&confirm(`Remove ${t.name} from this household?\n\nThis will permanently delete their chores, prizes and points history. This cannot be undone.`)&&(s.kids.profiles=s.kids.profiles.filter(t=>t.id!==e),s.kids.chores=s.kids.chores.filter(t=>t.assignedTo!==e),s.kids.completions=s.kids.completions.filter(t=>t.kidId!==e),s.kids.redemptions=s.kids.redemptions.filter(t=>t.kidId!==e),s.meals?.lunchbox?.profiles&&(s.meals.lunchbox.profiles=s.meals.lunchbox.profiles.filter(t=>t.id!==e)),s.childEvents&&(s.childEvents=s.childEvents.filter(t=>{if(t.isHouseholdWide)return!0;let n=(Array.isArray(t.assignedTo)?t.assignedTo:[t.assignedTo]).filter(t=>t!==e);return n.length?(t.assignedTo=n,!0):!1})),window.getDeviceProfile()===e&&window.setDeviceProfile(`adult`),window.saveData(s),window.closeModal(),Nu=`parent`,W())}function dd(e){let t=e?s.kids.chores.find(t=>t.id===e):null,n=s.kids.profiles;document.getElementById(`modal-title`).textContent=t?`Edit Chore`:`Add Chore`,document.getElementById(`modal-body`).innerHTML=`
    ...
    <div class="form-group"><label class="form-label">Emoji</label>${id(nd,t?.emoji||`🧹`)}</div>
    <div class="form-row">
  ...
  `,document.getElementById(`modal-footer`).innerHTML=`<button class="btn btn-ghost" onclick="window.closeModal()">Cancel</button><button class="btn btn-primary" onclick="saveChore()">Save Chore</button>`,document.getElementById(`modal-overlay`).classList.remove(`hidden`)}function fd(){let e=document.getElementById(`c-name`).value.trim();if(!e)return;let t=parseInt(document.getElementById(`c-pts`).value)||10,n=document.getElementById(`c-freq`).value,r=document.getElementById(`c-who`).value,i=od(`🧹`),a=document.getElementById(`c-id`)?.value;if(a){let o=s.kids.chores.find(e=>e.id===a);o&&Object.assign(o,{name:e,emoji:i,points:t,frequency:n,assignedTo:r})}else s.kids.chores.push({id:Fu(),name:e,emoji:i,points:t,frequency:n,assignedTo:r});window.saveData(s),window.closeModal(),W()}function pd(e){let t=e?s.kids.prizes.find(t=>String(t.id)===String(e)):null;document.getElementById(`modal-title`).textContent=t?`Edit Prize`:`Add Prize`,document.getElementById(`modal-body`).innerHTML=`
    ...
    <div class="form-group"><label class="form-label">Emoji</label>${id(rd,t?.emoji||`🎁`)}</div>
    <div class="form-row">
  ...
  `,document.getElementById(`modal-footer`).innerHTML=`<button class="btn btn-ghost" onclick="window.closeModal()">Cancel</button><button class="btn btn-primary" onclick="savePrize()">Save Prize</button>`,document.getElementById(`modal-overlay`).classList.remove(`hidden`)}function md(){let e=document.getElementById(`p-name`).value.trim();if(!e)return;let t=parseInt(document.getElementById(`p-cost`).value)||50,n=document.getElementById(`p-type`).value,r=document.getElementById(`p-desc`).value.trim(),i=od(`🎁`),a=document.getElementById(`p-id`)?.value;if(a){let o=s.kids.prizes.find(e=>e.id===a);o&&Object.assign(o,{name:e,emoji:i,pointCost:t,type:n,description:r})}else s.kids.prizes.push({id:Fu(),name:e,emoji:i,pointCost:t,type:n,description:r});window.saveData(s),window.closeModal(),W()}function hd(e){let t=s.kids.profiles.find(t=>t.id===e);t&&(document.getElementById(`modal-title`).textContent=`${t.emoji} ${t.name}'s Savings Jar`,document.getElementById(`modal-body`).innerHTML=`
  ...
  `,document.getElementById(`modal-footer`).innerHTML=`<button class="btn btn-ghost" onclick="window.closeModal()">Cancel</button><button class="btn btn-primary" onclick="addSavings('${e}')">Add to Jar 💰</button>`,document.getElementById(`modal-overlay`).classList.remove(`hidden`))}function gd(e){let t=parseFloat(document.getElementById(`s-amount`).value)||0;if(!t)return;let n=s.kids.profiles.find(t=>t.id===e);n&&(n.savings=(n.savings||0)+t),window.saveData(s),window.closeModal(),W()}var _d=null,vd={};function yd(){return _d?Promise.resolve(_d):new Promise((e,t)=>{let n=indexedDB.open(`home_finance_receipts`,1);n.onupgradeneeded=e=>{let t=e.target.result;t.objectStoreNames.contains(`receipts`)||t.createObjectStore(`receipts`,{keyPath:`id`,autoIncrement:!0}).createIndex(`itemKey`,`itemKey`,{unique:!1})},n.onsuccess=t=>{_d=t.target.result,e(_d)},n.onerror=()=>t(n.error)})}async function bd(){let e=await yd();return new Promise(t=>{let n=e.transaction(`receipts`,`readonly`).objectStore(`receipts`).getAll();n.onsuccess=()=>{vd={},n.result.forEach(e=>{vd[e.itemKey]=(vd[e.itemKey]||0)+1}),t()}})}async function xd(e){let t=await yd();return new Promise(n=>{let r=t.transaction(`receipts`,`readonly`).objectStore(`receipts`).index(`itemKey`).getAll(e);r.onsuccess=()=>n(r.result)})}async function Sd(e,t){let n=await yd();return new Promise((r,i)=>{let a=n.transaction(`receipts`,`readwrite`).objectStore(`receipts`).add({itemKey:e,fileName:t.name,fileType:t.type,fileSize:t.size,data:t,uploadedAt:new Date().toISOString()});a.onsuccess=()=>r(),a.onerror=()=>i(a.error)})}async function Cd(e){let t=await yd();return new Promise(n=>{let r=t.transaction(`receipts`,`readwrite`);r.objectStore(`receipts`).delete(e),r.oncomplete=n})}function wd(e){return e===`own-funds`?`<span class="badge" style="background:#ecfeff;color:#166534;border:1px solid #bbf7d0">Own Funds</span>`:`<span class="badge" style="background:#eff6ff;color:#1e40af;border:1px solid #bfdbfe">Loan</span>`}function Td(e,t){let n=vd[e]||0;return`<button class="attach-btn${n?` has-files`:``}" onclick="openReceiptsModal('${e}','${t.replace(/'/g,`\\'`)}')" title="${n?n+` receipt(s)`:`Add receipt`}">📎${n?` ${n}`:``}</button>`}function Ed(e){return e>1048576?`${(e/1048576).toFixed(1)} MB`:`${Math.round(e/1024)} KB`}function Dd(e){return e===`application/pdf`?`📄`:e.startsWith(`image/`)?`🖼️`:`📎`}async function Od(e,t){window.openModal(`Receipts — ${t}`,`<div style="padding:20px;text-align:center;color:var(--text-muted)">Loading…</div>`,()=>{}),document.getElementById(`modal-footer`).innerHTML=`<button class="btn btn-ghost" onclick="window.closeModal()">Close</button>`,await kd(e,t)}async function kd(e,t){let n=e,r=t.replace(/'/g,`\\'`),i=await xd(e),a=``;i.length?i.forEach(e=>{let t=e.type===`link`,i=t?`🔗`:Dd(e.fileType),o=t?e.linkName||e.url:e.fileName,s=t?`<span style="color:var(--text-muted);font-size:11px;overflow:hidden;text-overflow:ellipsis;white-space:nowrap;display:block;max-width:260px">${e.url}</span>`:`<span>${Ed(e.fileSize)} · ${new Date(e.uploadedAt).toLocaleDateString(`en-AU`)}</span>`,c=t?`<button class="btn btn-ghost btn-sm" onclick="window.open('${e.url.replace(/'/g,`\\'`).replace(/"/g,`&quot;`)}','_blank')">Open</button>`:`<button class="btn btn-ghost btn-sm" onclick="viewReceipt(${e.id})">View</button>`;a+=`
    ...
    <div class="drop-zone" id="drop-zone" onclick="document.getElementById('receipt-file-input').click()">
    ...
    </div>`,document.getElementById(`modal-body`).innerHTML=a;let o=document.getElementById(`drop-zone`);o.addEventListener(`dragover`,e=>{e.preventDefault(),o.classList.add(`drag-over`)}),o.addEventListener(`dragleave`,()=>o.classList.remove(`drag-over`)),o.addEventListener(`drop`,async n=>{n.preventDefault(),o.classList.remove(`drag-over`),await Md(e,t,Array.from(n.dataTransfer.files))})}async function Ad(e,t){let n=document.getElementById(`link-url`).value.trim();if(!n){document.getElementById(`link-url`).focus();return}let r=document.getElementById(`link-name`).value.trim(),i=await yd();await new Promise((t,a)=>{let o=i.transaction(`receipts`,`readwrite`).objectStore(`receipts`).add({itemKey:e,type:`link`,url:n,linkName:r,uploadedAt:new Date().toISOString()});o.onsuccess=t,o.onerror=()=>a(o.error)}),await bd(),renderBuild(),await kd(e,t)}async function jd(e,t){let n=document.getElementById(`receipt-file-input`);await Md(e,t,Array.from(n.files))}async function Md(e,t,n){for(let t of n)await Sd(e,t);await bd(),renderBuild(),await kd(e,t)}async function Nd(e){let t=await yd(),n=await new Promise(n=>{let r=t.transaction(`receipts`,`readonly`).objectStore(`receipts`).get(e);r.onsuccess=()=>n(r.result)});if(n.type===`link`){window.open(n.url,`_blank`);return}let r=URL.createObjectURL(n.data);window.open(r,`_blank`),setTimeout(()=>URL.revokeObjectURL(r),15e3)}async function Pd(e,t,n){confirm(`Delete this receipt?`)&&(await Cd(e),await bd(),renderBuild(),await kd(t,n))}var Fd=null;window.addEventListener(`beforeinstallprompt`,e=>{e.preventDefault(),Fd=e;let t=document.getElementById(`install-btn`);t&&(t.style.display=`flex`)}),window.addEventListener(`appinstalled`,()=>{Fd=null;let e=document.getElementById(`install-btn`);e&&(e.style.display=`none`)});function Id(){Fd&&(Fd.prompt(),Fd.userChoice.then(()=>{Fd=null}))}var Ld={building:[{name:`Mortgage / Rent`,category:`Mortgage / Rent`,amount:3e3},{name:`Groceries`,category:`Groceries`,amount:1200},{name:`Utilities`,category:`Utilities`,amount:400},{name:`Transport`,category:`Transport`,amount:500},{name:`Insurance`,category:`Insurance`,amount:350},{name:`Entertainment`,category:`Entertainment`,amount:200}],mortgage:[{name:`Mortgage`,category:`Mortgage / Rent`,amount:3500},{name:`Groceries`,category:`Groceries`,amount:1200},{name:`Utilities`,category:`Utilities`,amount:400},{name:`Transport`,category:`Transport`,amount:500},{name:`Insurance`,category:`Insurance`,amount:350},{name:`Entertainment`,category:`Entertainment`,amount:200}],renting:[{name:`Rent`,category:`Mortgage / Rent`,amount:2500},{name:`Groceries`,category:`Groceries`,amount:1200},{name:`Utilities`,category:`Utilities`,amount:300},{name:`Transport`,category:`Transport`,amount:500},{name:`Insurance`,category:`Insurance`,amount:250},{name:`Entertainment`,category:`Entertainment`,amount:200}],own:[{name:`Groceries`,category:`Groceries`,amount:1200},{name:`Utilities`,category:`Utilities`,amount:400},{name:`Transport`,category:`Transport`,amount:500},{name:`Insurance`,category:`Insurance`,amount:400},{name:`Health`,category:`Health`,amount:300},{name:`Entertainment`,category:`Entertainment`,amount:300}]},G={step:1,adults:2,adultNames:[``,``],adultAges:[``,``],kids:0,kidProfiles:[],homeType:`mortgage`,incomes:[{name:``,amount:``,frequency:`Monthly`}],expenses:[],_emojiPickerOpen:null};function Rd(){return G.kids>0?[1,2,3,4,5,6]:[1,2,4,5,6]}function zd(){return Rd().indexOf(G.step)}function Bd(){G={step:1,adults:2,adultNames:[``,``],adultAges:[``,``],kids:0,kidProfiles:[],homeType:`mortgage`,incomes:[{name:``,amount:``,frequency:`Monthly`}],expenses:[],_emojiPickerOpen:null},document.getElementById(`onboarding-overlay`).style.display=`flex`,qd()}function Vd(){document.getElementById(`onboarding-overlay`).style.display=`none`}function Hd(e){for(G.adults=e;G.adultNames.length<e;)G.adultNames.push(``);for(G.adultNames=G.adultNames.slice(0,e);G.adultAges.length<e;)G.adultAges.push(``);G.adultAges=G.adultAges.slice(0,e),qd()}function Ud(e){G.kids=e,G.kidProfiles.length>e&&(G.kidProfiles=G.kidProfiles.slice(0,e)),qd()}function Wd(e){G._emojiPickerOpen=G._emojiPickerOpen===e?null:e,qd()}function Gd(e,t){G.kidProfiles[e].emoji=t,G._emojiPickerOpen=null,qd()}function Kd(e){G.expenses[e].skipped=!G.expenses[e].skipped,qd()}function qd(){let e=document.getElementById(`onboarding-card`),t=Rd(),n=zd(),r=G.step,i=t.map((e,t)=>`<div class="ob-dot ${t<n?`done`:t===n?`active`:``}"></div>`).join(``),a=``,o=``,s=``;if(r===1)a=`
      ...
      <button class="ob-back" onclick="obBack()">← Back</button>
      <button class="ob-next" onclick="obNext()">Continue →</button>`}else if(r===5)G.expenses.length||(G.expenses=Ld[G.homeType].map(e=>({...e,skipped:!1})),G.kids>0&&G.expenses.push({name:`Kids activities / sport`,category:`Childcare / Education`,amount:300,skipped:!1})),a=`
      <div class="ob-step-dots">${i}</div>
    ...
    <div class="ob-footer">${s}</div>`}function Jd(){let e=Rd(),t=zd();if(t>=e.length-1){Qd();return}if(G.step===2){if(G.adultNames.some(e=>!e.trim())){G._nameError=!0,qd();return}G._nameError=!1}let n=e[t+1];n===3&&(G.kidProfiles=Array.from({length:G.kids},(e,t)=>G.kidProfiles[t]||{name:``,age:``,emoji:KID_EMOJIS[t%KID_EMOJIS.length]})),n===5&&!G.expenses.length&&(G.expenses=Ld[G.homeType].map(e=>({...e,skipped:!1})),G.kids>0&&G.expenses.push({name:`Kids activities / sport`,category:`Childcare / Education`,amount:300,skipped:!1})),G.step=n,qd()}function Yd(){let e=Rd(),t=zd();t<=0||(G.step=e[t-1],qd())}function Xd(){s.onboarded=!0,window.saveData(s),Vd()}function Zd(){G.expenses=[];let e=Rd(),t=zd();G.step=e[t+1],qd()}function Qd(){s.householdProfile.members=[];for(let e=0;e<G.adults;e++)s.householdProfile.members.push({role:`adult`,name:G.adultNames[e]||``,age:G.adultAges[e]?parseInt(G.adultAges[e]):null});s.kids||(s.kids={profiles:[],chores:[],prizes:[],completions:[],redemptions:[]}),s.meals||(s.meals={}),s.meals.lunchbox||(s.meals.lunchbox={profiles:[]}),s.meals.lunchbox.profiles||(s.meals.lunchbox.profiles=[]),s.kids.profiles=[],s.meals.lunchbox.profiles=[],G.kidProfiles.forEach(e=>{if(s.householdProfile.members.push({role:`child`,name:e.name,age:e.age?parseInt(e.age):null,emoji:e.emoji}),!e.name)return;let t=nextId(s.kids.profiles);s.kids.profiles.push({id:t,name:e.name,age:e.age?parseInt(e.age):null,emoji:e.emoji}),s.meals.lunchbox.profiles.push({id:t,name:e.name,emoji:e.emoji})}),s.householdProfile.homeType=G.homeType;let e=window.ensureMonthOverride(window.selectedBudgetMonth);G.incomes.forEach((t,n)=>{let r=parseFloat(t.amount);if(!r)return;let i=t.frequency===`Weekly`?r*52/12:t.frequency===`Fortnightly`?r*26/12:t.frequency===`Annual`?r/12:r,a=t.name||(G.adultNames[n]?`${G.adultNames[n]}'s salary`:`Income ${n+1}`);e.income.push({id:nextId(e.income),name:a,amount:i,frequency:`monthly`,category:`Salary`})}),G.expenses.forEach(t=>{t.skipped||!t.amount||e.expenses.push({id:nextId(e.expenses),name:t.name,amount:t.amount,frequency:`monthly`,category:t.category,recurring:!0})}),s.onboarded=!0,window.saveData(s),Vd(),window.renderAll()}document.getElementById(`btn-guest-mode`)?.addEventListener(`click`,()=>mf()),document.getElementById(`btn-google-signin`)?.addEventListener(`click`,()=>pf());function $d(){document.getElementById(`sidebar`).classList.toggle(`collapsed`)}var ef={};function tf(e){let t=document.getElementById(`icon-group-${e}`),n=document.getElementById(`icon-group-${e}-children`),r=document.getElementById(`text-group-${e}`),i=document.getElementById(`text-group-${e}-children`),a=n&&n.classList.contains(`open`);t&&t.classList.toggle(`open`,!a),n&&n.classList.toggle(`open`,!a),r&&r.classList.toggle(`open`,!a),i&&i.classList.toggle(`open`,!a)}function nf(e){for(let[t,n]of Object.entries(ef))if(n.includes(e)){let e=document.getElementById(`icon-group-${t}-children`);e&&!e.classList.contains(`open`)&&tf(t);return}}window.addEventListener(`unhandledrejection`,e=>{console.error(`[unhandledrejection]`,e.reason)}),window.addEventListener(`error`,e=>{console.error(`[uncaughtError]`,e.message,e.filename,e.lineno)});var rf=`toto_household_owner`,af=`toto_pending_household`;function of(){return sessionStorage.getItem(af)||Ff(rf)||K?.uid||null}function sf(){let e=of();return e?fbStore.collection(`families`).doc(e):null}function cf(e){If(rf,e),sessionStorage.removeItem(af)}var K=null,lf=!1,uf=null;Object.defineProperty(window,`_fsUnsubscribe`,{get(){return uf},set(e){uf=e},configurable:!0});var df=null;Object.defineProperty(window,`_pendingLogEntry`,{get(){return df},set(e){df=e},configurable:!0}),Object.defineProperty(window,`_currentUser`,{get(){return K},set(e){K=e},configurable:!0});function ff(e,t){df={ts:new Date().toISOString(),name:K&&(K.displayName||K.email)||`Unknown`,photo:K&&K.photoURL||``,action:e,detail:t||``}}function pf(){let e=new fbAuth.GoogleAuthProvider;fbAuth.signInWithPopup(e).catch(e=>{let t=document.getElementById(`login-error`);t&&(t.textContent=e.message,t.style.display=``)})}async function mf(){lf=!0;let e=document.getElementById(`login-overlay`);e&&e.classList.add(`hidden`),await Wf,s.onboarded||(s.onboarded=!0,q(s));let t=location.hash.slice(1);t&&document.getElementById(`tab-`+t)?(history.replaceState({tab:t},``,`#`+t),m(t)):pm(),Kf()}function hf(){uf&&(uf(),uf=null),kf=!1,J=null,Uf(),fbAuth.signOut()}function gf(e){return e.budget||(e.budget={income:[],expenses:[],actuals:{},months:{}}),e.budget.actuals||(e.budget.actuals={}),e.budget.months||(e.budget.months={}),e.budget.suggestions||(e.budget.suggestions=[]),e.goals||(e.goals=[]),e.scenarios||(e.scenarios=[]),e.furniture||(e.furniture=[]),e.appliances||(e.appliances=[]),e.planner||(e.planner={events:[]}),e.lists||(e.lists={food:{items:[],budget:0,weeklyBudget:200,stores:[],favourites:[],history:[]},clothes:{items:[],budget:0,weeklyBudget:0,stores:[],favourites:[],history:[]},wishlist:{items:[],budget:0,weeklyBudget:0,stores:[],favourites:[],history:[]},home:{items:[],budget:0,weeklyBudget:0,stores:[],favourites:[],history:[]},pharmacy:{items:[],budget:0,weeklyBudget:0,stores:[],favourites:[],history:[]}},e.meals&&e.meals.shopping&&e.meals.shopping.length&&(e.lists.food.items=e.meals.shopping.map(function(e,t){return{id:`si-`+t,name:e.name,quantity:1,unit:`units`,notes:``,aisle:e.cat||`other`,state:e.checked?`got_it`:`active`,addedBy:`migration`,addedAt:new Date().toISOString(),mealTag:null,manualPrice:null,barcodeId:null}}))),[`food`,`clothes`,`wishlist`,`home`,`pharmacy`].forEach(function(t){e.lists[t]||(e.lists[t]={items:[],budget:0,weeklyBudget:0,stores:[],favourites:[],history:[]}),e.lists[t].items||(e.lists[t].items=[]),e.lists[t].stores||(e.lists[t].stores=[]),e.lists[t].favourites||(e.lists[t].favourites=[]),e.lists[t].history||(e.lists[t].history=[])}),e.routines&&e.routines.forEach(function(e){e.ownerType===`household`&&e.ownerId!==`household`&&(e.ownerId=`household`)}),e}function _f(){uf&&uf(),!Ff(rf)&&!sessionStorage.getItem(af)&&cf(K.uid);let e=sf();if(!e){console.error(`No household doc ref`);return}uf=e.onSnapshot(async t=>{if(t.exists){let e=gf(t.data());Object.assign(s,e),If(yf,JSON.stringify(s))}else{let t=await fbStore.collection(`family`).doc(`shared`).get().catch(()=>null);if(t&&t.exists){let n=gf(t.data());Object.assign(s,n),e.set(s).catch(()=>{}),If(yf,JSON.stringify(s))}else e.set(s).catch(()=>{})}bd().then(()=>{or();let e=location.hash.slice(1);e&&document.getElementById(`tab-`+e)?(history.replaceState({tab:e},``,`#`+e),m(e)):pm(),s.onboarded||(s.onboarded=!0,q(s));let t=sessionStorage.getItem(gp),n=sessionStorage.getItem(`toto_post_invite_action`);t||n?(sessionStorage.removeItem(`toto_post_invite_action`),vp()):Kf()})},e=>{console.error(`Firestore sync error:`,e),pm()})}function vf(){fbAuth.onAuthStateChanged(e=>{if(lf&&!e)return;K=e;let t=document.getElementById(`login-overlay`),n=document.getElementById(`header-avatar`),r=document.getElementById(`header-sign-out`);if(e)t&&t.classList.add(`hidden`),n&&(n.src=e.photoURL||``,n.style.display=`block`),r&&(r.style.display=`block`),_f();else{t&&t.classList.remove(`hidden`),n&&(n.style.display=`none`),r&&(r.style.display=`none`),uf&&(uf(),uf=null);let e=sessionStorage.getItem(gp),i=document.getElementById(`login-invite-banner`),a=document.getElementById(`login-tagline`);e&&i&&(i.style.display=`block`,a&&(a.style.display=`none`))}})}vf();var yf=`home_finance_v1`;window.STORAGE_KEY=yf;var bf=null,xf={};function Sf(e,t,n,r){xf[e]={options:t,value:n,onChange:r};let i=t.find(e=>(e.value??e)===n)||t[0],a=i?.label??i?.value??i??``;return`<div class="cs-wrap">
  ...
  </div>`,document.getElementById(`profile-list`).innerHTML=n,document.getElementById(`profile-overlay-title`).textContent=`Who uses this device?`,document.getElementById(`profile-overlay-sub`).textContent=`Set it once — the app will open straight to the right view. You can change this any time in Settings.`,document.getElementById(`profile-overlay`).classList.remove(`hidden`)}function Jf(e){if(zf(e),kf=!0,document.getElementById(`profile-overlay`).classList.add(`hidden`),e===`adult`)J=null,np(),pm();else if(e===`shared`)Yf();else{let t=(s.kids?.profiles||[]).find(t=>t.id===e);if(!t){J=null,zs(),pm();return}t.pinHash?(jf=t.id,X=``,Y=0,Qf(t)):(J={id:t.id,name:t.name,emoji:t.emoji,role:`child`},Hf(t.id),rp())}}function Yf(){document.getElementById(`pin-overlay`).classList.add(`hidden`);let e=(s.householdProfile?.members||[]).filter(e=>e.role===`adult`&&e.name),t=s.kids?.profiles||[],n=``;e.forEach((e,t)=>{let r=!!e.pinHash;n+=`<div class="profile-card" onclick="_pickAdult(${t})">
      <div class="profile-avatar">🧑</div>
      <div style="flex:1">
        <div style="font-size:15px;font-weight:700;color:#1e293b">${g(e.name)}</div>
    ...
    </div>`}),document.getElementById(`profile-list`).innerHTML=n,document.getElementById(`profile-overlay-title`).textContent=`Who's using Toto?`,document.getElementById(`profile-overlay-sub`).textContent=`Tap your name to continue`,document.getElementById(`profile-overlay`).classList.remove(`hidden`)}function Xf(e){let t=(s.householdProfile?.members||[]).filter(e=>e.role===`adult`&&e.name)[e??0];if(t?.pinHash){jf=`adult:`+(e??0),X=``,Y=0,Af=0,Qf({emoji:`🧑`,name:t.name,_isAdult:!0,_memberIndex:e??0});return}J=null,Uf(),document.getElementById(`profile-overlay`).classList.add(`hidden`),rp()}function Zf(e){let t=(s.kids?.profiles||[]).find(t=>String(t.id)===String(e));t&&(t.pinHash?(jf=e,X=``,Y=0,Qf(t)):(J={id:t.id,name:t.name,emoji:t.emoji,role:`child`},Hf(t.id),document.getElementById(`profile-overlay`).classList.add(`hidden`),rp()))}function Qf(e){document.getElementById(`profile-overlay`).classList.add(`hidden`),document.getElementById(`pin-avatar`).textContent=e.emoji||(e._isAdult?`🧑`:`😊`),document.getElementById(`pin-greeting`).textContent=`Hi ${e.name}! 👋`,document.getElementById(`pin-sub`).textContent=`Enter your PIN to continue`,document.getElementById(`pin-error`).textContent=``,$f(),ep(),document.getElementById(`pin-overlay`).classList.remove(`hidden`)}function $f(){let e=document.getElementById(`pin-dots`);e&&(e.innerHTML=[0,1,2,3].map(e=>`<div class="pin-dot ${e<X.length?`filled`:``}">${e<X.length?`●`:``}</div>`).join(``))}function ep(){let e=document.getElementById(`pin-pad`);if(!e)return;let t=Date.now();if(t<Af){let n=Math.ceil((Af-t)/1e3);document.getElementById(`pin-error`).textContent=`Too many attempts — try again in ${n}s`,e.innerHTML=``,setTimeout(ep,1e3);return}e.innerHTML=[1,2,3,4,5,6,7,8,9,``,0,`⌫`].map(e=>e===``?`<div class="pin-key empty"></div>`:`<div class="pin-key" onclick="_pinKey('${e}')">${e}</div>`).join(``)}function tp(e){if(!(Date.now()<Af)){if(e===`⌫`){X=X.slice(0,-1),$f();return}X.length>=4||(X+=e,$f(),X.length===4&&Qp())}}function np(){let e=document.getElementById(`header-switch-profile`);if(!e)return;let t=Rf();e.style.display=t&&t!==`adult`?``:`none`}function rp(){np(),zs(),J?.role===`child`?(window.kidsView=J.id,h(`kids`),nc(J.id)):document.querySelectorAll(`.nav-item, .nav-text-item`).forEach(e=>e.style.display=``),pm()}var ip=`member`,ap=null,op=null;function sp(e){ip=e;let t=document.getElementById(`inv-role-member`),n=document.getElementById(`inv-role-owner`);!t||!n||(e===`member`?(t.style.borderColor=`#0d9488`,t.style.background=`#f0fdfa`,t.style.color=`#0d9488`,n.style.borderColor=`var(--border)`,n.style.background=`var(--surface)`,n.style.color=`var(--text-muted)`):(n.style.borderColor=`#0d9488`,n.style.background=`#f0fdfa`,n.style.color=`#0d9488`,t.style.borderColor=`var(--border)`,t.style.background=`var(--surface)`,t.style.color=`var(--text-muted)`))}function cp(e,t){let n=document.getElementById(`inv-email`),r=n?n.value.trim():``,i=(s.householdProfile.members||[]).find(e=>e.role===`adult`)?.name||`Someone`,a=new Date(Date.now()+10080*60*1e3).toISOString(),o={id:mm(),email:r,role:ip,inviterName:i,memberName:t||null,memberIdx:e??null,createdAt:new Date().toISOString(),expiresAt:a,status:`pending`};s.householdProfile.invites||(s.householdProfile.invites=[]),s.householdProfile.invites.push(o),q(s),ap=o.id;let c=fp(o.id),l=document.getElementById(`invite-link-wrap`),u=document.getElementById(`invite-form-wrap`),d=document.getElementById(`invite-link-display`);l&&(l.dataset.invEmail=r,l.dataset.invToken=o.id,l.style.display=`block`),u&&(u.style.display=`none`),d&&(d.textContent=c)}function lp(e){let t=(s.householdProfile.members||[])[e];if(!t)return;let n=(s.householdProfile.members||[]).find((e,t)=>e.role===`adult`&&t===0)?.name||`Someone`,r=new Date(Date.now()+10080*60*1e3).toISOString(),i={id:mm(),email:``,role:`member`,inviterName:n,memberName:t.name||null,memberIdx:e,createdAt:new Date().toISOString(),expiresAt:r,status:`pending`};s.householdProfile.invites||(s.householdProfile.invites=[]),s.householdProfile.invites.push(i),q(s);let a=fp(i.id),o=g(t.name||`this person`),c=document.createElement(`div`);c.style.cssText=`position:fixed;inset:0;background:rgba(0,0,0,.5);z-index:9000;display:flex;align-items:center;justify-content:center;padding:24px`,c.innerHTML=`<div style="background:#fff;border-radius:16px;padding:24px;max-width:400px;width:100%">
  ...
  </div>`,document.body.appendChild(c),c.addEventListener(`click`,e=>{e.target===c&&(c.remove(),qi())})}function up(e){if(!e)return;let t=(s.kids?.profiles||[]).find(t=>t.name&&t.name.toLowerCase()===e.toLowerCase());if(!t){s.kids||(s.kids={profiles:[],chores:[],prizes:[],completions:[],redemptions:[]});let n=(s.householdProfile.members||[]).find(t=>t.name&&t.name.toLowerCase()===e.toLowerCase()),r=k(s.kids.profiles);t={id:r,name:e,age:n?.age||null,emoji:n?.emoji||`🧒`},s.kids.profiles.push(t),s.meals?.lunchbox?.profiles||(s.meals||(s.meals={}),s.meals.lunchbox||(s.meals.lunchbox={profiles:[]})),s.meals.lunchbox.profiles.push({id:r,name:e,emoji:t.emoji}),q(s)}zp(t.id)}function dp(e){navigator.clipboard.writeText(fp(e)).then(()=>{let e=event.target,t=e.textContent;e.textContent=`✓ Copied!`,setTimeout(()=>{e.textContent=t},2e3)})}function fp(e){let t=of()||``;return window.location.origin+window.location.pathname+`?invite=`+e+`&h=`+t}function pp(){let e=document.getElementById(`invite-link-wrap`)?.dataset.invToken||ap;e&&navigator.clipboard.writeText(fp(e)).then(()=>{let e=event.target,t=e.textContent;e.textContent=`✓ Copied!`,setTimeout(()=>{e.textContent=t},2e3)})}function mp(){let e=document.getElementById(`invite-link-wrap`),t=e?.dataset.invToken||ap,n=e?.dataset.invEmail||``;if(!t)return;let r=(s.householdProfile.invites||[]).find(e=>e.id===t)?.inviterName||`Your partner`,i=fp(t),a=encodeURIComponent(`${r} invited you to join their Toto household`),o=encodeURIComponent(`Hi,\n\n${r} has invited you to join their Toto household — a shared family finance and planning app.\n\nClick the link below to accept the invite (expires in 7 days):\n\n${i}\n\nSee you there!`);window.open(`mailto:${n}?subject=${a}&body=${o}`,`_blank`)}function hp(e){if(!confirm(`Revoke this invite link?`))return;let t=(s.householdProfile.invites||[]).find(t=>t.id===e);t&&(t.status=`revoked`),q(s),qi()}var gp=`toto_pending_invite`;function _p(){let e=new URLSearchParams(window.location.search),t=e.get(`invite`),n=e.get(`h`);t&&(sessionStorage.setItem(gp,t),n&&sessionStorage.setItem(af,n),window.history.replaceState({},``,window.location.pathname+window.location.hash))}function vp(){let e=sessionStorage.getItem(gp);if(!e)return;let t=(s.householdProfile.invites||[]).find(t=>t.id===e);if(!t){sessionStorage.removeItem(gp);return}if(t.status!==`pending`||new Date(t.expiresAt)<new Date){sessionStorage.removeItem(gp),Ap(t);return}sessionStorage.removeItem(gp),op=t,yp(t)}function yp(t){let n=s.householdProfile.members||[],r=n.filter(e=>e.role===`adult`),i=n.filter(e=>e.role===`child`),a=(()=>{let t=new Date,n=`${t.getFullYear()}-${String(t.getMonth()+1).padStart(2,`0`)}`,r=getOrCreateMonthData(n);return(r.income||[]).reduce((t,n)=>t+e(Number(n.amount)||0,n.frequency),0)-(r.expenses||[]).filter(e=>!e.skipped).reduce((t,n)=>t+e(Number(n.amount)||0,n.frequency),0)})(),o=[...r.map(e=>`
  ...
  `)}var xp=0,Sp=[{emoji:`💰`,title:`The Kitty`,desc:`Budget, bills, goals & net worth. See where the money goes each month.`},{emoji:`📅`,title:`Plan`,desc:`Weekly planner, meal plans & the kids' lunchboxes. One place for the week ahead.`},{emoji:`🏠`,title:`Home`,desc:`Documents, vehicles & maintenance reminders. Never miss a rego or warranty renewal.`}];function Cp(){xp=0,wp()}function wp(){let e=Sp[xp],t=Sp.map((e,t)=>`<div style="width:${t===xp?20:8}px;height:8px;border-radius:99px;background:${t===xp?`#0d9488`:`#e2e8f0`};transition:all .2s"></div>`).join(``),n=xp===Sp.length-1;jp(`
  ...
  `)}function Ep(){let e=document.getElementById(`inv-inc-name`)?.value.trim()||``,t=parseFloat(document.getElementById(`inv-inc-amount`)?.value)||0,n=document.getElementById(`inv-inc-freq`)?.value||`Monthly`;(e||t)&&(s.budget.income||(s.budget.income=[]),s.budget.income.push({id:k(s.budget.income),name:e,amount:t,frequency:n}),q(s)),kp()}function Dp(){let e=op;if(!K){sessionStorage.setItem(`toto_post_invite_action`,e?.id||``),pf();return}Op(e),bp()}function Op(e){if(!e)return;e.status=`accepted`;let t=K,n=of();n&&cf(n),s.householdProfile.authorizedUsers||(s.householdProfile.authorizedUsers=[]),s.householdProfile.authorizedUsers.some(e=>e.uid===t.uid)||s.householdProfile.authorizedUsers.push({uid:t.uid,name:t.displayName||``,email:t.email||``,role:e.role||`member`,joinedAt:new Date().toISOString()});let r=s.householdProfile.members||[],i=(t.displayName||``).split(` `)[0];i&&!r.some(e=>e.name===i)&&s.householdProfile.members.push({role:`adult`,name:i,age:null}),q(s)}function kp(){op=null,Mp(),pm(),Kf()}function Ap(e){jp(`
  ...
  `)}function jp(e){document.getElementById(`invite-flow-content`).innerHTML=e;let t=document.getElementById(`invite-flow-overlay`);t.classList.remove(`hidden`),t.style.display=`flex`,t.scrollTop=0}function Mp(){let e=document.getElementById(`invite-flow-overlay`);e.classList.add(`hidden`),e.style.display=``}var Np=`toto_pin_hard_`,Pp=`toto_pin_att_`,Fp=null,Z=`gate`,Ip=``,Q=``,Lp=0,Rp=null;function zp(e){let t=(s.kids?.profiles||[]).find(t=>String(t.id)===String(e));t&&(Fp=e,Q=``,Ip=``,Z=t.pinHash?`enter`:`gate`,Vp(),document.getElementById(`pin-setup-overlay`).classList.remove(`hidden`))}function Bp(){Rp&&(clearInterval(Rp),Rp=null),document.getElementById(`pin-setup-overlay`).classList.add(`hidden`),Fp=null}function Vp(){let e=(s.kids?.profiles||[]).find(e=>String(e.id)===String(Fp));if(!e)return;let t=document.getElementById(`pso-screen`);if(!t)return;let n=e.emoji||`🧒`,r=g(e.name),i={gate:`#f8fafc`,hello:`linear-gradient(160deg,#f0fdfa,#ecfeff)`,enter:`#f8fafc`,confirm:`#f8fafc`,tour:`#f8fafc`};if(document.getElementById(`pin-setup-overlay`).style.background=i[Z]||`#f8fafc`,Z===`gate`)t.innerHTML=`
      ...
      ${r?``:`<button onclick="_psoTourDone()" style="font-size:13px;color:#94a3b8;background:none;border:none;cursor:pointer">Skip</button>`}`}}function Hp(){if(Rp)return;let e=0,t=document.getElementById(`pso-hold-fill`);t&&(t.style.transition=`none`,t.style.transform=`scaleX(0)`),Rp=setInterval(()=>{e+=50;let n=Math.min(e/2e3,1);t&&(t.style.transition=`none`,t.style.transform=`scaleX(${n})`),e>=2e3&&(clearInterval(Rp),Rp=null,Z=`hello`,Vp())},50)}function Up(){Rp&&(clearInterval(Rp),Rp=null);let e=document.getElementById(`pso-hold-fill`);e&&(e.style.transition=`transform .3s`,e.style.transform=`scaleX(0)`)}function Wp(e){if(e===`⌫`){Q=Q.slice(0,-1),Vp();return}Q.length>=4||(Q+=e,Vp(),Q.length===4&&Gp())}async function Gp(){if(Z===`enter`)Ip=Q,Q=``,Z=`confirm`,Vp();else if(Z===`confirm`){if(Q!==Ip){Q=``,Ip=``,Z=`enter`,Vp();let e=document.getElementById(`pso-error`);e&&(e.textContent=`Those didn't match — try again 🙈`);return}await Vs(Fp,Q);let e=(s.kids?.profiles||[]).find(e=>String(e.id)===String(Fp));e&&!e._pinWasSet?(Lp=0,Z=`tour`,Vp()):(Bp(),W(),qi()),e._pinWasSet=!0}}function Kp(){let e=(s.kids?.profiles||[]).find(e=>String(e.id)===String(Fp));Bp(),e&&nc(e.id)}function qp(e){return parseInt(Ff(Pp+e)||`0`)}function Jp(e){let t=qp(e)+1;return If(Pp+e,String(t)),t}function Yp(e){Lf(Pp+e),Lf(Np+e)}function Xp(e){return Ff(Np+e)===`1`}function Zp(e){If(Np+e,`1`)}async function Qp(){let e=await Gf(X,of());if(typeof jf==`string`&&jf.startsWith(`adult:`)){let t=parseInt(jf.split(`:`)[1]),n=(s.householdProfile?.members||[]).filter(e=>e.role===`adult`&&e.name)[t];if(!n)return;e===n.pinHash?(Y=0,J=null,Uf(),document.getElementById(`pin-overlay`).classList.add(`hidden`),rp()):(Y++,X=``,$f(),Y>=3?(Af=Date.now()+3e4,Y=0,ep()):document.getElementById(`pin-error`).textContent=`Wrong PIN — ${3-Y} tr${3-Y==1?`y`:`ies`} left`);return}let t=(s.kids?.profiles||[]).find(e=>e.id===jf);if(t){if(Xp(t.id)){document.getElementById(`pin-error`).textContent=`PIN locked — ask mum or dad to reset it 🔒`,document.getElementById(`pin-pad`).innerHTML=``,$p(t);return}if(e===t.pinHash)Yp(t.id),Y=0,J={id:t.id,name:t.name,emoji:t.emoji,role:`child`},Hf(t.id),document.getElementById(`pin-overlay`).classList.add(`hidden`),rp();else if(X=``,$f(),Y++,Jp(t.id)>=10)Zp(t.id),Af=0,document.getElementById(`pin-error`).textContent=`PIN locked — ask mum or dad to reset it 🔒`,document.getElementById(`pin-pad`).innerHTML=``,$p(t);else if(Y>=3)Af=Date.now()+3e4,Y=0,ep();else{let e=3-Y;document.getElementById(`pin-error`).textContent=`Wrong PIN — ${e} try${e===1?``:`s`} left`}}}function $p(e){s.notifications||(s.notifications=[]),s.notifications.some(t=>t.type===`pin-lock`&&t.kidId===e.id)||(s.notifications.unshift({id:mm(),type:`pin-lock`,kidId:e.id,msg:`${e.name}'s PIN has been locked after too many failed attempts. Reset it in Settings → Household.`,ts:new Date().toISOString(),read:!1}),q(s))}function em(e){Yp(e),Y=0,Af=0,qi()}function tm(e){return s.budget.months&&s.budget.months[e]||{income:s.budget.income,expenses:s.budget.expenses}}function nm(e){return!!(s.budget.months&&s.budget.months[e])}function rm(e){return s.budget.months||(s.budget.months={}),s.budget.months[e]||(s.budget.months[e]={income:JSON.parse(JSON.stringify(s.budget.income)),expenses:JSON.parse(JSON.stringify(s.budget.expenses))}),s.budget.months[e]}function im(e){let[t,n]=e.split(`-`).map(Number),r=new Date(t,n-2,1);return`${r.getFullYear()}-${String(r.getMonth()+1).padStart(2,`0`)}`}function am(e){let t=im(e),n=tm(t);s.budget.months||(s.budget.months={}),s.budget.months[e]={income:JSON.parse(JSON.stringify(n.income.filter(e=>e.recurring!==!1))),expenses:JSON.parse(JSON.stringify(n.expenses.filter(e=>e.recurring!==!1)))},ff(`Auto-filled`,`${xm(e)} copied from ${xm(t)}`),q(s),um(No),um(Ei)}var om=null;Object.defineProperty(window,`_scopePending`,{get(){return om},set(e){om=e},configurable:!0});function sm(e,t){om={onThisMonth:e,onAllMonths:t};let n=xm($);document.getElementById(`modal-title`).textContent=`Apply changes`,document.getElementById(`modal-body`).innerHTML=`
    <p style="font-size:14px;line-height:1.6;margin:0;color:var(--text-muted)">
      Apply this change to <strong style="color:var(--text)">${n}</strong> only,
      or update the default for all months?
    </p>
    ${nm($)?`<p style="font-size:12px;color:var(--primary);margin-top:10px;margin-bottom:0">
  ...
  `,document.getElementById(`modal-overlay`).classList.remove(`hidden`)}function cm(){om&&(om.onThisMonth(),om=null),Io()}function lm(){om&&(om.onAllMonths(),om=null),Io()}function um(e){try{e()}catch(t){let n=t?.message||String(t),r=t?.stack||``;console.error(`Render error in `+(e.name||`?`)+`: `+n+`
`+r),typeof Sentry<`u`&&Sentry.withScope(n=>{n.setTag(`renderer`,e.name||`anonymous`),Sentry.captureException(t)})}}var dm={today:[()=>Ii()],money:[()=>Ei()],dashboard:[()=>Ki()],budget:[()=>No()],bills:[()=>gt()],networth:[()=>Ie()],goals:[()=>ra()],scenarios:[()=>ma()],insights:[()=>eo()],build:[()=>$i()],settings:[()=>qi()],kids:[()=>W()],planner:[()=>F()],forecast:[()=>Jt()],meals:[()=>Nr()],lunchbox:[()=>Uc()],pantry:[()=>De()],vehicles:[()=>ne()],documents:[()=>de()],maintenance:[()=>ye()],routines:[()=>U()],lists:[()=>ci()]},fm=!1;function pm(){zs(),um(Ii);let e=dm[f()];e&&e.forEach(e=>um(e)),fm||(fm=!0,window.Capacitor?.Plugins?.SplashScreen?.hide().catch(()=>{}))}a(pm),u(pm);function mm(){return Date.now().toString(36)+Math.random().toString(36).slice(2)}o(Ef()),_p(),setTimeout(()=>{try{ml()}catch{}},0);var $=new Date().toISOString().slice(0,7),hm=`grouped`;Object.defineProperty(window,`budgetViewMode`,{get(){return hm},set(e){hm=e},configurable:!0}),Object.defineProperty(window,`selectedBudgetMonth`,{get(){return $},set(e){$=e},configurable:!0}),window._settingsOpen=new Set([`ai`,`household`]);var gm=`all`;Object.defineProperty(window,`_billsSubsFilter`,{get(){return gm},set(e){gm=e},configurable:!0});var _m=new Date().getFullYear();Object.defineProperty(window,`dpViewYear`,{get(){return _m},set(e){_m=e},configurable:!0});var vm=new Date().getMonth()+1;Object.defineProperty(window,`dpViewMonth`,{get(){return vm},set(e){vm=e},configurable:!0});function ym(){let[e,t]=$.split(`-`).map(Number),n=new Date(e,t-2,1);$=`${n.getFullYear()}-${String(n.getMonth()+1).padStart(2,`0`)}`,No()}function bm(){let[e,t]=$.split(`-`).map(Number),n=new Date(e,t,1);if($=`${n.getFullYear()}-${String(n.getMonth()+1).padStart(2,`0`)}`,s.settings&&s.settings.autoFillMonths&&!nm($)){am($);return}No()}function xm(e){let[t,n]=e.split(`-`).map(Number);return new Date(t,n-1,1).toLocaleDateString(`en-AU`,{month:`long`,year:`numeric`})}function Sm(e){let[t,n]=e.split(`-`).map(Number);return new Date(t,n-1,1).toLocaleDateString(`en-AU`,{month:`short`,year:`2-digit`})}function Cm(){let e=[],t=new Date;for(let n=5;n>=0;n--){let r=new Date(t.getFullYear(),t.getMonth()-n,1);e.push(`${r.getFullYear()}-${String(r.getMonth()+1).padStart(2,`0`)}`)}return e}function wm(e,t){let n=(s.budget.actuals[t]||{})[e];return n?typeof n==`number`?[{id:1,amount:n,date:``,note:``}]:Array.isArray(n)?n:[]:[]}function Tm(e,t){return wm(e,t).reduce((e,t)=>e+(t.amount||0),0)}function Em(e,t,n){s.budget.actuals[t]||(s.budget.actuals[t]={});let r=wm(e,t);r.length===1&&!r[0].date&&!r[0].note?s.budget.actuals[t][e]=[{...r[0],amount:n}]:s.budget.actuals[t][e]=[{id:1,amount:n,date:``,note:``}],q(s)}function Dm(e){let t=tm($).expenses.find(t=>t.id===e);if(!t)return;let n=D(t);function r(){let t=wm(e,$),r=t.reduce((e,t)=>e+(t.amount||0),0),i=n>0?Math.round(r/n*100):0,a=i>=100?`var(--danger)`:i>=80?`var(--warning)`:`var(--success)`;return`
      ...
      </div>`:`<p style="font-size:13px;color:var(--text-muted);margin-bottom:14px">No entries yet for ${xm($)}.</p>`}
    ...
    `}document.getElementById(`modal-title`).textContent=`Actuals — ${t.name}`,document.getElementById(`modal-body`).innerHTML=`<div id="actual-editor-body">${r()}</div>`,document.getElementById(`modal-footer`).innerHTML=`<button class="btn btn-primary" onclick="closeModal();renderBudget()">Done</button>`,document.getElementById(`modal-overlay`).classList.remove(`hidden`)}function Om(e){let t=parseFloat(document.getElementById(`ae-amount`).value);if(!t||t<=0)return;let n=document.getElementById(`ae-date`).value||``,r=document.getElementById(`ae-note`).value.trim();s.budget.actuals[$]||(s.budget.actuals[$]={});let i=wm(e,$);i.push({id:i.length?Math.max(...i.map(e=>e.id))+1:1,amount:t,date:n,note:r}),s.budget.actuals[$][e]=i,q(s),window._actualEditorRefresh&&window._actualEditorRefresh()}function km(e,t){if(!s.budget.actuals[$])return;let n=wm(e,$);n.splice(t,1),s.budget.actuals[$][e]=n,q(s),window._actualEditorRefresh&&window._actualEditorRefresh()}function Am(e){Dm(e)}function jm(){let e=tm($),t=O(e.income),n=O(e.expenses),r=t-n,i=t>0?r/t:null,a;a=t===0?10:i>=.2?20:i>=0?Math.round(i/.2*20):0;let o=Cm().slice(3),c=0,l=0;o.forEach(e=>{let t=tm(e);t.expenses.some(t=>wm(t.id,e).length>0)&&(c++,t.expenses.reduce((t,n)=>t+Tm(n.id,e),0)<=O(t.expenses)&&l++)});let u=c===0?5:Math.round(c/3*10+l/c*10),d=(s.netWorth.assets||[]).reduce((e,t)=>e+(parseFloat(t.value)||0),0),f=(s.netWorth.liabilities||[]).reduce((e,t)=>e+(parseFloat(t.value)||0),0),p=d-f,m;if(d===0&&f===0)m=10;else if(p<=0)m=3;else{let e=f>0?f/d:0;m=e<.3?20:e<.6?15:e<.8?10:6}let h;if(n===0)h=10;else{let e=d/n;h=e>=6?20:e>=3?15:e>=1?8:d===0?5:3}let g=(s.goals||[]).filter(e=>e.status!==`achieved`),_;if(g.length===0)_=8;else{let e=g.reduce((e,t)=>e+Math.min((t.saved||0)/(t.target||1),1),0)/g.length;_=10+Math.round(e*10)}let v=a+u+m+h+_,y=v>=85?`A`:v>=70?`B`:v>=55?`C`:v>=40?`D`:`F`,x=v>=80?`#10b981`:v>=60?`#f59e0b`:v>=40?`#f97316`:`#ef4444`,S=[...[{score:a,tip:`Boost your savings rate — aim for 20%+ of income (currently ${t>0?Math.round((i||0)*100):0}%)`},{score:u,tip:`Log actuals monthly in the Budget tab to stay on track`},{score:m,tip:`Reduce liabilities or grow assets to strengthen your net worth`},{score:h,tip:`Build an emergency fund of 3–6 months expenses (${b(n*3)}–${b(n*6)})`},{score:_,tip:`Set specific savings goals in the Goals tab to stay focused`}]].sort((e,t)=>e.score-t.score)[0];return{total:v,grade:y,color:x,insight:S.score<12?S.tip:`Great shape — stay consistent and keep building your financial cushion.`,dimensions:[{label:`Savings Rate`,score:a,max:20},{label:`Budget Tracking`,score:u,max:20},{label:`Net Worth`,score:m,max:20},{label:`Emergency Buffer`,score:h,max:20},{label:`Goals`,score:_,max:20}]}}var Mm={Wallet:`#059669`,Plan:`#0891b2`,Home:`#7c3aed`};function Nm(e){return e?`<span style="font-size:10px;font-weight:700;color:${Mm[e]||`#71717a`};margin-right:6px">${e}</span>`:``}function Pm(e){let t={red:`#ef4444`,amber:`#f59e0b`,green:`#10b981`,blue:`#0891b2`}[e.cls]||`#a1a1aa`,n=e.onclick?e.onclick:e.tab?`activateTab('${e.tab}')`:``;return`<div class="notif-tl-item">
    <div class="notif-tl-dot" style="background:${t}"></div>
    <div class="notif-tl-body"><div class="notif-tl-title">${Nm(e.section)}${e.title}</div>${e.sub?`<div class="notif-tl-sub">${e.sub}</div>`:``}</div>
    <div class="notif-tl-action" onclick="${n}">${e.action.replace(` →`,``)}</div>
  </div>`}function Fm(e){let t=[];return e===`budget`&&tm($).expenses.forEach(e=>{let n=D(e),r=Tm(e.id,$);n>0&&r/n>=.8&&r<n?t.push({cls:`amber`,text:`${g(e.name)} at ${Math.round(r/n*100)}% — ${b(n-r)} left`,tab:null}):n>0&&r>=n&&t.push({cls:`red`,text:`${g(e.name)} over budget by ${b(r-n)}`,tab:null})}),t.slice(0,2).map(e=>`<div class="notif-banner ${e.cls}">
    ...
    </div>`,r.addEventListener(`click`,t=>{t.target===r&&Rm(e.id)}),document.body.appendChild(r)}function Hm(){if(!s.settings?.typeAMode)return;let e=Lm();if(!e)return;let t=new Date().toISOString().slice(0,10);if(s.settings.typeAMissionId!==e.id){s.settings.typeAMissionId=e.id,s.settings.typeAMissionShownDate=t,q(s);return}Bm()>=1&&setTimeout(()=>Vm(e),1500)}function Um(){Im();let e=tm($),t=e.expenses.reduce((e,t)=>e+Tm(t.id,$),0),r=O(e.expenses),i=jr(1),a=s.meals?.plan?.[i]||{},o=Object.values(a).reduce((e,t)=>e+ +!!t.b+ +!!t.l+ +!!t.d,0),c=(s.maintenance||[]).filter(e=>{let t=ve(e);return t!==null&&t<0}).length,l=(s.bills||[]).filter(e=>{let t=n(e);return t>=0&&t<=7}),u=(s.meals?.pantry||[]).filter(e=>e.status===`need`||e.status===`low`).length,d=document.createElement(`div`);d.className=`reset-overlay`,d.innerHTML=`
    ...
    </div>`,d.addEventListener(`click`,e=>{e.target===d&&d.remove()}),document.body.appendChild(d)}function Wm(){let e=document.querySelector(`.reset-overlay`);e&&e.remove(),s.settings||(s.settings={});let t=new Date().toISOString().slice(0,10),n=s.settings.typeALastResetDate||``;n&&Math.floor((new Date-new Date(n))/864e5)<=9?s.settings.typeAStreak=(s.settings.typeAStreak||0)+1:s.settings.typeAStreak=1,s.settings.typeALastResetDate=t,q(s),Ii()}function Gm(){let e=Im(),t=2*Math.PI*22,n=(e.total/100*t).toFixed(1),r=e.total>=80?`var(--good,#10b981)`:e.total>=60?`var(--iris-2)`:e.total>=40?`var(--amber,#f59e0b)`:`var(--ember,#f97316)`,i=e.total>=80?`Crushing it — keep going`:e.total>=60?`Good shape — a few things to tidy`:e.total>=40?`Getting there — some gaps to fill`:`Just getting started`,a=Nt?`▲`:`▼`,o=e.dims.map(e=>{let t=e.score>=75?`var(--good,#10b981)`:e.score>=50?`var(--iris-2)`:e.score>=30?`var(--amber,#f59e0b)`:`var(--ember,#f97316)`,n=e.score>=75?`var(--good,#10b981)`:e.score>=50?`var(--iris-1)`:e.score>=30?`var(--amber,#f59e0b)`:`var(--ember,#f97316)`;return`<div class="life-dim">
  ...
  </div>`}function Km(){let e=Lm();if(!e)return``;let t=Bm(),n=e.onclick?`onclick="${e.onclick}"`:e.tab?`onclick="activateTab('${e.tab}')"`:``;return`<div class="mission-card" ${n}>
  ...
  </div>`}function qm(){let e=tm($),t=s.householdProfile?.members||[],n=t.filter(e=>e.role===`adult`),r=t.filter(e=>e.role===`child`),i=[];if(i.push({id:`members`,label:`Add your household members`,done:n.length>0,tab:null}),i.push({id:`income`,label:`Set up income sources`,done:(e.income||[]).length>0,tab:`budget`}),i.push({id:`expenses`,label:`Add monthly expenses`,done:(e.expenses||[]).length>0,tab:`budget`}),r.length>0&&i.push({id:`kids`,label:`Add kids to your household`,done:!0,tab:null}),n.length>=2){let e=n[1]?.name||`your partner`,t=(s.householdProfile.authorizedUsers||[]).length>0||(s.householdProfile.invites||[]).some(e=>e.status===`accepted`);i.push({id:`invite`,label:`Invite ${e} to your household`,done:t,tab:`settings`,settingsSection:`household-access`})}if(i.push({id:`goals`,label:`Set your first savings goal`,done:(s.goals||[]).length>0,tab:`goals`}),i.push({id:`networth`,label:`Add your net worth (assets & debts)`,done:(s.netWorth?.assets||[]).length>0||(s.netWorth?.liabilities||[]).length>0,tab:`networth`}),i.push({id:`vehicles`,label:`Add your vehicles`,done:(s.vehicles||[]).length>0,tab:`vehicles`}),r.length>0){let e=r[0]?.name||`your child`;i.push({id:`chores`,label:`Set up ${g(e)}'s first chores`,done:(s.kids?.chores||[]).length>0,tab:`kids`})}return i}var Jm=!1;Object.defineProperty(window,`_spExpanded`,{get(){return Jm},set(e){Jm=e},configurable:!0});var Ym=!1;Object.defineProperty(window,`_spDoneExpanded`,{get(){return Ym},set(e){Ym=e},configurable:!0});function Xm(){let e=document.getElementById(`setup-progress-card`);e&&(e.innerHTML=Di())}var Zm=null;Object.defineProperty(window,`expenseSortCol`,{get(){return Zm},set(e){Zm=e},configurable:!0});var Qm=`asc`;Object.defineProperty(window,`expenseSortDir`,{get(){return Qm},set(e){Qm=e},configurable:!0});var $m=`all`;Object.defineProperty(window,`expenseFilterCat`,{get(){return $m},set(e){$m=e},configurable:!0});function eh(e){$m=e,No()}function th(e){Qm=Zm===e&&Qm===`asc`?`desc`:`asc`,Zm=e,No()}function nh(e,t,n=``){let r=Zm===e;return`<th class="sortable${r?` sort-active`:``}" onclick="sortExpenses('${e}')">${t}${n}<span class="sort-icon">${r?Qm===`asc`?`↑`:`↓`:`↕`}</span></th>`}function rh(e){let t=[`th`,`st`,`nd`,`rd`],n=e%100;return e+(t[(n-20)%10]||t[n]||t[0])}var ih=`plan`;Object.defineProperty(window,`_mealView`,{get(){return ih},set(e){ih=e},configurable:!0}),window._acceptInviteAndContinue=Dp,window._addRecurrenceToDate=ar,window._adultPinKey=Ys,window._adultPinSubmit=Xs,window._applyActiveProfile=rp,window._applyChildNav=zs,window._applyMigrations=gf,window._autoCreateRecurringEvents=or,window._briefIcon=Ai,window._briefRow=Fi,window._budgetAllocByCategory=ko,window._buildTotoContext=vr,window._capacitorPrefs=Nf,window._categoryIcon=Mo,window._checkInviteOnLoad=_p,window._checkMissionEscalation=Hm,window._checkSettingsUnsaved=yo,window._chipClassFor=ji,window._chipLabelFor=Mi,window._copyInviteLinkForMember=dp,window._csOpen=Cf,window._csvSetExpense=Es,window._csvToggle=ws,window._csvToggleAll=Ts,window._csvUpdateApplyBtn=Ds,window._cvAgeBracket=Zs,window._cvCalViewToggle=yc,window._cvConfetti=tc,window._cvDismissNotif=Tc,window._cvEventsForDate=dc,window._cvFmt12h=fc,window._cvMonthDayTap=Cc,window._cvRefreshSchedulePanel=hc,window._cvRender7Day=bc,window._cvRenderCalendar=vc,window._cvRenderMonth=Sc,window._cvRenderPrizesTab=ic,window._cvRoutineAvailLabel=ec,window._cvRoutineIsActive=$s,window._cvRoutineSchedCard=_c,window._cvScheduleHtml=gc,window._cvShowDayDetail=wc,window._cvShowPrizeConfirm=Ec,window._cvSwitchTab=uc,window._cvTimeGreeting=Qs,window._cvToggleRoutineExpand=pc,window._cvToggleStepFromCal=mc,window._cvUpdatePrizesBadge=rc,window._cvWeekDayTap=xc,window._devLoadAll=Rc,window._devLoadHome=Ic,window._devLoadKids=Nc,window._devLoadMeals=Lc,window._devLoadPlanner=Fc,window._devLoadRoutines=Pc,window._devLoadWallet=Mc,window._devReset=zc,window._devToolsClose=jc,window._devToolsOpen=Ac,window._dismissInviteFlow=Mp,window._docCatMeta=ue,window._ensureKidProfileAndPin=up,window._ensureNWModals=We,window._estimateMealCalories=Ur,window._fetchAIBriefing=Gi,window._finishInviteJourney=kp,window._getHouseholdDocRef=sf,window._getHouseholdOwnerUID=of,window._getInviteUrl=fp,window._getNthDayOfMonth=nr,window._getPinTotalAttempts=qp,window._handlePendingInvite=vp,window._hashPin=Gf,window._incPinTotalAttempts=Jp,window._inferAisle=Dr,window._initAuthListener=vf,window._isPinHardLocked=Xp,window._listsAddFavourite=ai,window._listsAddItem=Xr,window._listsAddUsual=oi,window._listsArchive=ii,window._listsClearTrolley=ri,window._listsDeleteItem=Qr,window._listsOpenAddForm=ei,window._listsQuickAdd=$r,window._listsSaveForm=ni,window._listsSetState=Zr,window._listsUpdateParsePreview=si,window._maintDaysUntil=ve,window._maintNextDue=_e,window._markSettingsDirty=go,window._mealGetSuggestions=Vr,window._mealPill=Rr,window._mealPriceSlide=Br,window._mealToggleFilter=zr,window._mealWeekDates=Mr,window._mealWeekKey=jr,window._missionDaysIgnored=Bm,window._nextForecastMonth=qt,window._pantryToAisle=ti,window._parseShopInput=Or,window._pickAdult=Xf,window._pickKid=Zf,window._pinKey=tp,window._plannerCloseDaySheet=dn,window._plannerCloseDetail=vn,window._plannerCloseLifeSheet=hn,window._plannerCloseShare=Sn,window._plannerCopyShareUrl=wn,window._plannerEditFromDetail=bn,window._plannerEvMemberIds=Rt,window._plannerEvPrimaryMember=zt,window._plannerEvWhoLabel=Bt,window._plannerEventsForDate=Ut,window._plannerFmt12h=Wt,window._plannerGoToday=an,window._plannerHandleDaySheetClick=fn,window._plannerHandleDetailClick=yn,window._plannerHandleLifeSheetClick=gn,window._plannerHandleShareClick=Cn,window._plannerMemberById=Lt,window._plannerMembers=It,window._plannerNextMonth=en,window._plannerNudges=En,window._plannerOpenDaySheet=ln,window._plannerOpenDetail=_n,window._plannerOpenLifeSheet=mn,window._plannerOpenModalFromSheet=pn,window._plannerOpenShare=xn,window._plannerPrevMonth=$t,window._plannerRecurrenceLabel=Vt,window._plannerRenderDaySheetList=un,window._plannerSelectDay=tn,window._plannerSetView=on,window._plannerShareVia=Tn,window._plannerToggleFilter=cn,window._plannerToggleSection=sn,window._plannerVisibleEvents=Ht,window._pmDpClear=Yn,window._pmDpNext=Qn,window._pmDpOpen=Gn,window._pmDpOutsideClick=Kn,window._pmDpPrev=Zn,window._pmDpRender=qn,window._pmDpSelect=Jn,window._pmDpToday=Xn,window._pmFmtDate=zn,window._pmFmtDateShort=Bn,window._pmHandleCatChange=Wn,window._pmRenderMemberPicker=Vn,window._pmToggleAllDay=Un,window._pmToggleMember=Hn,window._prevForecastMonth=Kt,window._psoHoldEnd=Up,window._psoHoldStart=Hp,window._psoKey=Wp,window._psoRender=Vp,window._psoSubmit=Gp,window._psoTourDone=Kp,window._pushAllEventsToBudget=tr,window._qaKey=Is,window._qaSelectCat=Ls,window._qahAction=Ms,window._qahApplyParsed=Ps,window._qahSendText=Ns,window._recordInviteAcceptance=Op,window._refreshSetupProgress=Xm,window._renderAdultPinModal=Js,window._renderApiKeySummary=Co,window._renderContextBanners=Fm,window._renderCsvPreview=xs,window._renderCsvReview=Cs,window._renderInviteFlow=jp,window._renderLifeAreas=Pi,window._renderLifeScore=Gm,window._renderListItem=di,window._renderListsDetail=ui,window._renderListsSelector=li,window._renderMealPlan=Pr,window._renderMissionCard=Km,window._renderNudgeSection=ir,window._renderPinDots=$f,window._renderPinPad=ep,window._renderPlannerAgenda=nn,window._renderPlannerEventRow=Qt,window._renderPlannerMonthGrid=Xt,window._renderPlannerWeekStrip=Zt,window._renderQAHub=js,window._renderQASheet=Fs,window._renderShoppingList=Wr,window._renderTourSlide=wp,window._renderWeekStrip=Ni,window._resetPinAttempts=Yp,window._saveInviteIncome=Ep,window._sectionTag=Nm,window._secureClear=Lf,window._secureGet=Ff,window._securePrewarm=Pf,window._secureSet=If,window.prefsGet=Ff,window.prefsSet=If,window.prefsClear=Lf,window._setHouseholdOwner=cf,window._setInviteRole=sp,window._setPinHardLock=Zp,window._showInviteA1=yp,window._showInviteA3=bp,window._showInviteA4=Cp,window._showInviteExpired=Ap,window._showInviteIncomePrompt=Tp,window._showMissionLightbox=Vm,window._showParentLockNotification=$p,window._showPinScreen=Qf,window._showToast=Yr,window._startFirestoreSync=_f,window._syncVehicleBill=j,window._tdCloseSheet=Bi,window._tdOpenHeadsUpSheet=Li,window._tdOpenSheet=zi,window._tdOpenSlippingSheet=Ri,window._tdToggleStep=Vi,window._ticker=jo,window._tlItem=Pm,window._todayAllocSegments=ki,window._totoAppendMessage=yr,window._totoInitPanel=mr,window._totoRemoveTyping=xr,window._totoSend=_r,window._totoSendSuggestion=hr,window._totoShowTyping=br,window._updatePillsOverflow=p,window._updateSwitchBtn=np,window._verifyPin=Qp,window.activateTab=h,window.addActualEntry=Om,window.addCatToGroup=uo,window.addCategory=Eo,window.addHouseholdMember=Na,window.addPet=Ia,window.addShopItem=Gr,window.addSubFromImport=lt,window.adjForm=xa,window.applianceForm=yi,window.applianceFromForm=bi,window.applyCsvImport=Os,window.approveSuggestion=An,window.assignDevice=Jf,window.aud=b,window.audD=x,window.billCatIcon=pt,window.billDueBadge=mt,window.billMonthlyEquiv=ht,window.billsModal=yt,window.calcFinancialHealth=jm,window.calcLifeScore=Im,window.calcScenario=pa,window.cancelSettingsChanges=vo,window.clearActivityLog=To,window.clearAdultPin=Ws,window.clearCheckedShopItems=Jr,window.clearDeviceProfile=Bf,window.clearKidPin=Hs,window.clearKidSession=Uf,window.closeBillModal=xt,window.closeModal=Io,window.closeNWModal=Ke,window.closeNWTargetModal=Ve,window.closePinSetupOverlay=Bp,window.closeQuickAdd=As,window.closeSubModal=at,window.closeTotoAssistant=pr,window.completeMission=zm,window.completeWeeklyReset=Wm,window.confirmScope=sm,window.copyInviteLink=pp,window.copyMonthFromPrevious=am,window.customSelect=Sf,window.cyclePantryStatus=Oe,window.deleteAdjustment=wa,window.deleteAppliance=Ci,window.deleteBill=wt,window.deleteCategoryGroup=co,window.deleteDoc=me,window.deleteExpense=us,window.deleteExtra=$o,window.deleteFurniture=vi,window.deleteGoal=la,window.deleteIncome=is,window.deleteMaint=Se,window.deleteNWItem=Je,window.deletePantryItem=je,window.deletePlannerEvent=Pn,window.deleteScenario=ba,window.deleteService=se,window.deleteStage=Ho,window.deleteSub=st,window.deleteVariation=qo,window.deleteVehicle=ae,window.detectSpendingPatterns=Ka,window.dismissMission=Rm,window.dismissSubResult=ut,window.dismissSuggestion=jn,window.doScopeAll=lm,window.doScopeMonth=cm,window.dpClearDate=_s,window.dpDateInput=ds,window.dpNextMonth=hs,window.dpPrevMonth=ms,window.dpSelectDate=gs,window.editActual=Am,window.ensureMonthOverride=rm,window.escAttr=_,window.escHtml=g,window.estimateAllEvents=Yt,window.estimatePlannerEvent=Fn,window.expenseCategories=ea,window.expenseForm=as,window.expenseFromForm=os,window.exportData=Yi,window.extraForm=Yo,window.extraFromForm=Xo,window.fmtDate=C,window.fmtNW=S,window.freqDisplay=ee,window.freqDisplayItem=E,window.freqLabel=T,window.freqLabelItem=te,window.furnitureForm=mi,window.furnitureFromForm=hi,window.generateInvite=cp,window.generateMission=Lm,window.generateShoppingList=fi,window.generateSmartInsights=Ya,window.generateSmartInsightsHTML=to,window.getAIKey=Ua,window.getActual=Tm,window.getActualEntries=wm,window.getBenchmarkStatus=Va,window.getBenchmarks=Ba,window.getCategoryHistoryData=Ga,window.getDeviceProfile=Rf,window.getKidSession=Vf,window.getLast6Months=Cm,window.getMonthData=tm,window.getSeasonalNudges=rr,window.goToPlannerDay=rn,window.goalForm=ia,window.goalFromForm=oa,window.goalProgress=na,window.guestMode=mf,window.handleCsvFile=bs,window.handleDeviceRouting=Kf,window.handleSubCSV=ct,window.importData=Xi,window.incomeCategories=ta,window.incomeForm=es,window.incomeFromForm=ts,window.inviteMember=lp,window.isMonthCustomized=nm,window.isOverdue=w,window.itemMonthly=D,window.loadColors=Da,window.loadData=Ef,window.logActivity=ff,window.markBillPaid=Tt,window.markChoreChildView=Dc,window.markGoalAchieved=ua,window.markMaintDone=Ce,window.monthLabel=xm,window.monthShortLabel=Sm,window.monthlyTotal=O,window.nextId=k,window.nextInsightsMonth=ro,window.nextMoneyMonth=Ti,window.nextMonth=bm,window.nwItemRow=Le,window.openActualEditor=Dm,window.openAddAdjustment=Ca,window.openAddAppliance=xi,window.openAddCatToGroup=fo,window.openAddCategoryGroup=oo,window.openAddExpense=cs,window.openAddExtra=Zo,window.openAddFurniture=gi,window.openAddGoal=sa,window.openAddIncome=ns,window.openAddScenario=va,window.openAddStage=Bo,window.openAddVariation=Go,window.openBillModal=bt,window.openCsvImport=vs,window.openDatePicker=fs,window.openDocForm=fe,window.openEditAppliance=Si,window.openEditContractTotal=Lo,window.openEditExpense=ls,window.openEditExtra=Qo,window.openEditFurniture=_i,window.openEditGoal=ca,window.openEditIncome=rs,window.openEditScenario=ya,window.openEditStage=Vo,window.openEditVariation=Ko,window.openEmojiPickerModal=ao,window.openIconPickerForGroup=so,window.openMaintForm=be,window.openMealEdit=Lr,window.openModal=R,window.openNWModal=Ge,window.openNWTargetModal=Be,window.openNavGroupFor=nf,window.openPantryForm=ke,window.openPinSetup=zp,window.openPlannerModal=Rn,window.openQuickAdd=ks,window.openServiceForm=oe,window.openSubModal=it,window.openTotoAssistant=fr,window.openVehicleForm=re,window.openWeeklyReset=Um,window.ordinal=rh,window.pantryToShoppingList=Ne,window.parseBankCSV=ys,window.prevInsightsMonth=no,window.prevMoneyMonth=wi,window.prevMonth=ym,window.prevMonthStr=im,window.profileAdults=ja,window.profileChildren=Ma,window.quickAddMaint=we,window.quickAddPantry=Me,window.redeemPrizeChildView=Oc,window.removeActualEntry=km,window.removeApiKey=So,window.removeCatFromGroup=po,window.removeCategory=Do,window.removeHouseholdMember=Pa,window.removePet=La,window.removeShopItem=qr,window.renderAll=pm,window.renderBenchmarksSection=$a,window.renderBills=gt,window.renderBudget=No,window.renderBudgetForecast=er,window.renderBudgetSuggestions=Mn,window.renderBuild=$i,window.renderCategoryBreakdown=qa,window.renderDashboard=Ki,window.renderDocuments=de,window.renderDpCalendar=ps,window.renderExpenseGroups=Oo,window.renderForecast=Jt,window.renderGoals=ra,window.renderInsightCards=Qa,window.renderInsights=eo,window.renderLists=ci,window.renderLunchbox=Uc,window.renderMaintenance=ye,window.renderMeals=Nr,window.renderMoneyDashboard=Ei,window.renderNWDebtCard=ze,window.renderNWTargetCard=Re,window.renderNWTrend=Ue,window.renderNetWorth=Ie,window.renderPantry=De,window.renderPlanner=F,window.renderRoutines=U,window.renderScenarios=ma,window.renderSettings=qi,window.renderSetupProgress=Di,window.renderSpendingPatterns=Ja,window.renderSubImportResults=rt,window.renderSubscriptions=vt,window.renderToday=Ii,window.renderVehicles=ne,window.resetAllData=Ji,window.resetKidPinLock=em,window.revokeInvite=hp,window.runAIInsights=Za,window.runCsvCategorise=Ss,window.safeRender=um,window.sanitiseState=A,window.saveAIKey=Wa,window.saveApiKey=xo,window.saveBill=Ct,window.saveColors=Oa,window.saveData=q,window.saveDoc=pe,window.saveMaint=xe,window.saveMealSlot=Hr,window.saveNWItem=qe,window.saveNWSnapshot=Ye,window.saveNWTarget=He,window.savePantryItem=Ae,window.savePlannerEvent=$n,window.saveQuickAdd=Rs,window.saveService=M,window.saveSettingsChanges=_o,window.saveSub=ot,window.saveVehicle=ie,window.scenarioForm=ga,window.scenarioFromForm=_a,window.sendInviteEmail=mp,window.sendTotoMessage=gr,window.setActual=Em,window.setAdultPin=Us,window.setBillsFilter=_t,window.setBudgetView=Qi,window.setDeviceProfile=zf,window.setExpenseFilter=eh,window.setKidPin=Vs,window.setKidSession=Hf,window.setupProgressTasks=qm,window.showChildView=nc,window.showDeviceSetup=qf,window.showProfileSelector=Yf,window.signInWithGoogle=pf,window.signOutUser=hf,window.sortExpenses=th,window.stageForm=Ro,window.stageFromForm=zo,window.subCatIcon=$e,window.subMonthlyAmount=et,window.suggestEventToBudget=kn,window.switchProfile=Bs,window.switchToKidMode=kc,window.thSort=nh,window.toggleAdjFields=Sa,window.toggleBillDayField=St,window.toggleBudgetDetail=Fo,window.toggleCustomFreq=ss,window.toggleGoalFields=aa,window.toggleGroupExpand=Zi,window.toggleInsightSheet=Hi,window.toggleNavGroup=tf,window.togglePlannerCard=Dn,window.togglePlannerEstimate=On,window.toggleScenario=ha,window.toggleSettingsSection=wo,window.toggleShopItem=Kr,window.toggleSidebar=$d,window.toggleTotoAssistant=dr,window.uid=mm,window.unpushEventFromBudget=Nn,window.updateCars=za,window.updateCategoryGroup=lo,window.updateColor=Aa,window.updateMember=Fa,window.updatePet=Ra,window.updateSetting=bo,window.upgradeSelects=wf,window.variationForm=Uo,window.variationFromForm=Wo,window.LB_ALLERGIES=Hc,window.LB_SLOTS=Vc,window._estimateLbCalories=Yc,window._lbActiveKid=Bc,window._lbWeekOffset=0,window._saveLbSlot=Jc,window.aiPlanLunchbox=Xc,window.deleteLbProfile=Kc,window.lbToShoppingList=Zc,window.openLunchboxEdit=qc,window.openLunchboxProfile=Wc,window.saveLbProfile=Gc,window.HEALTH_GUIDES=cl,window._cleanupGuide=sl,window._cvReadOnly=ul,window._cvViewCalendar=fl,window._guideCleanup=tl,window._guideIdx=el,window._guideSteps=$c,window._highlightStep=il,window._recurrenceMatchesDate=Et,window._showGuideStep=rl,window.endGuide=ol,window.exitChildView=pl,window.nextGuideStep=al,window.startGuide=nl,window.startHealthGuide=ll,window.toggleHealthPopover=Qc,window.viewChildToday=dl,window.ROUTINE_SUGGESTIONS=gl,window.SUGG_PREVIEW=3,window._assignmentCompletedToday=El,window._assignmentHistory=Ol,window._assignmentStreak=Dl,window._renderAdultRoutines=zl,window._renderChildRoutines=ql,window._renderRoutinesTodayCard=Il,window._renderSuggestionsSection=Wl,window._routineActiveTab=Ll,window._routineAddStep=uu,window._routineAddSuggestion=Kl,window._routineAssertScope=Nl,window._routineAvailableSuggestions=Ml,window._routineAwardPoints=Ql,window._routineAwardStepPoints=Zl,window._routineCheckDailyReset=ml,window._routineCompletedToday=kl,window._routineCreate=Su,window._routineCurrentUserId=vl,window._routineDateKey=_l,window._routineDelete=wu,window._routineDeleteChild=Jl,window._routineDeleteStep=mu,window._routineDragEnd=lu,window._routineDragIdx=au,window._routineDragOver=su,window._routineDragStart=ou,window._routineDrop=cu,window._routineDuplicateFromJoined=ju,window._routineDuplicateTo=ku,window._routineEdit=pu,window._routineEditStep=hu,window._routineEditSuggestion=Gl,window._routineExpandSugg=Ul,window._routineGetAssignment=Tl,window._routineHistory=jl,window._routineIntelNudge=Fl,window._routineIsOwner=xl,window._routineJoin=Au,window._routineKids=Sl,window._routineLeave=Tu,window._routineManageAssignments=gu,window._routineMatchesDate=hl,window._routineNextId=Cl,window._routineOtherAdults=wl,window._routinePauseMenu=tu,window._routinePropagateStepAdd=du,window._routinePropagateStepDelete=fu,window._routineRecurrenceCollect=xu,window._routineRecurrenceFormHtml=vu,window._routineRecurrenceSummaryUpdate=bu,window._routineRecurrenceTypeChange=yu,window._routineRemovePause=nu,window._routineResetToday=$l,window._routineResetTodayAllKids=iu,window._routineResetTodayKid=ru,window._routineSaveValidated=Pl,window._routineSetTab=Rl,window._routineShareMenu=Du,window._routineShowHistory=Mu,window._routineSkipDay=eu,window._routineStreak=Al,window._routineSuggCollapsed=Bl,window._routineSuggExpanded=Vl,window._routineTodayKey=H,window._routineToggleAssignment=_u,window._routineToggleShare=Ou,window._routineToggleStep=Yl,window._routineToggleStepKid=Xl,window._routineToggleSugg=Hl,window._routineTypeSelect=Cu,window._routineUnassign=Eu,window._routinesForCurrentUser=yl,window._routinesForHousehold=bl,window.CHILD_EVENT_EMOJIS=Wu,window.CHORE_EMOJIS=nd,window.KID_EMOJIS=td,window.PRIZE_EMOJIS=rd,window._deleteChildEvent=Ku,window._openChildEventModal=Gu,window._renderChildEventsMgmt=Uu,window.addSavings=gd,window.approveCompletion=Ju,window.approveRedemption=Zu,window.deleteChore=$u,window.deleteKid=ud,window.deletePrize=ed,window.emojiPicker=id,window.kidBalance=Iu,window.kidsParentTab=Pu,window.kidsView=Nu,window.markChoreDone=qu,window.openAddKidModal=sd,window.openChoreModal=dd,window.openEditKidModal=cd,window.openPrizeModal=pd,window.openSavingsModal=hd,window.pickEmoji=ad,window.pickedEmoji=od,window.rejectCompletion=Yu,window.rejectRedemption=Qu,window.renderApprovals=Vu,window.renderChoreMgmt=zu,window.renderKidView=Hu,window.renderKids=W,window.renderKidsOverview=Ru,window.renderKidsParent=Lu,window.renderPrizeMgmt=Bu,window.requestRedemption=Xu,window.saveChore=fd,window.saveKid=ld,window.savePrize=md,window.addLink=Ad,window.attachBtn=Td,window.deleteReceiptById=Cd,window.fileIcon=Dd,window.fileSizeStr=Ed,window.fundingBadge=wd,window.getDB=yd,window.getReceipts=xd,window.installApp=Id,window.openReceiptsModal=Od,window.refreshReceiptCounts=bd,window.removeReceipt=Pd,window.renderReceiptsList=kd,window.saveReceipt=Sd,window.uploadFiles=Md,window.uploadReceiptFiles=jd,window.viewReceipt=Nd,window.OB_EXPENSE_PRESETS=Ld,window.hideOnboarding=Vd,window.obBack=Yd,window.obFinish=Qd,window.obNext=Jd,window.obPickEmoji=Gd,window.obSetAdults=Hd,window.obSetKids=Ud,window.obSkip=Xd,window.obSkipExpenses=Zd,window.obStepPosition=zd,window.obStepSequence=Rd,window.obToggleEmojiPicker=Wd,window.obToggleExpenseSkip=Kd,window.renderObStep=qd,window.showOnboarding=Bd;
//# sourceMappingURL=index-jCrWVfdB.js.map